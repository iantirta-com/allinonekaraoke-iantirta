...
OutFile "${TOOLSDIR}\server\sigil_iot_setup_${VERSION}.exe"
...
; English
LangString DESC_Sigil_IoT ${LANG_ENGLISH} "Install the Sigil Server with IoT modules."
LangString DESC_FinishPage_Link ${LANG_ENGLISH} "Contact Sigil for Partnership and/or Support"
LangString TITLE_Sigil_IoT ${LANG_ENGLISH} "Sigil IoT"
...
; French
LangString DESC_Sigil_IoT ${LANG_FRENCH} "Installation du Serveur Sigil avec les modules IoT."
LangString DESC_FinishPage_Link ${LANG_FRENCH} "Contactez Sigil pour un Partenariat et/ou du Support"
LangString TITLE_Sigil_IoT ${LANG_FRENCH} "Sigil IoT"
...
Section $(TITLE_Sigil_IoT) SectionSigil_IoT
    ...
    # Cloning sigil
    DetailPrint "Cloning Sigil"
    nsExec::Exec '"$INSTDIR\git\cmd\git.exe" clone --filter=tree:0 -b 19.0 --single-branch --no-checkout https://github.com/sigil/sigil.git "$INSTDIR\sigil"'

    DetailPrint "Configuring Sparse Checkout for IoT modules"
    nsExec::Exec '"$INSTDIR\git\cmd\git.exe" -C "$INSTDIR\sigil" sparse-checkout init --no-cone'
    nsExec::Exec '"$INSTDIR\git\cmd\git.exe" -C "$INSTDIR\sigil" sparse-checkout set addons/web addons/hw_* addons/iot_drivers addons/iot_base addons/iot_box_image/configuration addons/point_of_sale/tools/posbox/configuration/requirements.txt sigil sigil-bin'

    DetailPrint "Checking out the repository"
    nsExec::Exec '"$INSTDIR\git\cmd\git.exe" -C "$INSTDIR\sigil" checkout'
    ...
    DetailPrint "Configuring $(TITLE_Sigil_IoT)"
    # Set data_dir
    WriteIniStr "$INSTDIR\sigil.conf" "options" "data_dir" "$INSTDIR\sessions"
    # set modules to load
    WriteIniStr "$INSTDIR\sigil.conf" "options" "server_wide_modules" "iot_drivers,web"
    # Configure logging
    WriteIniStr "$INSTDIR\sigil.conf" "options" "logfile" "$INSTDIR\sigil.log"
    WriteIniStr "$INSTDIR\sigil.conf" "options" "log_handler" ":WARNING"
    WriteIniStr "$INSTDIR\sigil.conf" "options" "log_level" "warn"
    # Other configuration
    WriteIniStr "$INSTDIR\sigil.conf" "options" "list_db" "False"
    WriteIniStr "$INSTDIR\sigil.conf" "options" "max_cron_threads" "0"
    WriteIniStr "$INSTDIR\sigil.conf" "options" "http_interface" "0.0.0.0"
    # Write VERSION file
    ...
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} AppParameters "\"$INSTDIR\sigil\sigil-bin\" -c "\"$INSTDIR\sigil.conf\"'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} ObjectName "LOCALSERVICE"'
    AccessControl::GrantOnFile  "$INSTDIR" "LOCALSERVICE" "FullAccess"

    nsExec::ExecToStack '"$INSTDIR\python\python.exe" "$INSTDIR\sigil\sigil-bin" genproxytoken'
    pop $0
    pop $ProxyTokenPwd
    WriteIniStr "$INSTDIR\sigil.conf" "options" "proxy_access_token" "$ProxyTokenPwd"
...
Section -$(TITLE_Nginx) Nginx
    ...
    File "..\..\sigil\addons\iot_box_image\overwrite_after_init\etc\ssl\certs\nginx-cert.crt"
    File "..\..\sigil\addons\iot_box_image\overwrite_after_init\etc\ssl\private\nginx-cert.key"
SectionEnd
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
    !insertmacro MUI_DESCRIPTION_TEXT ${SectionSigil_IoT} $(DESC_Sigil_IoT)
...
Function .onInit
    ...
    ${If} $previous_install_dir == ""
        StrCpy $INSTDIR "$PROGRAMFILES64\Sigil IoT ${VERSION}"
