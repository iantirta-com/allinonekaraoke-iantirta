...
version_info = ...
nt_service_name = ...
exec(open(os.path.join(ROOTDIR, 'sigil', 'release.py'), 'rb').read())
...
DOCKERUSER = """
RUN mkdir /var/lib/sigil && \
    groupadd -g %(group_id)s sigil && \
    useradd -u %(user_id)s -g sigil sigil -d /var/lib/sigil && \
    mkdir /data && \
    chown sigil:sigil /var/lib/sigil /data
USER sigil
""" % {'group_id': os.getgid(), 'user_id': os.getuid()}


class SigilTestTimeoutError(Exception):
    pass


class SigilTestError(Exception):
    pass
...
def _rpc_count_modules(addr='http://127.0.0.1', port=8069, dbname='mycompany'):
    ...
    if len(modules) > 1:
        ...
        if toinstallmodules:
            ...
            raise SigilTestError("Installation of package failed")
        else:
        ...
        raise SigilTestError("Package test: FAILED. Not able to install base.")
...
def publish(args, pub_type, extensions):
    ...
    for extension in extensions:
        release = glob("%s/sigil_*.%s" % (args.build_dir, extension))
        if release:
...
def _prepare_build_dir(args, win32=False, move_addons=True):
    ...
    run_cmd(cmd + ['%s/' % args.sigil_dir, args.build_dir])
    if not move_addons:
        return
    for addon_path in glob(os.path.join(args.build_dir, 'addons/*')):
        if args.blacklist is None or os.path.basename(addon_path) not in args.blacklist:
            try:
                shutil.move(addon_path, os.path.join(args.build_dir, 'sigil/addons'))
...
class Docker():
    ...
    def __init__(self, args):
        ...
        """
        self.args = args
        self.tag = 'sigil-%s-%s-nightly-tests' % (DOCKERVERSION, self.arch)
        ...
        self.docker_template = Path(docker_templates[self.arch]).read_text(encoding='utf-8').replace('USER sigil', DOCKERUSER)
    ...
    def run(self, cmd, build_dir, container_name, user='sigil', exposed_port=None, detach=False, timeout=None):
    ...
    def test_sigil(self):
        logging.info('Starting to test Sigil install test')
        start_time = time.time()
        while self.is_running() and (time.time() - start_time) < INSTALL_TIMEOUT:
            time.sleep(5)  # give some time for sigil to install and start
            if os.path.exists(os.path.join(args.build_dir, 'sigil.pid')):
                try:
        ...
        if self.is_running():
            self.stop()
            raise SigilTestTimeoutError('Sigil pid file never appeared after %s sec' % INSTALL_TIMEOUT)
        raise SigilTestError('Error while installing/starting Sigil after %s sec.\nSee testlogs.txt in build dir' % int(time.time() - start_time))
...
class DockerTgz(Docker):
    ...
    def build(self):
        ...
        self.run('python3 setup.py sdist --quiet --formats=gztar,zip', self.args.build_dir, 'sigil-src-build-%s' % TSTAMP)
        os.rename(glob('%s/dist/sigil-*.tar.gz' % self.args.build_dir)[0], '%s/sigil_%s.%s.tar.gz' % (self.args.build_dir, VERSION, TSTAMP))
        os.rename(glob('%s/dist/sigil-*.zip' % self.args.build_dir)[0], '%s/sigil_%s.%s.zip' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'su postgres -s /bin/bash -c "createuser -s sigil"',
            'su sigil -s /bin/bash -c "python3 -m venv /var/lib/sigil/sigilvenv"',
            'su sigil -s /bin/bash -c "/var/lib/sigil/sigilvenv/bin/python3 -m pip install --upgrade pip"',
            'su sigil -s /bin/bash -c "/var/lib/sigil/sigilvenv/bin/python3 -m pip install -r /opt/release/requirements.txt"',
            f'su sigil -s /bin/bash -c "/var/lib/sigil/sigilvenv/bin/python3 -m pip install /data/src/sigil_{VERSION}.{TSTAMP}.tar.gz"',
            'su sigil -s /bin/bash -c "createdb mycompany"',
            'su sigil -s /bin/bash -c "/var/lib/sigil/sigilvenv/bin/sigil -d mycompany -i base --stop-after-init"',
            'su sigil -s /bin/bash -c "/var/lib/sigil/sigilvenv/bin/sigil -d mycompany --pidfile=/data/src/sigil.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'sigil-src-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_sigil()
...
class DockerDeb(Docker):
    ...
    def build(self):
        ...
        changelog = fr"sigil ({VERSION}.{TSTAMP}) stable; urgency=low\n\n  * {VERSION}\n\n -- Sigil Bot <info@iantirta.com>  {TFULL}\n"
        ...
        cmds.append('mv ../sigil_* ./')
        self.run(' && '.join(cmds), self.args.build_dir, 'sigil-deb-build-%s' % TSTAMP)
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            f'/usr/bin/apt-get install -y /data/src/sigil_{VERSION}.{TSTAMP}_all.deb',
            'su sigil -s /bin/bash -c "sigil -d mycompany -i base --pidfile=/data/src/sigil.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'sigil-deb-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_sigil()
...
class DockerRpm(Docker):
    ...
    def build(self):
        ...
        rpmbuild_dir = '/var/lib/sigil/rpmbuild'
        build_date = datetime.now().strftime('%a %b %d %Y')
        cmds = [
            'cd /data/src',
            'mkdir -p dist',
            'rpmdev-setuptree -d',
            f'cp -a /data/src/setup/rpm/sigil.spec {rpmbuild_dir}/SPECS/',
            f'tar --transform "s/^\\./sigil-{VERSION}/" -c -z -f {rpmbuild_dir}/SOURCES/sigil-{VERSION}.tar.gz .',
            f'rpmbuild -bb --define="%version {VERSION}" --define "%release {TSTAMP}" --define "%build_date {build_date}" /data/src/setup/rpm/sigil.spec',
            f'mv {rpmbuild_dir}/RPMS/noarch/sigil*.rpm /data/src/dist/'
        ]
        self.run(' && '.join(cmds), self.args.build_dir, f'sigil-rpm-build-{TSTAMP}')
        os.rename(glob('%s/dist/sigil-*.noarch.rpm' % self.args.build_dir)[0], '%s/sigil_%s.%s.rpm' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'sleep 5',
            'su postgres -c "createuser -s sigil"',
            'su sigil -c "createdb mycompany"',
            'dnf install -q /data/src/sigil_%s.%s.rpm -y' % (VERSION, TSTAMP),
            'su sigil -s /bin/bash -c "sigil -c /etc/sigil/sigil.conf -d mycompany -i base --stop-after-init"',
            'su sigil -s /bin/bash -c "sigil -c /etc/sigil/sigil.conf -d mycompany --pidfile=/data/src/sigil.pid"',
        ]
        self.run(' && '.join(cmds), args.build_dir, 'sigil-rpm-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_sigil()
    ...
    def gen_rpm_repo(self, args, rpm_filepath):
        ...
        self.run('createrepo /data/src/', temp_path, 'sigil-rpm-createrepo-%s' % TSTAMP)
...
class DockerWine(Docker):
    ...
    def __init__(self, args):
        ...
        self.nsi_filepath = r"c:\sigilbuild\server\setup\win32\setup.nsi"
    ...
    def build(self):
        ...
        container_python = '/var/lib/sigil/.wine/drive_c/sigilbuild/WinPy64/python-3.12.3.amd64/python.exe'
        ...
        bundle_po_files_cmd = (
            'cd /data/src && '
            'find sigil/addons -name "*.po" > .i18n_list.txt && '
        ...
        self.run(' && '.join(cmds), self.args.build_dir, 'sigil-win-build-%s' % TSTAMP)
...
class DockerIot(DockerWine):
    ...
    def __init__(self, args):
        ...
        self.nsi_filepath = r"c:\sigilbuild\server\setup\win32\setup-iot.nsi"
        self.nt_service_name = "sigil-iot"
        self.bundle_po_files = False

    def build_image(self):
        shutil.copy(os.path.join(self.args.build_dir, 'sigil/addons/iot_box_image/configuration/requirements.txt'), self.docker_dir / 'requirements-iot.txt')
