...
from sigil.modules.module import load_script
from sigil.orm.registry import Registry
from sigil.tools.misc import file_path
from sigil.tools.parse_version import parse_version

if typing.TYPE_CHECKING:
    from collections.abc import Iterator
    from sigil.sql_db import Cursor
...
VERSION_RE = re.compile(
    r"""^
        # Optional prefix with Sigil version
        ((
            6\.1|
        ...
        # After Sigil version we allow precisely 2 or 3 parts
...
class MigrationManager:
    ...
    def _get_files(self) -> None:
        def _get_upgrade_path(pkg: str) -> Iterator[str]:
            for path in sigil.upgrade.__path__:
    ...
    def migrate_module(self, pkg: module_graph.ModuleNode, stage: typing.Literal['pre', 'post', 'end']) -> None:
        ...
        def compare(version: str) -> bool:
            ...
            if majorless_version:
                # We should not re-execute major-less scripts when upgrading to new Sigil version
...
def exec_script(cr, installed_version, pyfile, addon, stage, version=None):
    ...
    if ext.lower() != '.py':
        return
    try:
        mod = load_script(pyfile, f"sigil.upgrade.{addon}.{version}.{name}")
