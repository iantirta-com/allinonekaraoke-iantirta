""" Initialize the database for module management and Sigil installation. """
...
if typing.TYPE_CHECKING:
    from sigil.sql_db import BaseCursor, Cursor
...
def is_initialized(cr: Cursor) -> bool:
    ...
    """
    return sigil.tools.sql.table_exists(cr, 'ir_module_module')
...
def initialize(cr: Cursor) -> None:
    ...
    """
    try:
        f = sigil.tools.misc.file_path('base/data/base_data.sql')
    except FileNotFoundError:
        m = "File not found: 'base.sql' (provided by module 'base')."
        _logger.critical(m)
        raise OSError(m)

    with sigil.tools.misc.file_open(f) as base_sql_file:
        cr.execute(base_sql_file.read())  # pylint: disable=sql-injection

    for info in sigil.modules.Manifest.all_addon_manifests():
    ...
    from sigil.tools import config  # noqa: PLC0415
...
def has_trigram(cr: BaseCursor) -> bool:
    ...
    pg_trgm module but any similar function will be picked by Sigil.
