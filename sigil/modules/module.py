...
import sigil.tools as tools
import sigil.upgrade
...
class UpgradeHook:
    """Makes the legacy `migrations` package being `sigil.upgrade`"""

    def find_spec(self, fullname, path=None, target=None):
        if re.match(r"^sigil\.addons\.base\.maintenance\.migrations\b", fullname):
    ...
    def load_module(self, name):
        ...
        canonical_upgrade = name.replace("sigil.addons.base.maintenance.migrations", "sigil.upgrade")
...
def initialize_sys_path() -> None:
    """
    Setup the addons path ``sigil.addons.__path__`` with various defaults
    ...
    for path in (
        ...
        *tools.config['addons_path'],
        ...
        if os.access(path, os.R_OK) and path not in sigil.addons.__path__:
            sigil.addons.__path__.append(path)

    # hook sigil.upgrade on upgrade-path
    ...
    for up in tools.config['upgrade_path'] or [legacy_upgrade_path]:
        if up not in sigil.upgrade.__path__:
            sigil.upgrade.__path__.append(up)

    # create decrecated module alias from sigil.addons.base.maintenance.migrations to sigil.upgrade
    spec = importlib.machinery.ModuleSpec("sigil.addons.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = sigil.upgrade  # type: ignore
    sys.modules["sigil.addons.base.maintenance"] = maintenance_pkg
    sys.modules["sigil.addons.base.maintenance.migrations"] = sigil.upgrade

    # hook for upgrades and namespace freeze
    if not getattr(initialize_sys_path, 'called', False):  # only initialize once
        sigil.addons.__path__._path_finder = lambda *a: None  # prevent path invalidation
        sigil.upgrade.__path__._path_finder = lambda *a: None  # prevent path invalidation
...
class Manifest(Mapping[str, typing.Any]):
    ...
    def _get_manifest_from_addons(module: str) -> Manifest | None:
        ...
        for adp in sigil.addons.__path__:
...
def get_resource_from_path(path: str) -> tuple[str, str, str] | None:
    ...
    """
    resource = None
    sorted_paths = sorted(sigil.addons.__path__, key=len, reverse=True)
...
def load_openerp_module(module_name: str) -> None:
    ...
    qualname = f'sigil.addons.{module_name}'
