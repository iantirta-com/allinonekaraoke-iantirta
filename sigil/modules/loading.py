...
import sigil.sql_db
import sigil.tools.sql
import sigil.tools.translate
from sigil import api, tools
from sigil.tools import OrderedSet
from sigil.tools.convert import convert_file, IdRef, ConvertMode as LoadMode
...
if typing.TYPE_CHECKING:
    ...
    from sigil.sql_db import BaseCursor
...
def load_data(env: Environment, idref: IdRef, mode: LoadMode, kind: LoadKind, package: ModuleNode) -> bool:
    ...
    for k in keys:
        if k == 'init_xml' and package.manifest[k]:
            _logger.warning("module %s: key 'init_xml' is deprecated in Sigil 19.", package.name)
...
def load_module_graph(
    ...
    report: SigilTestResult | None = None,
    ...
    assert isinstance(env.cr, sigil.sql_db.Cursor), "Need for a real Cursor to load modules"
    ...
    t0 = time.time()
    loading_extra_query_count = sigil.sql_db.sql_counter
    ...
    for index, package in enumerate(graph, 1):
        ...
        module_extra_query_count = sigil.sql_db.sql_counter
        ...
        if tools.config['test_enable'] and (update_operation or not update_from_config):
            ...
            if suite.countTestCases():
                ...
                # Python tests
                tests_t0, tests_q0 = time.time(), sigil.sql_db.sql_counter
                ...
                test_queries = sigil.sql_db.sql_counter - tests_q0

                # tests may have reset the environment
                module = env['ir.module.module'].browse(module_id)


        extra_queries = sigil.sql_db.sql_counter - module_extra_query_count - test_queries
        extras = []
        if test_queries:
    ...
    _logger.runbot("%s modules loaded in %.2fs, %s queries (+%s extra)",
                   ...
                   sigil.sql_db.sql_counter - loading_extra_query_count)  # extra queries: testes, notify, any other closed cursor
    ...
    with registry.cursor() as cr:
        ...
        if update_module and upgrade_modules:
            for pyfile in tools.config['pre_upgrade_scripts']:
                sigil.modules.migration.exec_script(cr, graph['base'].installed_version, pyfile, 'base', 'pre')
        ...
        if update_module and tools.sql.table_exists(cr, 'ir_model_fields'):
            ...
            if sigil.tools.sql.column_exists(cr, 'ir_model_fields', 'company_dependent'):
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                for pkg in pkgs:
                    ...
                    if uninstall_hook:
                        py_module = sys.modules['sigil.addons.%s' % (pkg.name,)]
...
def reset_modules_state(db_name: str) -> None:
    ...
    with db.cursor() as cr:
        if not sigil.tools.sql.table_exists(cr, 'ir_module_module'):
