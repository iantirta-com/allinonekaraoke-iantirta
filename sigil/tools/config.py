...
from sigil import release
from sigil.tools.func import classproperty
from . import appdirs
...
class _SigilOption(optparse.Option):
    ...
    def __init__(self, *opts, **attrs):
        ...
        if env_name is None and is_new_option and self.file_loadable:
            ...
            self.env_name = 'SIGIL_' + self.dest.upper()
...
class _FileOnlyOption(_SigilOption):
...
class _PosixOnlyOption(_SigilOption):
    def __init__(self, *opts, **attrs):
        if os.name != 'posix':
...
class configmanager:
    ...
    @property
    def rcfile(self):
        self._warn("Since 19.0, use sigil.tools.config['config'] instead", DeprecationWarning, stacklevel=2)
        return self['config']

    @rcfile.setter
    def rcfile(self, rcfile):
        self._warn(f"Since 19.0, use sigil.tools.config['config'] = {rcfile!r} instead", DeprecationWarning, stacklevel=2)
        self._runtime_options['config'] = rcfile

    def _build_cli(self):
        SigilOption = type('SigilOption', (_SigilOption,), {'config': self})
        FileOnlyOption = type('FileOnlyOption', (_FileOnlyOption, SigilOption), {})
        PosixOnlyOption = type('PosixOnlyOption', (_PosixOnlyOption, SigilOption), {})

        version = "%s %s" % (release.description, release.version)
        parser = optparse.OptionParser(version=version, option_class=SigilOption)
        ...
        group.add_option("-D", "--data-dir", dest="data_dir", type='path',  # sensitive default set in _load_default_options
                         help="Directory where to store Sigil data")
        ...
        temp_tests_dir = os.path.join(tempfile.gettempdir(), 'sigil_tests')
        ...
        group.add_option('--log-handler', action="append", type='comma', my_default=[':INFO'], metavar="MODULE:LEVEL",
                         help='setup a handler at LEVEL for a given MODULE. An empty MODULE indicates the root logger. '
                              'This option can be repeated. Example: "sigil.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")')
        group.add_option('--log-web', action="append_const", dest="log_handler", const=("sigil.http:DEBUG",),
                         help='shortcut for --log-handler=sigil.http:DEBUG')
        group.add_option('--log-sql', action="append_const", dest="log_handler", const=("sigil.sql_db:DEBUG",),
        ...
        group.add_option("--db_app_name", dest="db_app_name", my_default="sigil-{pid}", env_name='PGAPPNAME',
        ...
        group = optparse.OptionGroup(parser, "Internationalisation options",
            "Use these options to translate Sigil to another language. "
        ...
        group.add_option('--dev', dest='dev_mode', type='comma', metavar="FEATURE,...", my_default=[], file_exportable=False, env_name='SIGIL_DEV',
    ...
    def parse_config(self, args: list[str] | None = None, *, setup_logging: bool | None = None) -> None:
        ...
        Typical usage of this method:
            ...
            sigil.tools.config.parse_config(sys.argv[1:])
        """
        from sigil import modules, netsvc  # noqa: PLC0415
        ...
        if setup_logging is not False:
            ...
            if setup_logging is None:
                warnings.warn(
                    "As of Sigil 18, it's recommended to specify whether"
                    " you want Sigil to setup its own logging (or want to"
                    " handle it yourself)",
