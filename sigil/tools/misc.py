"""
Miscellaneous tools used by Sigil.
"""
...
from sigil.loglevels import exception_to_unicode, get_encodings, ustr  # noqa: F401
...
if typing.TYPE_CHECKING:
    ...
    from sigil.addons.base.models.res_lang import LangData
...
def file_path(file_path: str, filter_ext: tuple[str, ...] = ('',), env: Environment | None = None, *, check_exists: bool = True) -> str:
    ...
    if not is_abs and (module := sys.modules.get(f'sigil.addons.{file_path_split[0]}')):
        ...
        addons_paths = [*sigil.addons.__path__, root_path, *temporary_paths]
...
def file_open(name: str, mode: str = "r", filter_ext: tuple[str, ...] = (), env: Environment | None = None):
    ...
    Examples::
        ...
        >>> with file_open('/opt/sigil/addons/hr/static/description/icon.png', 'rb') as f:
        ...     contents = f.read()
...
def file_open_temporary_directory(env: Environment):
    ...
    Examples::
        ...
        >>> with sigil.tools.file_open_temporary_directory(self.env) as module_dir:
        ...    with zipfile.ZipFile('foo.zip', 'r') as z:
        ...        z.extract('foo/__manifest__.py', module_dir)
        ...    with sigil.tools.file_open('foo/__manifest__.py', env=self.env) as f:
        ...        manifest = f.read()
...
class mute_logger(logging.Handler):
    ...
    Can be used as context manager or decorator::
        ...
        @mute_logger('sigil.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('sigil.foo.bar'):
            do_suff()
    """
...
def format_date(
    ...
    if not value:
        return ''
    from sigil.fields import Datetime  # noqa: PLC0415
...
def limited_field_access_token(record, field_name, timestamp=None, *, scope):
    ...
    :type record: class:`sigil.models.Model`
