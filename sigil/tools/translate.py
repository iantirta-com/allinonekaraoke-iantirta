...
if typing.TYPE_CHECKING:
    from sigil.api import Environment
...
PYTHON_TRANSLATION_COMMENT = 'sigil-python'

# translation used for javascript code in web client
JAVASCRIPT_TRANSLATION_COMMENT = 'sigil-javascript'
...
def get_translated_module(arg: str | int | typing.Any) -> str:  # frame not represented as hint
    ...
    if isinstance(arg, str):
        if arg.startswith('sigil.addons.'):
            # get the name of the module
            return arg.split('.')[2]
        if '.' in arg or not arg:
            # module name is not in sigil.addons.
            return 'base'
        else:
            return arg
    else:
        ...
        if not frame:
            return 'base'
        if (module_name := frame.f_globals.get("__name__")) and module_name.startswith('sigil.addons.'):
            # just a quick lookup because `get_resource_from_path is slow compared to this`
            return module_name.split('.')[2]
        path = inspect.getfile(frame)
        from sigil.modules import get_resource_from_path  # noqa: PLC0415
...
def _get_cr(frame):
    ...
    if (local_self := frame.f_locals.get('self')) is not None:
        ...
        if (cr := getattr(local_self, 'cr', None)) is not None:
            return cr
    if (req := sigil.http.request) and (env := req.env):
        return env.cr
    return None
...
def _get_lang(frame, default_lang='') -> str:
    ...
    # get from request?
    if (req := sigil.http.request) and (env := req.env) and (lang := env.lang):
        return lang
    ...
    uid = _get_uid(frame)
    if cr and uid:
        from sigil import api  # noqa: PLC0415
...
def translation_file_reader(source, fileformat='po', module=None):
    """ Iterate over translation file to return Sigil translation entries """
    if fileformat == 'csv':
...
class PoFileReader:
    """ Iterate over po file to return Sigil translation entries """
    def __init__(self, source):
...
class PoFileWriter:
    ...
    def write_rows(self, rows):
        ...
        import sigil.release as release
...
class TranslationModuleReader(TranslationReader):
    ...
    def __init__(self, cr, modules=None, lang=None):
        ...
        self._path_list = [(path, True) for path in sigil.addons.__path__]
    ...
    def _babel_extract_terms(self, fname, path, root, extract_method='sigil.tools.babel:extract_python', trans_type='code',
        ...
        for (path, recursive) in self._path_list:
            ...
            for root, _dummy, files in os.walk(path, followlinks=True):
                for fname in fnmatch.filter(files, '*.py'):
                    self._babel_extract_terms(fname, path, root, 'sigil.tools.babel:extract_python',
                                              extra_comments=[PYTHON_TRANSLATION_COMMENT],
                                              extract_keywords={'_': None, '_lt': None})
                if fnmatch.fnmatch(root, '*/static/src*'):
                    # Javascript source files
                    for fname in fnmatch.filter(files, '*.js'):
                        self._babel_extract_terms(fname, path, root, 'sigil.tools.babel:extract_javascript',
                                                  extra_comments=[JAVASCRIPT_TRANSLATION_COMMENT],
                                                  extract_keywords={'_t': None})
                    # QWeb template files
                    for fname in fnmatch.filter(files, '*.xml'):
                        self._babel_extract_terms(fname, path, root, 'sigil.tools.translate:babel_extract_qweb',
                                                  extra_comments=[JAVASCRIPT_TRANSLATION_COMMENT])
                if fnmatch.fnmatch(root, '*/data/*'):
                    for fname in filter(spreadsheet_files_regex.match, files):
                        self._babel_extract_terms(fname, path, root, 'sigil.tools.translate:extract_spreadsheet_terms',
                                                  extra_comments=[JAVASCRIPT_TRANSLATION_COMMENT])
                if not recursive:
...
def get_datafile_translation_path(module_name: str) -> Iterator[str]:
    from sigil.modules import Manifest  # noqa: PLC0415
...
def _get_translation_upgrade_queries(cr, field):
    ...
    """
    from sigil.modules.registry import Registry  # noqa: PLC0415
