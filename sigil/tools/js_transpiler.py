"""
This code is what let us use ES6-style modules in sigil.
Classic Sigil modules are composed of a top-level :samp:`sigil.define({name},{dependencies},{body_function})` call.
This processor will take files starting with an `@sigil-module` annotation (in a comment) and convert them to classic modules.
If any file has the ``/** sigil-module */`` on top of it, it will get processed by this class.
It performs several operations to get from ES6 syntax to the usual sigil one with minimal changes.
...
from sigil.tools.misc import OrderedSet


def transpile_javascript(url, content):
    """
    Transpile the code from native JS modules to custom sigil modules.
    ...
    legacy_sigil_define = get_aliased_sigil_define_content(module_path, content)
    ...
    steps = [
        ...
        partial(wrap_with_sigil_define, module_path, dependencies),
        partial(convert_t, url)
    ]
    for s in steps:
        content = s(content)
    if legacy_sigil_define:
        content += legacy_sigil_define
    return content
...
def url_to_module_path(url):
    """
    Sigil modules each have a name. (sigil.define("<the name>", [<dependencies>], function (require) {...});
...
def wrap_with_sigil_define(module_path, dependencies, content):
    """
    Wraps the current content (source code) with the sigil.define call.
    ...
    """
    return f"""sigil.define({module_path!r}, {list(dependencies)}, function (require) {{
'use strict';
let __exports = {{}};
{content}
return __exports;
...
SIGIL_MODULE_RE = re.compile(r"""
    \s*                                # starting white space
    \/(\*|\/)                          # /* or //
    .*                                 # any comment in between (optional)
    @sigil-module                       # '@sigil-module' statement
...
def is_sigil_module(url, content):
    """
    Detect if the file is a native sigil module.
    We look for a comment containing @sigil-module.

    :param url:
    :param content: source code
    :return: is this a sigil module that need transpilation ?
    """
    result = SIGIL_MODULE_RE.match(content)
...
def get_aliased_sigil_define_content(module_path, content):
    """
    ...
    Example:
    If we have a require call somewhere in the sigil source base being:
    ...
    > sigil.define("web/chrome/abstract_action", ['web.AbstractAction'], function (require) {
    ...
    .. code-block:: javascript
        ...
        /** @sigil-module */
        // after
        /** @sigil-module alias=web.AbstractAction */
    ...
    .. code-block:: javascript
        ...
        /** @sigil-module */
        // after
        /** @sigil-module alias=web.AbstractAction default=false */

    :return: the alias content to append to the source code.
    """
    matchobj = SIGIL_MODULE_RE.match(content)
    if matchobj:
        alias = matchobj['alias']
        if alias:
            if matchobj['default']:
                return """\nsigil.define(`%s`, ['%s'], function (require) {
                        return require('%s');
