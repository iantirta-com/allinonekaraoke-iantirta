...
def _get_eval_context(self, env, model_str):
    from sigil import fields, release  # noqa: PLC0415
    ...
    if node.tag in ('field','value'):
        ...
        if a_eval := node.get('eval'):
            ...
            try:
                ...
                logging.getLogger('sigil.tools.convert.init').error(
...
form: module.record_id""" % (xml_id,)
    ...
    def _tag_menuitem(self, rec, parent=None):
        ...
        from sigil.fields import Command  # noqa: PLC0415
        groups = []
    ...
    def _tag_root(self, el):
        for rec in el:
            ...
            try:
                ...
                raise ParseError(msg) from None  # Restart with "--log-handler sigil.tools.convert:DEBUG" for complete traceback
...
def convert_file(
    ...
    if kind is not None:
        warnings.warn(
            "The `kind` argument is deprecated in Sigil 19.",
            DeprecationWarning,
            stacklevel=2,
        )
    if pathname is None:
