...
from sigil import modules
from sigil.tools.arabic_reshaper import reshape
from sigil.tools.parse_version import parse_version
from sigil.tools.misc import file_open, SENTINEL
...
def add_banner(pdf_stream, text=None, logo=False, thickness=SENTINEL):
    """ Add a banner on a PDF in the upper right corner, with Sigil's logo (optionally).

    :param pdf_stream (BytesIO):    The PDF stream where the banner will be applied.
    :param text (str):              The text to be displayed.
    :param logo (bool):             Whether to display Sigil's logo in the banner.
    ...
    with file_open('base/static/img/main_partner-image.png', mode='rb') as f:
        sigil_logo_file = io.BytesIO(f.read())
    sigil_logo = Image.open(sigil_logo_file)
    sigil_color = colors.Color(113 / 255, 75 / 255, 103 / 255, 0.8)
    ...
    for p in range(old_pdf.getNumPages()):
        ...
        logo and can.drawImage(
            ImageReader(sigil_logo), 0.25 * thickness, -2.05 * thickness, 40, 40, mask='auto', preserveAspectRatio=True)
...
class SigilPdfFileReader(PdfFileReader):
...
class SigilPdfFileWriter(PdfFileWriter):
    ...
    def embed_sigil_attachment(self, attachment, subtype=None, afrelationship='/Data'):
        assert attachment, "embed_sigil_attachment cannot be called without attachment."
