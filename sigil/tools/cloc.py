...
class Cloc(object):
    ...
    def count_path(self, path, exclude=None):
        path = path.rstrip('/')
        exclude_list = []
        for i in sigil.modules.module.MANIFEST_NAMES:
    ...
    def count_modules(self, env):
        ...
        exclude_path = {
            ...
            if (m := sigil.modules.Manifest.for_addon(name, display_warning=False))
        }
        ...
        for module_name in module_list:
            manifest = sigil.modules.Manifest.for_addon(module_name)
    ...
    def count_customization(self, env):
        ...
        # the module should be sigil/studio unless it comes from an imported module install
    ...
    def count_database(self, database):
        registry = sigil.modules.registry.Registry(database)
    ...
    def report(self, verbose=False, width=None):
        ...
        # Render
        s = fmt.format(k="Sigil cloc", lines="Line", other="Other", code="Code")
        s += hr
