...
def csv_reader(stream, **params):
    warnings.warn("Deprecated since Sigil 18.0: can just use `csv.reader` with a text stream or use `TextIOWriter` or `codec.getreader` to transcode.", DeprecationWarning, stacklevel=2)
...
def csv_writer(stream, **params):
    warnings.warn("Deprecated since Sigil 18.0: can just use `csv.writer` with a text stream or use `TextIOWriter` or `codec.getwriter` to transcode.", DeprecationWarning, stacklevel=2)
    assert not isinstance(stream, io.TextIOBase), \
...
def to_text(source: typing.Any) -> str:
    ...
    * rest is textified
    """
    warnings.warn("Deprecated since Sigil 18.0.", DeprecationWarning, stacklevel=2)
