...
from sigil import api, sql_db
from sigil.modules.registry import Registry
from sigil.release import nt_service_name
from sigil.tools import config, gc, osutil, OrderedSet, profiler
from sigil.tools.cache import log_ormcache_stats
from sigil.tools.misc import stripped_sys_argv, dumpstacks
from sigil.tools.osutil import memory_info
...
def set_limit_memory_hard():
    ...
    import sigil  # for eventd
    if sigil.evented and config['limit_memory_hard_gevent']:
...
class RequestHandler(CommonRequestHandler):
    def setup(self):
        ...
        me.name = 'sigil.service.http.request.%s' % (me.ident,)
...
class ThreadedWSGIServerReloadable(LoggingBaseWSGIServerMixIn, werkzeug.serving.ThreadedWSGIServer):
    ...
    def __init__(self, host, port, app):
        # The SIGIL_MAX_HTTP_THREADS environment variable allows to limit the amount of concurrent
        ...
        self.max_http_threads = os.environ.get("SIGIL_MAX_HTTP_THREADS")
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        import sigil.addons  # noqa: PLC0415
        for path in sigil.addons.__path__:
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="sigil.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        ...
        from sigil.addons.base.models.ir_cron import IrCron  # noqa: PLC0415
    ...
    def cron_spawn(self):
        ...
        for i in range(config['max_cron_threads']):
            t = threading.Thread(target=self.cron_thread, args=(i,), name=f"sigil.service.cron.cron{i}")
            t.daemon = True
            t.type = 'cron'
            t.start()
    ...
    def run(self, preload=None, stop=False):
        ...
        if stop:
            if config['test_enable']:
                from sigil.tests.result import _logger as logger  # noqa: PLC0415
...
class PreforkServer(CommonServer):
    ...
    def start(self):
        ...
        if config['http_enable']:
            ...
            if os.environ.get('SIGIL_HTTP_SOCKET_FD'):
                # reload
                self.socket = socket.socket(fileno=int(os.environ.pop('SIGIL_HTTP_SOCKET_FD')))
    ...
    def fork_and_reload(self):
        ...
        if pid != 0:
            ...
            os.environ['SIGIL_HTTP_SOCKET_FD'] = str(http_socket_fileno)
            os.environ['SIGIL_READY_SIGHUP_PID'] = str(pid)
    ...
    def run(self, preload, stop):
        ...
        if os.environ.get('SIGIL_READY_SIGHUP_PID'):
            os.kill(int(os.environ.pop('SIGIL_READY_SIGHUP_PID')), signal.SIGHUP)
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('sigil: %s %s %s' % (self.__class__.__name__, self.pid, title))
...
class WorkerHTTP(Worker):
    ...
    def __init__(self, multi):
        ...
        # The SIGIL_HTTP_SOCKET_TIMEOUT environment variable allows to control socket timeout for
        ...
        sock_timeout = os.environ.get("SIGIL_HTTP_SOCKET_TIMEOUT")
...
class WorkerCron(Worker):
    ...
    def start(self):
        ...
        if registries_size := os.environ.get('SIGIL_REGISTRY_LRU_SIZE_CRON'):
...
def load_server_wide_modules():
    from sigil.modules.module import load_openerp_module  # noqa: PLC0415
...
def preload_registries(dbnames):
    ...
    registries_size = int(os.environ.get('SIGIL_REGISTRY_LRU_SIZE') or 0)
    ...
    if registry_idle_timeout := os.environ.get("SIGIL_REGISTRY_MAX_IDLE_TIMEOUT"):
        Registry.idle_timeout = int(registry_idle_timeout)

    for dbname in dbnames:
        if os.environ.get('SIGIL_PROFILE_PRELOAD'):
            interval = float(os.environ.get('SIGIL_PROFILE_PRELOAD_INTERVAL', '0.1'))
            collectors = [profiler.PeriodicCollector(interval=interval)]
            if os.environ.get('SIGIL_PROFILE_PRELOAD_SQL'):
        ...
        try:
            with preload_profiler:
                ...
                if config['test_enable']:
                    from sigil.tests import loader  # noqa: PLC0415
                    t0 = time.time()
...
def start(preload=None, stop=False):
    """ Start the sigil http server and cron processor.
    """
    global server

    load_server_wide_modules()
    import sigil.http  # noqa: PLC0415

    if sigil.evented:
        server = GeventServer(sigil.http.root)
    elif config['workers']:
        if config['test_enable']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(sigil.http.root)
    else:
        if platform.system() == "Linux" and sys.maxsize > 2**32 and "MALLOC_ARENA_MAX" not in os.environ:
            ...
            # which Sigil is based upon in order to limit the memory usage for threaded workers.
        ...
        server = ThreadedServer(sigil.http.root)

    watcher = None
    if 'reload' in config['dev_mode'] and not sigil.evented:
        if inotify:
