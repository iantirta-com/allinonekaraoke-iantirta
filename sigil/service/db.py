...
import sigil.api
import sigil.modules.neutralize
import sigil.release
import sigil.sql_db
import sigil.tools
from sigil.exceptions import AccessDenied
from sigil.release import version_info
from sigil.sql_db import db_connect
from sigil.tools import osutil, SQL
from sigil.tools.misc import exec_pg_environ, find_pg_tool
...
def check_db_management_enabled(func, /):
    ...
    def if_db_mgt_enabled(*args, **kwargs):
        if not sigil.tools.config['list_db']:
...
def check_super(passwd):
    if passwd and sigil.tools.config.verify_admin_password(passwd):
        return True
    raise sigil.exceptions.AccessDenied()


# This should be moved to sigil.modules.db, along side initialize().
def _initialize_db(db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        sigil.tools.config['load_language'] = lang

        registry = sigil.modules.registry.Registry.new(db_name, update_module=True, new_db_demo=demo)

        with closing(registry.cursor()) as cr:
            env = sigil.api.Environment(cr, sigil.api.SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = sigil.tools.email_split(login)
                if emails:
...
def _check_faketime_mode(db_name):
    if os.getenv('SIGIL_FAKETIME_TEST_MODE') and db_name in sigil.tools.config['db_name']:
        try:
            db = sigil.sql_db.db_connect(db_name)
...
def _create_empty_database(name):
    db = sigil.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = sigil.tools.config['db_template']
    ...
    try:
        db = sigil.sql_db.db_connect(name)
        with db.cursor() as cr:
            cr.execute("CREATE EXTENSION IF NOT EXISTS pg_trgm")
            if sigil.tools.config['unaccent']:
                ...
                # But in the case of Sigil, we consider that those data don't
...
def exp_duplicate_database(db_original_name, db_name, neutralize_database=False):
    ...
    sigil.sql_db.close_db(db_original_name)
    db = sigil.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        # database-altering operations cannot be executed inside a transaction
        cr._cnx.autocommit = True
    ...
    registry = sigil.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = sigil.api.Environment(cr, sigil.api.SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)
        if neutralize_database:
            sigil.modules.neutralize.neutralize_database(cr)

    from_fs = sigil.tools.config.filestore(db_original_name)
    to_fs = sigil.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    sigil.modules.registry.Registry.delete(db_name)
    sigil.sql_db.close_db(db_name)
    ...
    fs = sigil.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'sigil_dump': '1',
        'db_name': cr.dbname,
        'version': sigil.release.version,
        'version_info': sigil.release.version_info,
        'major_version': sigil.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip', with_filestore=True):
    ...
    if backup_format == 'zip':
        with tempfile.TemporaryDirectory() as dump_dir:
            if with_filestore:
                filestore = sigil.tools.config.filestore(db_name)
...
def restore_db(db, dump_file, copy=False, neutralize_database=False):
    ...
    with tempfile.TemporaryDirectory() as dump_dir:
        ...
        registry = sigil.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = sigil.api.Environment(cr, sigil.api.SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
...
def exp_rename(old_name, new_name):
    sigil.modules.registry.Registry.delete(old_name)
    sigil.sql_db.close_db(old_name)
    ...
    old_fs = sigil.tools.config.filestore(old_name)
    new_fs = sigil.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    sigil.tools.config.set_admin_password(new_password)
    sigil.tools.config.save(['admin_passwd'])
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        sigil.modules.registry.Registry.new(db, update_module=True, upgrade_modules={'base'})
    return True
...
@sigil.tools.mute_logger('sigil.sql_db')
...
def list_dbs(force=False):
    if not sigil.tools.config['list_db'] and not force:
        raise sigil.exceptions.AccessDenied()

    if not sigil.tools.config['dbfilter'] and sigil.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, Sigil will not
        ...
        return sorted(sigil.tools.config['db_name'])
...
def list_db_incompatible(databases):
    """"Check a list of databases if they are compatible with this version of Sigil
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if sigil.tools.sql.table_exists(cr, 'ir_module_module'):
    ...
    for database_name in incompatible_databases:
        # release connection
        sigil.sql_db.close_db(database_name)
...
def exp_list_lang():
    return sigil.tools.misc.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(sigil.tools.config.root_path, 'addons/base/data/res_country_data.xml')).getroot()
