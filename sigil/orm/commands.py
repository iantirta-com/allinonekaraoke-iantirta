...
class Command(enum.IntEnum):
    """
    :class:`~sigil.fields.One2many` and :class:`~sigil.fields.Many2many` fields
    ...
    def create(cls, values: ValuesType) -> CommandValue:
        ...
        In case of a :class:`~sigil.fields.Many2many` relation, one unique
        new record is created in the comodel such that all records in `self`
        are linked to the new record.

        In case of a :class:`~sigil.fields.One2many` relation, one new record
    ...
    def delete(cls, id: int) -> CommandValue:
        ...
        In case of a :class:`~sigil.fields.Many2many` relation, removing the
    ...
    def unlink(cls, id: int) -> CommandValue:
        ...
        In case of a :class:`~sigil.fields.One2many` relation, the given record
