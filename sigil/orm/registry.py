...
from sigil.tools.func import locked, reset_cached_properties
from sigil.tools.lru import LRU
from sigil.tools.misc import Collector, format_frame
from sigil.tools.version_tag_reset import reset_classes_tp_versions_used
...
if typing.TYPE_CHECKING:
    ...
    from sigil.fields import Field
    from sigil.models import BaseModel
    from sigil.sql_db import BaseCursor, Connection, Cursor
    from sigil.modules import module_graph


_logger = logging.getLogger('sigil.registry')
_schema = logging.getLogger('sigil.schema')
...
class Registry(Mapping[str, type["BaseModel"]]):
        ...
        *,
        ...
        try:
            ...
            with registry.cursor() as cr:
                ...
                from sigil.modules import db  # noqa: PLC0415
                if db.is_initialized(cr):
                    cr.execute("DELETE FROM ir_config_parameter WHERE key='base.partially_updated_database'")
                    if cr.rowcount:
                        update_module = True
            from sigil.http import borrow_request  # noqa: PLC0415
            # This should be a method on Registry
            from sigil.modules.loading import load_modules, reset_modules_state  # noqa: PLC0415
    ...
    def init(self, db_name: str) -> None:
        ...
        if config['test_enable']:
            from sigil.tests.result import SigilTestResult  # noqa: PLC0415
            self._assertion_report: SigilTestResult | None = SigilTestResult()
        else:
        ...
        for indexname, tablename, field in expected:
            ...
            if will_index:
                ...
                if index == 'trigram':
                    ...
                    from sigil.modules.db import FunctionStatus  # noqa: PLC0415
