...
from sigil.tools import float_compare, float_is_zero, float_repr, float_round
from sigil.tools.misc import SENTINEL, Sentinel
...
class Float(Field[float]):
    ...
    :param digits: a pair (total, decimal) or a string referencing a
        :class:`~sigil.addons.base.models.decimal_precision.DecimalPrecision` record name.
    ...
    :func:`~sigil.fields.Float.round()` to round a float with the given precision.
    :func:`~sigil.fields.Float.is_zero()` to check if a float equals zero at the given precision.
    :func:`~sigil.fields.Float.compare()` to compare two floats at the given precision.
...
class Monetary(Field[float]):
    ...
    :class:`res_currency<sigil.addons.base.models.res_currency.Currency>`.
    ...
    :param str currency_field: name of the :class:`Many2one` field
        holding the :class:`res_currency <sigil.addons.base.models.res_currency.Currency>`
    ...
    def convert_to_cache(self, value, record, validate=True):
        ...
        if value and validate:
            # FIXME @rco-sigil: currency may not be already initialized if it is
    ...
    def _filter_not_equal(self, records: BaseModel, cache_value: typing.Any) -> BaseModel:
        ...
        # see fix sigil/sigil#177200 (commit 7164d5295904b08ec3a0dc1fb54b217671ff531c)
        env = records.env
