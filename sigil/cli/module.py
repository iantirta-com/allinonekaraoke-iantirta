...
from sigil import SUPERUSER_ID
from sigil.api import Environment
from sigil.cli.command import Command
from sigil.modules.loading import force_demo
from sigil.modules.module import get_module_path, initialize_sys_path
from sigil.modules.registry import Registry
from sigil.tools import OrderedSet, config, parse_version
...
class Module(Command):
    ...
    def __init__(self, *args, **kwargs):
        ...
        install_parser.epilog = textwrap.dedent("""\
            Before installing modules, an Sigil database needs to be created and initialized
            on your PostgreSQL instance, using the `db init` command:

            $ sigil-bin db init <db_name>

            To get help on its parameters, see:

            $ sigil-bin db init --help
        """)
