...
from sigil.tools import SQL, config

_logger = logging.getLogger(__name__)


class Obfuscate(Command):
    """Obfuscate data in a given sigil database"""
    def __init__(self):
        super().__init__()
        self.cr = None
    ...
    def set_pwd(self, pwd):
        ...
        self.cr.execute("INSERT INTO ir_config_parameter (key, value) VALUES ('sigil_cyph_pwd', 'sigil_cyph_'||encode(pgp_sym_encrypt(%s, %s), 'base64')) ON CONFLICT(key) DO NOTHING", [pwd, pwd])
    ...
    def check_pwd(self, pwd):
        ...
        try:
            query = SQL("SELECT %s FROM ir_config_parameter WHERE key='sigil_cyph_pwd'", uncypher_pwd)
    ...
    def clear_pwd(self):
        ...
        self.cr.execute("DELETE FROM ir_config_parameter WHERE key='sigil_cyph_pwd' ")

    def cypher_string(self, sql_field: SQL, password):
        # don't double cypher fields
        return SQL("""CASE WHEN starts_with(%(field_name)s, 'sigil_cyph_') THEN %(field_name)s ELSE 'sigil_cyph_'||encode(pgp_sym_encrypt(%(field_name)s, %(pwd)s), 'base64') END""", field_name=sql_field, pwd=password)

    def uncypher_string(self, sql_field: SQL, password):
        return SQL("""CASE WHEN starts_with(%(field_name)s, 'sigil_cyph_') THEN pgp_sym_decrypt(decode(substring(%(field_name)s, 11)::text, 'base64'), %(pwd)s) ELSE %(field_name)s END""", field_name=sql_field, pwd=password)
