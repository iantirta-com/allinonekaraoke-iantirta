...
import sigil.cli
from sigil.modules import initialize_sys_path, load_script
...
def load_internal_commands():
    """ Load ``commands`` from ``sigil.cli`` """
    for path in sigil.cli.__path__:
        for module in Path(path).iterdir():
            if module.suffix != '.py':
                continue
            __import__(f'sigil.cli.{module.stem}')
    ...
    for path in sigil.addons.__path__:
        for fullpath in Path(path).glob(f'*/cli/{command}.py'):
            if (found_command := fullpath.stem) and Command.is_valid_name(found_command):
                # loading as sigil.cli and not sigil.addons.{module}.cli
                # so it doesn't load sigil.addons.{module}.__init__
                mapping[f'sigil.cli.{found_command}'] = fullpath 
...
def find_command(name: str) -> Command | None:
    ...
    # import from sigil.cli
    with contextlib.suppress(ImportError):
        __import__(f'sigil.cli.{name}')
        return commands[name]

    # import from sigil.addons.*.cli
...
def main():
    ...
    if command := find_command(command_name):
        sigil.cli.COMMAND = command_name
        command().run(args)
    else:
        message = (
