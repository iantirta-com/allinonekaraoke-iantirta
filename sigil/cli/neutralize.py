...
class Neutralize(Command):
    ...
    def run(self, args):
        parser = sigil.tools.config.parser
        ...
        opt = sigil.tools.config.parse_config(args, setup_logging=True)
        ...
        try:
            with sigil.sql_db.db_connect(dbname).cursor() as cursor:
                if opt.to_stdout:
                    installed_modules = sigil.modules.neutralize.get_installed_modules(cursor)
                    queries = sigil.modules.neutralize.get_neutralization_queries(installed_modules)
                    ...
                    sigil.modules.neutralize.neutralize_database(cursor)
