...
* addon: the sigil addon in which the file is;
...
try:
    ...
    from sigil.modules import initialize_sys_path
    from sigil.tools import config, parse_version
except ImportError:
    # Assume the script is directy executed (by opposition to be
    # executed via sigil-bin), happily release/parse_version are
    # standalone so we can hack our way there without importing sigil
    sys.path.insert(0, str(ROOT))
    sys.path.insert(0, str(ROOT / 'tools'))
    import release
    from parse_version import parse_version
    # Avoid shadowing stdlib modules when running upgrade scripts, e.g. json with sigil/tools/json.py
    del sys.path[:2]
    class Command:
        ...
        def __init__(self):
            self.parser = argparse.ArgumentParser(
                ...
                description=__doc__.replace('/sigil/upgrade_code', str(UPGRADE)),
...
class UpgradeCode(Command):
    """ Rewrite the entire source code using the scripts found at /sigil/upgrade_code """
    name = 'upgrade_code'
    ...
    def run(self, cmdargs):
        ...
        if initialize_sys_path:
            ...
            initialize_sys_path()
            options.addons_path = sigil.addons.__path__
