...
from sigil.release import author as __author__  # noqa: F401
from sigil.release import version as __version__  # noqa: F401
from sigil.service import server
from sigil.tools import config

from . import Command

# Also use the `sigil` logger for the main script.
_logger = logging.getLogger('sigil')
...
def report_configuration():
    ...
    import sigil.release  # noqa: PLC0415
    _logger.info("Sigil version %s", sigil.release.version)
    if os.path.isfile(config['config']):
        _logger.info("Using configuration file at %s", config['config'])
    _logger.info('addons paths: %s', sigil.addons.__path__)
    ...
    if sys.version_info[:2] > sigil.release.MAX_PY_VERSION:
        _logger.warning("Python %s is not officially supported, please use Python %s instead",
            '.'.join(map(str, sys.version_info[:2])),
            '.'.join(map(str, sigil.release.MAX_PY_VERSION))
        )
...
def setup_pid_file():
    ...
    This function assumes the configuration has been initialized.
    """
    import sigil  # for evented  # noqa: PLC0415
    if not sigil.evented and config['pidfile']:
        pid = os.getpid()
...
def main(args):
    ...
    for db_name in config['db_name']:
        from sigil.service import db  # noqa: PLC0415
        try:
...
class Server(Command):
    """Start the sigil server (default command)"""
