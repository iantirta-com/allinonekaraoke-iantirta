...
def log(logger, level, prefix, msg, depth=None):
    warnings.warn(
        "sigil.netsvc.log is deprecated starting Sigil 18, use normal logging APIs",
...
class PostgreSQLHandler(logging.Handler):
    ...
    def __init__(self, log_db=None):
        ...
        if self._log_db != '%d':
            with contextlib.suppress(Exception), tools.mute_logger('sigil.sql_db'), sql_db.db_connect(self._log_db, allow_uri=True).cursor() as cr:
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with contextlib.suppress(Exception), tools.mute_logger('sigil.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings('ignore', category=BytesWarning, module='sigil.tools.image')
    ...
    if conf:
        ...
        if not conf.get('keep_sigil_default', False):
            return
    ...
    if os.name == 'posix' and isinstance(handler, logging.StreamHandler) and (is_a_tty(handler.stream) or os.environ.get("SIGIL_PY_COLORS")):
...
DEFAULT_LOG_CONFIGURATION = [
    'sigil.http.rpc.request:INFO',
    'sigil.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['sigil:DEBUG', 'sigil.sql_db:INFO', 'sigil.http.rpc:DEBUG'],
    'debug_rpc': ['sigil:DEBUG', 'sigil.sql_db:INFO', 'sigil.http.rpc.request:DEBUG'],
    'debug': ['sigil:DEBUG', 'sigil.sql_db:INFO'],
    'debug_sql': ['sigil.sql_db:DEBUG'],
    'info': [],
    'runbot': ['sigil:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['sigil:WARNING', 'werkzeug:WARNING'],
    'error': ['sigil:ERROR', 'werkzeug:ERROR'],
    'critical': ['sigil:CRITICAL', 'werkzeug:CRITICAL'],
}
...
def showwarning_with_traceback(message, category, filename, lineno, file=None, line=None):
    ...
    for frame in traceback.extract_stack():
        if frame.name == '__call__' and frame.filename.endswith('/sigil/http.py'):
