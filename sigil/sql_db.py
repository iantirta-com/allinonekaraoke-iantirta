...
if typing.TYPE_CHECKING:
    ...
    from sigil.orm.environments import Transaction
...
MAX_IDLE_TIMEOUT = int(os.getenv("SIGIL_DB_MAX_IDLE_TIMEOUT", "600"))
...
class Cursor(BaseCursor):
    ...
    def __init__(self, pool: ConnectionPool, dbname: str, dsn: dict):
        ...
        if os.getenv('SIGIL_FAKETIME_TEST_MODE') and self.dbname in tools.config['db_name']:
    ...
    def execute_values(self, query, argslist, template=None, page_size=100, fetch=False):
        ...
        """
        # Sigil Cursor only proxies all methods of psycopg2 Cursor. This is a patch for problems caused by passing
...
def connection_info_for(db_or_uri: str, readonly=False) -> tuple[str, dict]:
    ...
    if 'SIGIL_PGAPPNAME' in os.environ:
        warnings.warn("Since 19.0, use PGAPPNAME instead of SIGIL_PGAPPNAME", DeprecationWarning)
        app_name = os.environ['SIGIL_PGAPPNAME']
...
def db_connect(to: str, allow_uri=False, readonly=False) -> Connection:
    ...
    maxconn = (tools.config['db_maxconn_gevent'] if hasattr(sigil, 'evented') and sigil.evented else 0) or tools.config['db_maxconn']
...
def close_db(db_name: str) -> None:
    """ You might want to call sigil.modules.registry.Registry.delete(db_name) along this function."""
    if _Pool:
