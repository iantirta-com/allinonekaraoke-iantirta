r"""\
Sigil HTTP layer / WSGI application
...
:class:`~sigil.http.Controller`: class and its companion the
:func:`~sigil.http.route`: method decorator. Together they are used to
...
Application.__call__
  ...
  request and itself in an Sigil http request. The Sigil http request is
...
import sigil.init  # import first for core setup
...
Sigil URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.iantirta.com/documentation/master/developer/reference/addons/http.html#csrf
for more details.

* if this endpoint is accessed through Sigil via py-QWeb form, embed a CSRF
...
<!-- Alternatively, use the X-Sigil-Database header. -->
"""
...
SESSION_ROTATION_INTERVAL_HEADER_SKIP = "X-Sigil-Skip-Session-Rotation-Interval"
...
def db_list(force=False, host=None):
    ...
    :param bool force: See :func:`~sigil.service.db.list_dbs`:
    ...
    """
    try:
        dbs = sigil.service.db.list_dbs(force)
...
def db_filter(dbs, host=None):
    ...
    if config['db_name']:
        # In case --db-filter is not provided and --database is passed, Sigil will
...
def dispatch_rpc(service_name, method, params):
    ...
    """
    rpc_dispatchers = {
        'common': sigil.service.common.dispatch,
        'db': sigil.service.db.dispatch,
        'object': sigil.service.model.dispatch,
    }
...
class Stream:
    ...
    def from_path(cls, path, filter_ext=('',), public=False):
        ...
        :param path: See :func:`~sigil.tools.file_path`
        :param filter_ext: See :func:`~sigil.tools.file_path`
    ...
    def get_response(
        ...
        **send_file_kwargs
        ...
        :param send_file_kwargs: Other keyword arguments to send to
            :func:`sigil.tools._vendor.send_file.send_file` instead of
...
class Controller:
    ...
    :class:`~sigil.http.Controller` can use the :func:`~sigil.http.route`:
    ...
    :func:`~sigil.http.route`:. Please note that the decorators of all
    ...
    .. code-block:
        ...
        class GreetingController(sigil.http.Controller):
    ...
    def __init_subclass__(cls):
        ...
        if Controller in cls.__bases__:
            ...
            module = path[2] if path[:2] == ['sigil', 'addons'] else ''
...
def route(route=None, **routing):
    ...
    .. warning::
        ...
        :class:`~sigil.http.Controller` for more details.
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    ...
    def is_valid(cls):
        ...
        path = cls.__module__.split('.')
        return path[:2] == ['sigil', 'addons'] and path[2] in modules
    ...
    def build_controllers():
        ...
        installed modules). Modules in this context are Sigil addons.
        """
        # Controllers defined outside of sigil addons are outside of the
        # controller inheritance/extension mechanism.
        yield from (ctrl() for ctrl in Controller.children_classes.get('', []))

        # Controllers defined inside of sigil addons can be extended in
...
class GeoIP(collections.abc.Mapping):
    ...
    .. note:
        ...
        :attr:`~sigil.http.request.geoip`.

    .. code-block:

        >>> GeoIP('127.0.0.1').country.iso_code
        >>> sigil_ip = socket.gethostbyname('iantirta.com')
        >>> GeoIP(sigil_ip).country.iso_code
        'FR'
    """
...
class _Response(werkzeug.wrappers.Response):
    ...
    def load(cls, result, fname="<function>"):
        ...
        :returns: The created :class:`~sigil.http.Response`.
        :rtype: Response
...
class Response(Proxy):
    ...
    # sigil.http._response attributes
    load = ProxyFunc()
...
class Request:
    ...
    def _get_session_and_dbname(self):
        ...
        header_dbname = self.httprequest.headers.get('X-Sigil-Database')
    ...
    def update_env(self, user=None, context=None, su=None):
        ...
        :type user: int or :class:`res.users record<~sigil.addons.base.models.res_users.ResUsers>`
    ...
    def make_response(self, data, headers=None, cookies=None, status=200):
        ...
        :rtype: :class:`~sigil.http.Response`
        """
    ...
    def _serve_db(self):
        ...
        try:
            ...
            self.env = sigil.api.Environment(cr, self.session.uid, self.session.context)
            try:
...
class HttpDispatcher(Dispatcher):
    ...
    def dispatch(self, endpoint, args):
        ...
        See :meth:`~sigil.http.Response.load` method for the compatible
...
class JsonRPCDispatcher(Dispatcher):
    ...
    def handle_error(self, exc: Exception) -> collections.abc.Callable:
        ...
        if isinstance(exc, NotFound):
            ...
            error['code'] = 100
            error['message'] = "Sigil Session Expired"
...
class Application:
    ...
    def initialize(self):
        ...
        from sigil.service.server import load_server_wide_modules  # noqa: PLC0415
    ...
    def session_store(self):
        path = sigil.tools.config.session_dir
    ...
    def __call__(self, environ, start_response):
        ...
        if sigil.tools.config['proxy_mode'] and environ.get("HTTP_X_FORWARDED_HOST"):
            ...
            try:
                ...
                if self.get_static_file(httprequest.path):
                    ...
                    try:
                        ...
                        if (httprequest.path.startswith('/sigil/')
                            or httprequest.path in (
                                '/sigil', '/web', '/web/login', '/test_http/ensure_db',
                            )):
