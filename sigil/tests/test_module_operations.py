...
from sigil.tools import config, topological_sort, unique, profiler
from sigil.modules.registry import Registry
from sigil.netsvc import init_logger
from sigil.tests import standalone_tests
import sigil.tests.loader

_logger = logging.getLogger('sigil.tests.test_module_operations')
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall"
                    " cycle of Sigil modules. Prefer the 'cycle' subcommand to"
    ...
    parser.add_argument("--addons-path", "-p", type=addons_path,
        help="Comma-separated list of paths to directories containing extra Sigil modules")
...
def test_standalone(args):
    ...
    sigil.service.db._check_faketime_mode(args.database)  # noqa: SLF001
    ...
    for module_name in registry._init_modules:
        ...
        sigil.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    if args.addons_path:
        ...
        sigil.modules.module.initialize_sys_path()
    ...
    logging.config.dictConfig({
        ...
        'loggers': {
            'sigil.modules.loading': {'level': 'CRITICAL'},
            'sigil.sql_db': {'level': 'CRITICAL'},
            'sigil.models.unlink': {'level': 'WARNING'},
            'sigil.addons.base.models.ir_model': {'level': "WARNING"},
        }
    })
