# -*- coding: utf-8 -*-
"""
The module :mod:`sigil.tests.common` provides unittest test cases and a few
...
import sigil.addons.base
import sigil.cli
import sigil.http
import sigil.models
import sigil.orm.registry
from sigil import api
from sigil.exceptions import AccessError
from sigil.fields import Command
from sigil.modules.registry import Registry, DummyRLock
from sigil.service import security
...
if sigil.cli.COMMAND in ('server', 'start') and not config['test_enable']:
    _logger.error(
...
def __getattr__(name):
    ...
    warnings.warn(
        "Since 18.0: sigil.tests.common.Form is deprecated, use sigil.tests.Form",
...
# The sigil library is supposed already configured.
HOST = '127.0.0.1'
...
def get_db_name():
    dbnames = sigil.tools.config['db_name']
...
def release_test_lock():
    ...
    try:
        ...
        if not _registry_test_lock.acquire(timeout=60):
            tag = sigil.modules.module.current_test.canonical_tag
...
def standalone(*tags):
    ...
    ``tags`` and the corresponding Sigil module name.
    """
    def register(func):
        # register func by sigil module name
        if func.__module__.startswith('sigil.addons.'):
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most sigil operations;
    """
    if not login:
...
class BaseCase(case.TestCase):
    """ Subclass of TestCase for Sigil-specific code. This class is abstract and
    ...
    def __init_subclass__(cls):
        ...
        if cls.__module__.startswith('sigil.addons.'):
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~sigil.models.BaseModel`
        """
    ...
    def debug_mode(self):
        ...
        request = Mock(
            ...
            env=self.env,
            session=DotDict(sigil.http.get_default_session(), debug='1'),
        )
        try:
            self.env.flush_all()
            self.env.invalidate_all()
            sigil.http._request_stack.push(request)
            yield
            self.env.flush_all()
            self.env.invalidate_all()
        finally:
            popped_request = sigil.http._request_stack.pop()
    ...
    def _patchExecute(self, actual_queries, flush=True):
        ...
        with (
            patch('sigil.sql_db.Cursor.execute', execute),
        ...
        if self.warm:
            ...
            with patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/sigil/addons/" in filename:
                        filename = filename.rsplit("/sigil/addons/", 1)[1]
                    if count > expected:
    ...
    def assertRecordValues(
            self,
            records: sigil.models.BaseModel,
        ...
        for record in records:
            ...
            for field_name in field_names:
                ...
                match record._fields[field_name]:
                    case sigil.fields.Many2one():
                        record_value = record_value.id
                    case sigil.fields.One2many() | sigil.fields.Many2many():
                        record_value = sorted(record_value.ids)
                    case sigil.fields.Float() as field if digits := field.get_digits(record.env):
                        record_value = Approx(record_value, digits[1], decorate=False)
                    case sigil.fields.Monetary() as field if currency_field_name := field.get_currency_field(record):
    ...
    def get_method_additional_tags(self, test_method):
        ...
        """
        additional_tags = []
        if sigil.tools.config['test_tags'] and 'is_query_count' in sigil.tools.config['test_tags']:
...
class Approx:  # noqa: PLW1641
    ...
    """
    def __init__(self, value: float, rounding: int | float | sigil.addons.base.models.res_currency.ResCurrency, /, decorate: bool) -> None:  # noqa: PYI041
        self.value = value
...
class TransactionCase(BaseCase):
    ...
    """
    muted_registry_logger = mute_logger(sigil.orm.registry._logger.name)
    freeze_time = None
    ...
    def setUpClass(cls):
        ...
        cls._crypt_context_patcher = patch('sigil.addons.base.models.res_users.ResUsersPatchedInTest._crypt_context', _crypt_context)
...
class ChromeBrowser:
    ...
    def __init__(self, test_case: HttpCase, success_signal: str = DEFAULT_SUCCESS_SIGNAL, headless: bool = True, debug: bool = False):
        ...
        if scs := sigil.tools.config['screencasts']:
...
which leads to stray network requests and inconsistencies."""
    ...
    def _from_remoteobject(self, arg):
        ...
        # of the way Sigil widgets are created they all appear as Class(...)
...
class HttpCase(TransactionCase):
    ...
    touch_enabled = False
    session: sigil.http.Session = None
    ...
    @classmethod
    def http_port(cls):
        if sigil.service.server.server is None:
            return None
        return sigil.service.server.server.httpd.server_port
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        def get_http_request_threads():
            return [t for t in threading.enumerate() if t.name.startswith('sigil.service.http.request.')]
        ...
        if request_threads:
            ...
            sigil.tools.misc.dumpstacks()

    def logout(self, keep_db=True):
        self.session.logout(keep_db=keep_db)
        sigil.http.root.session_store.save(self.session)

    def authenticate(self, user, password, *,
        browser: ChromeBrowser = None, session_extra: dict | None = None):
        if getattr(self, 'session', None):
            sigil.http.root.session_store.delete(self.session)

        self.session = session = sigil.http.root.session_store.new()
        session.update(
            sigil.http.get_default_session(),
            db=get_db_name(),
            # In order to avoid perform a query to each first `url_open`
            # in a test (insert `res.device.log`).
            _trace_disable=True,
        )
        session.context['lang'] = sigil.http.DEFAULT_LANG
        ...
        if user: # if authenticated
            ...
            with patch('sigil.addons.base.models.res_users.ResUsersPatchedInTest._check_credentials', new=patched_check_credentials):
        ...
        sigil.http.root.session_store.save(session)
    ...
    def browser_js(self, url_path, code, ready='', login=None, timeout=60, cookies=None, error_checker=None, watch=False, success_signal=DEFAULT_SUCCESS_SIGNAL, debug=False, cpu_throttling=None, **kw):
        ...
        with self.allow_requests(browser=browser), contextlib.ExitStack() as atexit:
            ...
            if "bus.bus" in self.env.registry:
                from sigil.addons.bus.websocket import CloseCode, _kick_all, WebsocketConnectionHandler  # noqa: PLC0415
                from sigil.addons.bus.models.bus import BusBus  # noqa: PLC0415
            ...
            cpu_throttling_os = os.environ.get('SIGIL_BROWSER_CPU_THROTTLING')  # used by dedicated runbot builds
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        options = {
            ...
            'startUrl': url_path,
            'delayToCheckUndeterminisms': kwargs.pop('delay_to_check_undeterminisms', int(os.getenv("SIGIL_TOUR_DELAY_TO_CHECK_UNDETERMINISMS", "0")) or 0),
        }
        code = kwargs.pop('code', f"sigil.startTour({tour_name!r}, {json.dumps(options)})")
        ready = kwargs.pop('ready', f"sigil.isTourReady({tour_name!r})")
    ...
    def profile(self, **kwargs):
        ...
        return profiler.Nested(_profiler, patch('sigil.http.Request._get_profiler_context_manager', route_profiler))
    ...
    def get_method_additional_tags(self, test_method):
        ...
        if sigil.tools.config['test_tags'] and 'is_tour' in sigil.tools.config['test_tags']:
...
def tagged(*tags):
    ...
    By default, all Test classes from sigil.tests.common have a test_tags
...
class freeze_time:
    """ Object to replace the freezegun in Sigil test suites
