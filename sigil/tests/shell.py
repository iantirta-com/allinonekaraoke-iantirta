...
TEST_MODULE_NAME_PATTERN = re.compile(r'^sigil\.addons\.\w+\.tests')


def run_tests(env, test_tags, modules=None, reload_tests=False):
    """Run tests for the given modules and test tags."""

    if sigil.cli.COMMAND != 'shell':
        _logger.error('run_tests should be used only in sigil shell')
        return

    if sigil.tools.config['workers'] != 0:
        _logger.error('run_tests should be used only in threaded mode')
        return

    from sigil.service.server import server  # noqa: PLC0415
    if not server.httpd:
    ...
    sigil.tools.config['test_tags'] = test_tags
    sigil.tools.config['test_enable'] = True
    report = _run_tests(env.cr.dbname, modules)
    sigil.tools.config['test_enable'] = None
    sigil.tools.config['test_tags'] = None
