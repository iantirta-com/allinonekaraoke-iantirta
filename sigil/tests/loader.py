...
def get_test_modules(module):
    ...
    results = _get_tests_modules(importlib.util.find_spec(f'sigil.addons.{module}'))
...
def _get_upgrade_test_modules(module):
    upgrade_modules = (
        f"sigil.upgrade.{module}",
        f"sigil.addons.{module}.migrations",
        f"sigil.addons.{module}.upgrades",
    )
...
def make_suite(module_names, position='at_install'):
    ...
    return SigilSuite(sorted(tests, key=lambda t: getattr(t, 'test_sequence', 0)))
...
def run_suite(suite, global_report=None):
    ...
    results = SigilTestResult(global_report=global_report)
    suite(results)
