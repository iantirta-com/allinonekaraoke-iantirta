...
Sigil tests customisation combined with the need of a cross version compatibility
started to make TestSuite and other unitest object more complicated than vendoring
the part we need for Sigil. This versions is simplified in order
...
class TestSuite(BaseTestSuite):
    ...
    def run(self, result, debug=False):
        default_tests_run_count = int(os.environ.get('SIGIL_TEST_FAILURE_RETRIES', '0')) + 1
        for test in self:
            if result.shouldStop:
                break
            assert isinstance(test, (TestCase))
            sigil.modules.module.current_test = test
                ...
                for retry in range(tests_run_count):
                    ...
                    if retry < tests_run_count - 1:
                        with warnings.catch_warnings(), \
                                result.soft_fail(), \
                                sigil.tools.misc.lower_logging(25, logging.INFO) as quiet_log:
                            test(result)
