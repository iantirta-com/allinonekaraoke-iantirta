...
import sigil.tests
from sigil.tools.translate import TranslationImporter


@sigil.tests.tagged('post_install', '-at_install')
class TestRelatedTranslation(sigil.tests.TransactionCase):
    ...
    def test_edit_translations_related_html(self):
        ...
        expected = ''.join(
            '<p>'
                '<span '
                    'data-oe-model="test_orm.related_translation_1" '
