...
@sigil.tests.common.tagged('post_install', '-at_install')
class TestUi(HttpCaseWithUserDemo):
    ...
    def test_01_admin_widget_x2many(self):
        ...
        self.start_tour("/sigil/action-test_orm.action_discussions",
            'widget_x2many', login="admin", timeout=120)


@sigil.tests.tagged('-at_install', 'post_install')
class TestUiTranslation(sigil.tests.HttpCase):

    @mute_logger('sigil.sql_db', 'sigil.http')
    def test_01_sql_constraints(self):
        ...
        self.start_tour("/sigil/action-test_orm.action_categories",
