...
from sigil.tools import mute_logger, SQL
from sigil.tools.sql import table_columns

_logger = logging.getLogger('sigil.addons.base.partner.merge')
    ...
    def _update_foreign_keys_generic(self, model, src_records, dst_record):
        ...
        for table, column in relations:
            ...
            if len(columns) <= 1:
                ...
                try:
                    with mute_logger('sigil.sql_db'), self.env.cr.savepoint():
                        query = 'UPDATE "%(table)s" SET "%(column)s" = %%s WHERE "%(column)s" IN %%s' % query_dic
                        self.env.cr.execute(query, (dst_record.id, tuple(src_records.ids)))
