...
from sigil import api, fields, models, tools, _
from sigil.api import SUPERUSER_ID
from sigil.exceptions import AccessDenied, AccessError, UserError, ValidationError
from sigil.fields import Command, Domain
from sigil.http import request, DEFAULT_LANG
from sigil.tools import email_domain_extract, is_html_empty, frozendict, reset_cached_properties, str2bool, SQL
...
class ResUsers(models.Model):
    ...
    def _check_credentials(self, credential, env):
        ...
        * catch :class:`~sigil.exceptions.AccessDenied` and perform their
          own checking
        * (re)raise :class:`~sigil.exceptions.AccessDenied` if the
    ...
    def _unlink_except_master_data(self):
        ...
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by Sigil (updates, module installation, ...)'))
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: sigil.exceptions.AccessDenied when old password is wrong
        :raise: sigil.exceptions.UserError when new password is not set or empty
        """
        if not old_passwd:
            raise AccessDenied()
    ...
    def _deactivate_portal_user(self, **post):
        ...
        it removed because they don't use this Sigil portal anymore.
        ...
        if self._on_login_cooldown(failures, previous):
            ...
            if ipaddress.ip_address(source).is_private:
                _logger.warning(
                    ...
                    "and *might* be a proxy. If your Sigil is behind a proxy, "
                    "it may be mis-configured. Check that you are running "
                    "Sigil in Proxy Mode and that the proxy is properly configured, see "
                    "https://www.iantirta.com/documentation/latest/administration/install/deploy.html#https for details.",
                    source
                )
...
def _check_apikey_credentials(cr, *, scope, key, table='res_users_apikeys'):
    ...
    :param sigil.sql_db.BaseCursor cr: database cursor
