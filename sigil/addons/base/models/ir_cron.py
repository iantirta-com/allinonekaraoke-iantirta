...
from sigil import api, fields, models, sql_db
from sigil.exceptions import LockError, UserError
from sigil.http import serialize_exception
from sigil.modules import Manifest
from sigil.modules.registry import Registry
from sigil.tools import SQL
from sigil.tools.constants import GC_UNLINK_LIMIT
...
SIGIL_NOTIFY_FUNCTION = os.getenv('SIGIL_NOTIFY_FUNCTION', 'pg_notify')
...
class IrCron(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            vals['usage'] = 'ir_cron'
        if os.getenv('SIGIL_NOTIFY_CRON_CHANGES'):
    ...
    def _check_modules_state(cr, jobs):
        ...
        # reset_module_states.
        from sigil.modules.loading import reset_modules_state  # noqa: PLC0415
    ...
    def write(self, vals):
        ...
        if ('nextcall' in vals or vals.get('active')) and os.getenv('SIGIL_NOTIFY_CRON_CHANGES'):
            self.env.cr.postcommit.add(self._notifydb)
    ...
    def _trigger_list(self, at_list: list[datetime]):
        ...
        if min(at_list) <= now or os.getenv('SIGIL_NOTIFY_CRON_CHANGES'):
            self.env.cr.postcommit.add(self._notifydb)
        return triggers
    ...
    def _notifydb(self):
        ...
        The SIGIL_NOTIFY_CRON_CHANGES environment variable allows to force the notifydb on both
        IrCron modification and on trigger creation (regardless of call_at)
        """
        with sql_db.db_connect('postgres').cursor() as cr:
            cr.execute(SQL("SELECT %s('cron_trigger', %s)", SQL.identifier(SIGIL_NOTIFY_FUNCTION), self.env.cr.dbname))
