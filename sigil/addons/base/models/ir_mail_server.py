...
from sigil import _, api, fields, models, modules, tools
...
_test_logger = logging.getLogger('sigil.tests')
...
class IrMail_Server(models.Model):
    ...
    def _connect__(self, host=None, port=None, user=None, password=None, encryption=None,  # noqa: PLW3201
        ...
        """Returns a new SMTP connection to the given SMTP server.
           ...
           :param ssl_certificate: filename of the SSL certificate used for authentication
               Used when no mail server is given and overwrite  the sigil-bin argument "smtp_ssl_certificate"
           :param ssl_private_key: filename of the SSL private key used for authentication
               Used when no mail server is given and overwrite  the sigil-bin argument "smtp_ssl_private_key"
        ...
        # Store the "from_filter" of the mail server / sigil-bin argument to  know if we
    ...
    def _alter_message__(self, message, smtp_from, smtp_to_list):  # noqa: PLW3201
        ...
        del message['Bcc']                   # see sigil/sigil@2445f9e3c22db810d61996afde883e4ca608f15b
    ...
    def _find_mail_server(self, email_from, mail_servers=None):
        ...
        :returns: A two-elements tuple: ``(Record<ir.mail_server>, email_from)``
          ...
          1. Mail server to use to send the email (``None`` if we use the sigil-bin arguments)
        ...
        # Skip if passed email_from is False (example Sigilbot has no email address)
        ...
        # 5: SMTP config in sigil-bin arguments
        from_filter = self.env['ir.mail_server']._get_default_from_filter()
