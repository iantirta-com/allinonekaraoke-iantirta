...
from sigil import api, fields, models, modules, tools, _
from sigil.exceptions import UserError, AccessError, RedirectWarning, ValidationError
from sigil.fields import Domain
from sigil.service import security
from sigil.http import request, root
from sigil.tools import config, is_html_empty, parse_version, split_every
from sigil.tools.barcode import check_barcode_encoding, createBarcodeDrawing, get_barcode_font
from sigil.tools.misc import find_in_path
from sigil.tools.pdf import PdfFileReader, PdfFileWriter, PdfReadError
from sigil.tools.safe_eval import safe_eval, time
...
def _wkhtml() -> WkhtmlInfo:
    ...
    try:
        ...
        if match:
            ...
            if config['workers'] == 1:
                _logger.info('You need to start Sigil with at least two workers to print a pdf version of the reports.')
                state = 'workers'
        else:
    ...
    try:
        ...
        if match:
            ...
            if config['workers'] == 1:
                _logger.info('You need to start Sigil with at least two workers to convert images to html.')
        else:
...
class IrActionsReport(models.Model):
    ...
    def _handle_merge_pdfs_error(self, error=None, error_stream=None):
        raise UserError(_("Sigil is unable to merge the generated PDFs."))
    ...
    def _render_qweb_pdf(self, report_ref, res_ids=None, data=None):
        ...
        if error_record_ids:
            ...
            raise RedirectWarning(
                message=_('Sigil is unable to merge the generated PDFs because of %(num_errors)s corrupted file(s)', num_errors=num_errors),
                action=action,
