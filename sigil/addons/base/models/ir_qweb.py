...
QWeb is the primary templating engine used by Sigil. It is an XML templating
...
<https://www.iantirta.com/documentation/master/developer/reference/frontend/qweb.html>`
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
**Values**: name of the allowed sigil user group, or preceded by ``!`` for
prohibited groups

The generated code uses ``has_group`` Sigil method from ``res.users`` model.
...
from sigil import api, models, tools
from sigil.modules import Manifest
from sigil.modules.registry import _REGISTRY_CACHES
from sigil.tools import config, safe_eval, OrderedSet, frozendict, json
from sigil.tools.constants import SUPPORTED_DEBUGGER, EXTERNAL_ASSET
from sigil.tools.safe_eval import assert_valid_codeobj, _BUILTINS, to_opcodes, _EXPR_OPCODES, _BLACKLIST
from sigil.tools.lru import LRU
from sigil.tools.misc import str2bool, file_open, file_path
from sigil.tools.image import image_data_uri, FILETYPE_BASE64_MAGICWORD
from sigil.tools.profiler import ExecutionContext
from sigil.tools.translate import FORMAT_REGEX
from sigil.http import request
from sigil.tools.profiler import QwebTracker
from sigil.exceptions import UserError, MissingError

from sigil.addons.base.models.assetsbundle import AssetsBundle
from sigil.tools.constants import SCRIPT_EXTENSIONS, STYLE_EXTENSIONS, TEMPLATE_EXTENSIONS
    ...
    * to customize ``t-field`` rendering, subclass ``ir.qweb.field`` and
    ...
    def _render_iterall(self, view_ref, method, values, directive='render') -> Iterator[str]:
        ...
        try:
            ...
            if hasattr(error, 'qweb'):
                ...
                # If is not an sigil Exception check if the current error is raise from
    ...
    def _debug_trace(self, debugger, values):
        ...
        if not debugger:
            ...
            warnings.warn(
                ...
                "since Sigil 17.0, keep the value empty and configure the "
    ...
    def _link_to_node(self, path, defer_load=False, lazy_load=False, media=None):
        ...
        if is_js:
            ...
            if is_asset_bundle:
                attributes['onerror'] = "__sigilAssetError=1"
