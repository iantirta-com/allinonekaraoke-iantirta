...
from sigil.tools import OrderedSet, misc, profiler
from sigil.tools.constants import SCRIPT_EXTENSIONS, STYLE_EXTENSIONS
from sigil.tools.json import scriptsafe as json
from sigil.tools.misc import file_open, file_path
...
class AssetsBundle(object):
            ...
            if self.templates:
                ...
                template_bundle = textwrap.dedent(f"""
                    ...
                    *******************************************/
                    ...
                    sigil.define("{self.name}.bundle.xml", ["@web/core/templates"], function(require) {{
                        "use strict";
    ...
    def js_with_sourcemap(self, template_bundle=None):
        ...
        """
        from sigil.tools.sourcemap_generator import SourceMapGenerator  # noqa: PLC0415
        ...
        * 't-inherit-mode':  'primary' or 'extension'.
        ...
        for asset in self.templates:
            ...
            try:
                ...
                template = content if content.startswith('<sigil>') else f'<templates>{asset.content}</templates>'
    ...
    def compile_css(self, compiler, source):
        ...
        def sanitize(matchobj):
            ...
            if '.' not in ref and line not in imports and not ref.startswith(('.', '/', '~')):
                imports.append(line)
                return line
            msg = "Local import '%s' is forbidden for security reasons. Please remove all @import {your_file} imports in your custom files. In Sigil you have to import all files in the assets, and not through the @import statement." % ref
            _logger.warning(msg)
...
class JavascriptAsset(WebAsset):
    ...
    def is_transpiled(self):
        if self._is_transpiled is None:
            from sigil.tools.js_transpiler import is_sigil_module  # noqa: PLC0415
            self._is_transpiled = bool(is_sigil_module(self.url, super().content))
    ...
    def content(self):
        ...
        if self.is_transpiled:
            if not self._converted_content:
                from sigil.tools.js_transpiler import transpile_javascript  # noqa: PLC0415
