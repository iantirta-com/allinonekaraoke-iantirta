...
from sigil import fields
from sigil.tests.common import RecordCapturer, TransactionCase
from sigil.tools import mute_logger

from sigil.addons.base.models.ir_cron import (
...
class TestIrCron(TransactionCase, CronMixinCase):
    ...
    def test_cron_direct_trigger_exception(self):
        ...
        with (
            ...
            self.assertLogs('sigil.addons.base.models.ir_cron', 40),  # logging.ERROR
        ...
        for cb, curr_failures, trigger, call_count, done_count, fail_count, active in CASES:
            with self.subTest(cb=cb, failure=curr_failures), closing(self.cr.savepoint()):
                ...
                with self.enter_registry_test_mode():
                    ...
                    with mute_logger('sigil.addons.base.models.ir_cron'),\
    ...
    def test_cron_failed_increase(self):
        ...
        with self.enter_registry_test_mode():
            with (
                ...
                mute_logger('sigil.addons.base.models.ir_cron'),
                self.registry.cursor() as cr,
            ):
                self.registry['ir.cron']._process_job(
                    cr,
    ...
    def test_cron_timeout_failure(self):
        ...
        self.env.flush_all()
        with self.enter_registry_test_mode(), mute_logger('sigil.addons.base.models.ir_cron'), self.registry.cursor() as cr:
            self.registry['ir.cron']._process_job(
                cr,
