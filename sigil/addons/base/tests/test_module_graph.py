...
from sigil.modules.module_graph import ModuleGraph
from sigil.modules.module import _DEFAULT_MANIFEST, Manifest
from sigil.tools import mute_logger


class TestGraph(BaseCase):
    @mute_logger('sigil.modules.module_graph')
    def _test_graph_order(
        ...
        with (
            patch('sigil.modules.module_graph.ModuleGraph._update_from_database'),
            patch('sigil.modules.module_graph.Manifest.for_addon', make_manifest),
            patch('sigil.modules.module_graph.ModuleGraph._imported_modules', {'studio_customization'}),
        ):
            dummy_cr = None
