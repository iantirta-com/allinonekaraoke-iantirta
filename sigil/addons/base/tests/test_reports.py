...
@sigil.tests.tagged('post_install', '-at_install', 'post_install_l10n')
class TestReports(sigil.tests.TransactionCase):
    def test_reports(self):
        ...
        specific_model_domains = {
            ...
            'account_edi_ubl_cii.account_invoices_generated_by_sigil': [('move_type', 'in', ('in_invoice', 'in_refund'))],
    ...
    def test_merge_pdfs(self):
        ...
        with self.assertRaises(UserError, msg="Sigil is unable to merge the generated PDFs."):
        ...
        minimal_file = tools.pdf.SigilPdfFileReader(minimal_pdf_content)
        merged_file = tools.pdf.SigilPdfFileReader(merged_pdf)
...
class TestReportsRenderingCommon(sigil.tests.HttpCase):
    ...
    def save_pdf(self):
        ...
        sigil.tests.save_test_file(self._testMethodName, self.last_pdf_content, 'pdf_', 'pdf', document_type='Report PDF', logger=_logger)
...
@sigil.tests.tagged('post_install', '-at_install', 'pdf_rendering')
...
@sigil.tests.tagged('post_install', '-at_install', '-standard', 'pdf_rendering')
...
class TestAggregatePdfReports(sigil.tests.HttpCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
