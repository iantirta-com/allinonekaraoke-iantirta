...
import sigil.addons
from sigil.modules.module import Manifest
from sigil.release import major_version
...
class TestModuleManifest(BaseCase):
    @classmethod
    def setUpClass(cls):
        cls._tmp_dir = tempfile.TemporaryDirectory(prefix='sigil_test_addons_')
        cls.addClassCleanup(cls._tmp_dir.cleanup)
        cls.addons_path = cls._tmp_dir.name

        patcher = patch.object(sigil.addons, '__path__', [cls.addons_path])
        cls.startClassPatcher(patcher)

    def setUp(self):
        self.module_root = tempfile.mkdtemp(prefix='sigil_test_module_', dir=self.addons_path)
    ...
    def test_default_manifest(self):
        ...
        with self.assertNoLogs('sigil.modules.module', 'WARNING'):
    ...
    def test_missing_manifest(self):
        with self.assertLogs('sigil.modules.module', 'DEBUG') as capture:
    ...
    def test_missing_license(self):
        ...
        with self.assertLogs('sigil.modules.module', 'WARNING') as capture:
            manifest = Manifest.for_addon(self.module_name)
