...
import sigil
from sigil.tests import TransactionCase
from sigil.tools import file_open, file_open_temporary_directory, file_path
from sigil.tools.config import configmanager

EMPTY_CONFIG_PATH = file_path('base/tests/config/empty.conf')
PROJECT_PATH = sigil.tools.config.root_path.removesuffix('/sigil')
DEFAULT_DATADIR = sigil.tools.config._default_options['data_dir']

MISSING_HTTP_INTERFACE = """\
WARNING:sigil.tools.config:missing --http-interface/http_interface, \
...
class TestConfigManager(TransactionCase):
    ...
    def setUp(self):
        super().setUp()
        patcher = patch.dict('os.environ', {'SIGIL_RC': EMPTY_CONFIG_PATH}, clear=True)
        patcher.start()
    ...
    def test_01_default_config(self):
        self.assertConfigEqual({
            ...
            'proxy_access_token': '',
            'publisher_warranty_url': 'http://services.iantirta.com/publisher-warranty/',
            'reportgz': False,
    ...
    def test_02_config_file(self):
        ...
        with self.assertLogs('sigil.tools.config', 'WARNING') as capture:
        ...
        self.assertEqual(capture.output, [
            "WARNING:sigil.tools.config:option addons_path, no such directory '/tmp/sigil', skipped",
            "WARNING:sigil.tools.config:option upgrade_path, no such directory '/tmp/upgrade', skipped",
            "WARNING:sigil.tools.config:option pre_upgrade_scripts, no such file '/tmp/pre-custom.py', skipped",
        ])
    ...
    def test_04_sigil16_config_file(self):
        # test that loading the Sigil 16.0 generated default config works
        ...
        with (
            ...
            self.assertLogs('sigil.tools.config', 'WARNING') as capture_warn,
        ):
        ...
        def missing(*options):
            return [
                f"WARNING:sigil.tools.config:unknown option '{option}' in "
        ...
        def falsy(*options):
            return [
                f"WARNING:sigil.tools.config:option {option} reads 'False' "
    ...
    def test_06_cli(self):
        ...
        self.assertEqual(capture.output, [
            "WARNING:sigil.tools.config:test file '/tmp/file-file' cannot be found",
        ])
        ...
        for args, stop_after_init in [
            ...
            with self.subTest(args=args):
                if any('--test' in arg for arg in args):
                    ...
                    self.assertEqual(capture.output, [
                        "WARNING:sigil.tools.config:Empty -d/--database/db_name, tests won't run",
                    ])
                else:
                    _, options = self.parse_reset(args)
    ...
    def test_14_addons_path_glob(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with self.assertLogs('sigil.tools.config') as capture:
                _, options = self.parse_reset(['--addons-path', not_addons])
            self.assertEqual(options['addons_path'], [])
            self.assertEqual(capture.output, [
                f"WARNING:sigil.tools.config:option --addons-path, invalid addons directory {not_addons!r}, skipped"
            ])
