...
from sigil.tools.translate import _push, quote, unquote, xml_translate, html_translate, TranslationImporter, TranslationModuleReader, TranslationReader
from sigil.tests.common import TransactionCase, BaseCase, new_test_user, tagged

_stats_logger = logging.getLogger('sigil.tests.stats')
    ...
    def test_translate_xml_a(self):
        ...
        source = """<t t-name="stuff">
                        <ul class="nav navbar-nav">
                            <li class="nav-item">
                                <a class="nav-link oe_menu_leaf" href="/sigil/action-54?menu_id=42">
...
class TestLanguageInstall(TransactionCase):
    def test_language_install(self):
        ...
        with patch('sigil.addons.base.models.ir_module.IrModuleModule._load_module_terms', _load_module_terms):
            wizard.lang_install()
...
class TestTranslationExport(TransactionCase):
    ...
    def test_export_translatable_resources(self):
        ...
        with self.assertNoLogs('sigil.tools.translate', "ERROR"):
...
class TestTranslation(TransactionCase):
    ...
    #     country_1 = Country.create({'name': 'Sigil'})
    #     country_1.with_context(lang='fr_FR').name = 'Sigil_Fr'
    #     country_1.flush_recordset()
    #
    #     country_2 = Country.create({'name': 'Sigil2'})
    #     with self.assertRaises(IntegrityError), mute_logger('sigil.sql_db'):
    #         country_2.name = 'Sigil'
    #         country_2.flush_recordset()
    #
    #     with self.assertRaises(IntegrityError), mute_logger('sigil.sql_db'):
    #         country_3 = Country.create({'name': 'Sigil'})
