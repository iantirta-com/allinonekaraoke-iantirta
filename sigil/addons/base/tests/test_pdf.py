...
from sigil.tools.pdf import reshape_text
import io
...
class TestPdf(TransactionCase):
    ...
    def setUp(self):
        ...
        self.minimal_pdf_reader = pdf.SigilPdfFileReader(self.minimal_reader_buffer)

    def test_sigil_pdf_file_reader(self):
        ...
        r = pdf.SigilPdfFileReader(io.BytesIO(out.getvalue()))
        self.assertEqual(len(list(r.getAttachments())), 1)

    def test_sigil_pdf_file_writer(self):
        attachments = list(self.minimal_pdf_reader.getAttachments())
        self.assertEqual(len(attachments), 0)
        ...
        for count, (name, data) in enumerate([
            ...
            pdf_writer = pdf.SigilPdfFileWriter()
    ...
    def test_sigil_pdf_file_reader_with_owner_encryption(self):
        pdf_writer = pdf.SigilPdfFileWriter()
        pdf_writer.cloneReaderDocumentRoot(self.minimal_pdf_reader)
    ...
    def test_merge_pdf(self):
        ...
        merged_pdf_reader = pdf.SigilPdfFileReader(merged_reader_buffer)
    ...
    def test_branded_file_writer(self):
        ...
        self.assertEqual(pdf_info['/Producer'], 'Sigil')
        self.assertEqual(pdf_info['/Creator'], 'Sigil')
        reader_buffer.close()
