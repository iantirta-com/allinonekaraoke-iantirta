...
from sigil.http import route, Controller, request
from sigil.tests.common import HttpCase, tagged, ChromeBrowser, TEST_CURSOR_COOKIE_NAME, Like
    ...
    def test_console_error_string(self):
        with self.assertLogs(level='ERROR') as log_catcher:
            with self.assertRaises(AssertionError) as error_catcher:
                ...
                with patch('sigil.tests.common.ChromeBrowser.take_screenshot', return_value=None):
...
class TestRequestRemainingNoCookie(TestRequestRemainingCommon):
    ...
    def test_requests_b(self):
        with self.assertLogs('sigil.tests.common') as log_catcher:
            self._test_requests_b()
        self.assertEqual(
            log_catcher.output,
            [Like('... sigil.tests.common:Request with path /web/concurrent has been ignored during test as it it does not contain the test_cursor cookie or it is expired. '
...
class TestRequestRemainingStartDuringNext(TestRequestRemainingCommon):
    ...
    def test_requests_b(self):
        with self.assertLogs('sigil.tests.common') as log_catcher, self.allow_requests():
            self._test_requests_b()
        self.assertEqual(
            log_catcher.output,
            [Like('... sigil.tests.common:Request with path /web/concurrent has been ignored during test as it it does not contain the test_cursor cookie or it is expired. '
