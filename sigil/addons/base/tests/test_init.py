...
class TestInit(BaseCase):
    ...
    def sigil_modules_to_test(self):
        import sigil.cli  # noqa: PLC0415
        for path in (*sigil.__path__, *sigil.cli.__path__):
            parent = Path(path)
            for module in parent.iterdir():
                if (module.is_dir() or module.suffix == '.py') and '__' not in module.name:
                    if parent.name == 'sigil':
                        yield f"sigil.{module.stem}"
                    else:
                        yield f"sigil.{parent.name}.{module.stem}"
    ...
    def test_import(self):
        ...
        for module in sorted(self.sigil_modules_to_test()):
