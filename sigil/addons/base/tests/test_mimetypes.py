...
from sigil.tools.mimetypes import fix_filename_extension, get_extension, guess_mimetype
...
<Document xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns="urn:iso:std:iso:20022:tech:xsd:pain.001.001.03">
  <CstmrCdtTrfInitn>
    <GrpHdr>
      <MsgId>123456iantirta.com893873733</MsgId>
...
class test_guess_mimetype(BaseCase):
    ...
    def test_default_mimetype_empty(self):
        ...
        # sigil implementation returns application/octet-stream by default
    ...
    def test_mimetype_svg(self):
        ...
        self.assertTrue(mimetype.startswith('image/svg'))
        # Tests that whitespace padded SVG are not detected as SVG in sigil implementation
        if not magic:
    ...
    def test_mimetype_fix_extension(self):
        ...
        with self.assertLogs('sigil.tools.mimetypes', 'WARNING') as capture:
            self.assertEqual(fix('image.txt', 'image/jpeg'), 'image.txt.jpg')
            self.assertEqual(fix('words.jpg', 'text/plain'), 'words.jpg.txt')
        self.assertEqual(capture.output, [
            "WARNING:sigil.tools.mimetypes:File 'image.txt' has an invalid "
                "extension for mimetype 'image/jpeg', adding '.jpg'",
            "WARNING:sigil.tools.mimetypes:File 'words.jpg' has an invalid "
