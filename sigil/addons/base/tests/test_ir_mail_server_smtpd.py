...
from sigil.tools import config, file_path, mute_logger
...
@patch('sigil.addons.base.models.ir_mail_server.SMTP_TIMEOUT', SMTP_TIMEOUT)
...
class TestIrMailServerSMTPD(TransactionCaseWithUserDemo):
    ...
    def setUpClass(cls):
        ...
        # decrease aiosmtpd verbosity, sigil INFO = aiosmtpd WARNING
        ...
        # force aiosmtpd/sigil to bind/connect to a fixed ipv4 OR ipv6
        # address.
                    ...
                    with self.subTest(encryption=encryption, certificate=name):
                        ...
                        if error_pattern:
                            ...
                            with self.assertRaises(UserError) as error_capture, \
                                 patch('sigil.addons.base.models.ir_mail_server.SMTP_TIMEOUT', timeout):
    ...
    def test_man_in_the_middle_matrix(self):
        ...
        traffic in between the Sigil server and the legitimate SMTP
        server.
        """
        mail_server = self.env['ir.mail_server'].create({
            'name': 'test smtpd',
