...
from sigil.cli.command import commands, load_addons_commands, load_internal_commands
from sigil.tests import BaseCase
from sigil.tools import config, file_path
...
class TestCommand(BaseCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.sigil_bin = Path(__file__).parents[4].resolve() / 'sigil-bin'
        addons_path = config.format('addons_path', config['addons_path'])
        cls.run_args = (sys.executable, cls.sigil_bin, f'--addons-path={addons_path}')
    ...
    def test_docstring(self):
        ...
        for name, cmd in commands.items():
            self.assertTrue(cmd.__doc__,
                msg=f"Command {name} needs a docstring to be displayed with 'sigil-bin help'")
            self.assertFalse('\n' in cmd.__doc__ or len(cmd.__doc__) > 120,
                msg=f"Command {name}'s docstring format is invalid for 'sigil-bin help'")
        ...
        for name in ('bonbon', 'café'):
            with self.subTest(name):
                ...
                self.assertEqual(
                    ...
                    "Use 'sigil-bin --help' to see the list of available commands."
                )
    ...
    def test_upgrade_code_standalone(self):
        from sigil.cli import upgrade_code  # noqa: PLC0415
        proc = sp.run(
