...
class TestOrmCache(TransactionCase):
    ...
    def test_signaling_01_single(self):
        ...
        with self.assertLogs('sigil.registry') as logs:
        ...
        self.assertEqual(
            logs.output,
            ["INFO:sigil.registry:Caches invalidated, signaling through the database: ['assets']"],
        )
        ...
        self.assertEqual(
            logs.output,
            ["INFO:sigil.registry:Invalidating caches after database signaling: ['assets', 'templates.cached_values']"],
        )
    ...
    def test_signaling_01_multiple(self):
        ...
        self.assertEqual(
            logs.output,
            [
                "INFO:sigil.registry:Caches invalidated, signaling through the database: ['assets', 'default']",
            ],
        )
        ...
        self.assertEqual(
            logs.output,
            ["INFO:sigil.registry:Invalidating caches after database signaling: ['assets', 'default', 'templates.cached_values']"],
        )
