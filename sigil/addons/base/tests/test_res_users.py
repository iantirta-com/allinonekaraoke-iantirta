...
from sigil.http import _request_stack
from sigil.tests import Form, TransactionCase, new_test_user, tagged, HttpCase, users, warmup
...
class TestUsers(UsersCommonCase):
    ...
    @mute_logger('sigil.sql_db', 'sigil.addons.base.models.res_users_deletion')
    ...
    def test_context_get_lang(self):
        ...
        request_patch = patch('sigil.addons.base.models.res_users.request', request)
...
class TestUsers2(UsersCommonCase):
    ...
    def test_view_group_hierarchy(self):
        ...
        with patch('sigil.addons.base.models.res_groups.ResGroups._get_view_group_hierarchy') as mock:
...
class TestUsersIdentitycheck(HttpCase):
    ...
    def test_revoke_all_devices(self):
        ...
        self.env.user.password = "admin@sigil"

        # Create a first session that will be used to revoke other sessions
        session = self.authenticate('admin', 'admin@sigil', session_extra={'_trace_disable': False})

        # Create a second session that will be used to check it has been revoked
        self.authenticate('admin', 'admin@sigil', session_extra={'_trace_disable': False})
