...
from sigil.addons.base.tests import mail_examples
from sigil.addons.base.tests.common import MockSmtplibCase
from sigil.tests import tagged, users
...
class TestIrMailServer(TransactionCase, MockSmtplibCase):
    ...
    @mute_logger('sigil.models.unlink', 'sigil.addons.base.models.ir_mail_server')
    ...
    def test_mail_server_config_bin(self):
        """ Test the configuration provided in the sigil-bin arguments. This config
        ...
        sigil-bin argument "--from-filter".
        """
        IrMailServer = self.env['ir.mail_server']

        # Remove all mail server so we will use the sigil-bin arguments
        ...
        # for from_filter in ICP, overwrite the one from sigil-bin
    ...
    def test_mail_server_config_cli(self):
        ...
        "cli". It should take the configuration from the sigil-bin argument. The
        ...
        for mail_from, (expected_smtp_from, expected_msg_from, expected_mail_server) in zip(
            [
                # check that the CLI server take the configuration in the sigil-bin argument
