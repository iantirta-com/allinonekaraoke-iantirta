...
from sigil.addons.test_http.utils import (
    TEST_IP,
...
class TestDevice(TestHttpBase):
    ...
    def hit(self, time, endpoint, headers=None, ip=None):
        ...
        with freeze_time(time), \
            patch.dict(sigil.tools.config.options, {'proxy_mode': bool(ip)}):
    ...
    def _create_device_log_for_user(self, session, count):
        for _ in range(count):
            self.DeviceLog.create({
                'session_identifier': sigil.http.root.session_store.generate_key(),
                'user_id': session.uid,
                'revoked': False,
