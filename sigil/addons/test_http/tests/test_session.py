...
from sigil.tests import get_db_name, tagged
from sigil.tools import config, mute_logger, reset_cached_properties

from .test_common import TestHttpBase
from sigil.addons.base.tests.common import HttpCase, HttpCaseWithUserDemo
from sigil.addons.test_http.controllers import CT_JSON
...
class TestHttpSession(TestHttpBase):
    ...
    @mute_logger('sigil.http')  # greeting_none called ignoring args {'debug'}
    ...
    def test_session01_default_session(self):
        ...
        with patch.object(sigil.http.root.session_store, 'save') as mock_save:
    ...
    def test_session03_logout_15_0_geoip(self):
        ...
        with self.assertLogs('sigil.http', level='WARNING') as (_, warnings):
            res = self.multidb_url_open('/test_http/ensure_db', dblist=['db1', 'db2'])

        self.assertEqual(warnings, [
            "WARNING:sigil.http:Logged into database 'idontexist', but dbfilter rejects it; logging session out.",
        ])
    ...
    @patch("sigil.http.root.session_store.vacuum")
    def test_session08_gc_ignored_no_db_name(self, mock):
        with patch.dict(os.environ, {'SIGIL_SKIP_GC_SESSIONS': ''}):
            self.env['ir.http']._gc_sessions()
            mock.assert_called_once()

        with patch.dict(os.environ, {'SIGIL_SKIP_GC_SESSIONS': '1'}):
            mock.reset_mock()
            self.env['ir.http']._gc_sessions()
    ...
    def test_session09_logout(self):
        ...
        self.assertTrue(sigil.http.root.session_store.get(sid), "session should exist")
        self.url_open('/web/session/logout', allow_redirects=False).raise_for_status()
        self.assertFalse(sigil.http.root.session_store.get(sid), "session should not exist")
    ...
    def test_session10_explicit_session(self):
        ...
        with self.assertLogs('sigil.http') as capture:
        ...
        self.assertRegex(capture.output[0],
            r"^WARNING:sigil.http:<function sigil\.addons\.\w+\.controllers\.\w+\.logout> "
    ...
    def test_session12_x_sigil_database_good_no_prior_session(self):
        res = self.multidb_url_open(
            '/test_http/ensure_db',
            dblist=('a', 'b'),
            headers={'X-Sigil-Database': 'a'})
        self.assertEqual(res.status_code, 200, res.text)
        self.assertEqual(res.text, 'a')
        self.assertNotIn('session_id', res.cookies)

    def test_session13_x_sigil_database_bad_no_prior_session(self):
        res = self.multidb_url_open(
            '/test_http/ensure_db',
            dblist=('a', 'b'),
            headers={
    ...
    def test_session14_x_sigil_database_with_prior_session_same(self):
        session = self.authenticate(None, None)
        self.assertEqual(session.db, get_db_name())

        res = self.url_open('/test_http/ensure_db', headers={
            'X-Sigil-Database': get_db_name(),
        })
        res.raise_for_status()
        self.assertEqual(res.status_code, 200, res.text)
    ...
    def test_session15_x_sigil_database_with_prior_session_different(self):
        session = self.authenticate(None, None)
        self.assertEqual(session.db, get_db_name())

        res = self.url_open('/test_http/ensure_db', headers={
            'X-Sigil-Database': f'not-{get_db_name()}',
        })
        self.assertEqual(res.status_code, 403, res.text)
        self.assertIn(
            "Cannot use both the session_id cookie and the x-sigil-database header.",
            res.text,
        )
    ...
    def test_session16_soft_rotate_with_abort(self):
        ...
        # the abort in sigil.http.JsonRPCDispatcher.dispatch.
...
class TestSessionStore(HttpCaseWithUserDemo):
    ...
    def test01_session_nan(self):
        ...
        with self.assertLogs('sigil.http', level='WARNING') as logs:
            self.assertEqual(sigil.http.get_session_max_inactivity(self.env), SESSION_LIFETIME)
            self.assertEqual(logs.output[0], "WARNING:sigil.http:Invalid value for 'sessions.max_inactivity_seconds', using default value.")
    ...
    def test02_session_lifetime_1week(self):
        ...
        with freeze_time() as freeze:
            ...
            self.env['ir.http']._gc_sessions()
            session_from_store = sigil.http.root.session_store.get(session.sid)
    ...
    def test04_session_lifetime_nodb(self):
        ...
        with freeze_time() as freeze:
            ...
            self.env['ir.http']._gc_sessions()
            session_from_store = sigil.http.root.session_store.get(session)
...
class TestSessionRotation(HttpCase):
    ...
    def test_session_rotation_interval_skip_header(self):
        ...
        self.url_open("/sigil", headers={SESSION_ROTATION_INTERVAL_HEADER_SKIP: "1"})
