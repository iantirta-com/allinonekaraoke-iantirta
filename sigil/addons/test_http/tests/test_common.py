...
import sigil
from sigil.http import Session
from sigil.addons.base.tests.common import HttpCaseWithUserDemo
from sigil.tools import config, reset_cached_properties
from sigil.addons.test_http.utils import MemoryGeoipResolver, MemorySessionStore
...
class TestHttpBase(HttpCaseWithUserDemo):
    ...
    def setUpClass(cls):
        ...
        cls.classPatch(sigil.http.Application, 'session_store', session_store)
        cls.classPatch(sigil.http.Application, 'geoip_city_db', geoip_resolver)
        cls.classPatch(sigil.http.Application, 'geoip_country_db', geoip_resolver)

    def setUp(self):
        super().setUp()
        sigil.http.root.session_store.store.clear()
    ...
    def nodb_url_open(self, url, *args, allow_redirects=False, **kwargs):
        with patch('sigil.http.db_list') as db_list, \
             patch('sigil.http.db_filter') as db_filter:
    ...
    def multidb_url_open(self, url, *args, allow_redirects=False, dblist=(), **kwargs):
        ...
        with patch('sigil.http.db_list') as db_list, \
             patch('sigil.http.db_filter') as db_filter, \
             patch('sigil.http.Registry') as Registry:
