...
from sigil.sql_db import close_db, db_connect
from sigil.tests import HOST, BaseCase, Like, get_db_name, tagged
from sigil.tools import mute_logger, reset_cached_properties, SQL
from sigil.tools.urls import urljoin
...
class TestHttpRegistry(BaseCase):
    ...
    def setUpClass(cls):
        ...
        cls.addClassCleanup(reset_cached_properties, sigil.http.root)
        cls.classPatch(sigil.conf, 'server_wide_modules', ['base', 'web', 'rpc', 'test_http'])

        # make sure there are always many databases, to break monodb
        cls._db_list = cls.startClassPatcher(patch('sigil.http.db_list'))
        cls._db_list.return_value = ['postgres', get_db_name()]
        cls.startClassPatcher(patch('sigil.http.db_filter',
    ...
    def authenticate(self, *, db=None):
        session = sigil.http.root.session_store.new()
        session.update(sigil.http.get_default_session(), db=db or get_db_name())
    ...
    def url_open(self, path, *, allow_redirects=False):
        ...
        url = urljoin(f"http://{HOST}:{sigil.tools.config['http_port']}", path)
    ...
    def test_signaling(self):
        ...
        with self.assertLogs('sigil.registry', logging.INFO) as capture:
            res = self.url_open('/test_http/ensure_db')
            self.assertEqual(res.status_code, 200)
        self.assertEqual(capture.output, [
            "INFO:sigil.registry:Reloading the model registry after database signaling.",
            Like("INFO:sigil.registry:Registry loaded in ...s"),
        ])
    ...
    def test_missing_db(self):
        ...
        with self.assertLogs('sigil.http', logging.WARNING) as capture:
            res = self.url_open('/test_http/ensure_db')
        ...
        self.assertEqual(capture.output, [
            Like("WARNING:sigil.http:Database or registry unusable, trying without\n"
    ...
    def test_corrupt_ir_module_module_table(self):
        ...
        with self.assertLogs('sigil.registry', logging.ERROR) as capture1, \
             self.assertLogs('sigil.http', logging.WARNING) as capture2:
            res = self.url_open('/test_http/greeting-public')
            self.assertEqual(res.status_code, 404)
        self.assertEqual(capture1.output, [
            "ERROR:sigil.registry:Failed to load registry",
        ])
