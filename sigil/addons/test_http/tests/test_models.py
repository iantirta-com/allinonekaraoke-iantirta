...
import sigil.http
from sigil.tests import tagged, get_db_name
from sigil.tests.common import new_test_user, Like
from sigil.tools import mute_logger
from sigil.tools.misc import submap
from sigil.addons.test_http.utils import HtmlTokenizer
...
class TestHttpModels(TestHttpBase):
    ...
    def test_models3_stargate_ko(self):
        ...
        self.assertEqual(logs.output, ["WARNING:sigil.http:The goauld destroyed the gate"])
    ...
    def test_models4_stargate_setname(self):
        ...
        res = self.url_open(f'/test_http/{milky_way.id}/setname?readonly=0', {
            'name': "Wilky May",
            'csrf_token': sigil.http.Request.csrf_token(self),
        })
    ...
    def test_models5_stargate_setname_readonly(self):
        ...
        with self.assertLogs('sigil.http', 'WARNING') as capture_http,\
             self.assertLogs('sigil.sql_db', 'WARNING') as capture_sql_db:
    ...
    def test_models6_rpc_path_poisoning(self):
        with self.assertLogs('werkzeug', logging.INFO) as capture:
            with mute_logger('sigil.addons.rpc.controllers.xmlrpc'):
