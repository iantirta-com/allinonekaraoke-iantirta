...
import sigil
from sigil.http import content_disposition, root
from sigil.tests import Like, tagged
from sigil.tests.common import HOST, BaseCase, get_db_name, new_test_user
from sigil.tools import config, file_path, mute_logger, parse_version

from .test_common import TestHttpBase
from sigil.addons import test_http
from sigil.addons.test_http.controllers import CT_JSON
from sigil.addons.test_http.utils import TEST_IP
...
class TestHttpMisc(TestHttpBase):
    ...
    def test_misc2_local_redirect(self):
        def local_redirect(path):
            fake_req = sigil.tools.misc.DotDict(db=False)
            return sigil.http.Request.redirect(fake_req, path, local=True).headers['Location']
    ...
    def test_misc3_is_static_file(self):
        ...
        self.assertEqual(root.get_static_file(f'iantirta.com/{uri}', host='iantirta.com'), path, "Valid file with valid host")
        self.assertEqual(root.get_static_file(f'http://iantirta.com/{uri}', host='iantirta.com'), path, "Valid file with valid host")
        ...
        self.assertIsNone(root.get_static_file(f'iantirta.com/{uri}'), "No host allowed")
        self.assertIsNone(root.get_static_file(f'http://iantirta.com/{uri}'), "No host allowed")
    ...
    def test_misc4_rpc_qweb(self):
        ...
        for method in (self.db_url_open, self.nodb_url_open):
            with self.subTest(method=method.__name__):
                with mute_logger('sigil.addons.rpc.controllers.jsonrpc'):
    ...
    def test_misc5_geoip(self):
        ...
        headers = {
            ...
            'X-Forwarded-Host': 'iantirta.com',
            'X-Forwarded-Proto': 'https'
        }
        with patch.dict(sigil.tools.config.options, {'proxy_mode': True}):
    ...
    def test_misc8_concurrency_error(self):
        with (
            self.assertLogs('sigil.service.model') as log_catcher,
...
class TestHttpEnsureDb(TestHttpBase):
    ...
    def test_ensure_db1_grant_db(self):
        ...
        self.assertEqual(sigil.http.root.session_store.get(res.cookies['session_id']).db, 'db0')
    ...
    def test_ensure_db3_change_db(self):
        ...
        new_session = sigil.http.root.session_store.get(res.cookies['session_id'])
    ...
    def test_ensure_db4_unicode(self):
        ...
        self.assertEqual(
            sigil.http.root.session_store.get(res.cookies['session_id']).db,
            'basededonnée1')
...
class TestFragmentToQueryString(TestHttpBase):
    def test_fragment_to_query_string(self):
        ...
        with self.assertLogs(
            'sigil.addons.test_http.controllers.test_fragment_to_query_string',
            logging.INFO,
        ) as capture:
