...
def parse_version(s):
    # can't use sigil.tools.parse_version because pythonpath is screwed from
    # inside pylint on runbot
    return [s.rjust(3, '0') for s in s.split('.')]


class SigilBaseChecker(BaseChecker):
    ...
    msgs = {
        'E8502': (
            ...
            'gettext-variable',
            'See https://www.iantirta.com/documentation/master/developer/howtos/translations.html#variables',
        ),
        'E8505': (
...
def register(linter):
    linter.register_checker(SigilBaseChecker(linter))
