...
from sigil.tests.common import TransactionCase, no_retry
from sigil.tools.which import which

from . import _sigil_checker_sql_injection
...
class TestGetTextLint(TestPylintChecks):
    def check(self, testtext):
        return super().check(testtext, "_sigil_checker_gettext", "gettext-placeholders")
...
class TestSqlLint(TestPylintChecks):
    def check(self, testtext):
        return super().check(testtext, "_sigil_checker_sql_injection", "sql-injection")
    ...
    def test_sql_injection_detection(self):
        ...
        checker = _sigil_checker_sql_injection.SigilBaseChecker(self.linter)
...
class TestI18nChecks(TestPylintChecks):
    def check(self, test_content):
        return super().check(
            test_content, "_sigil_checker_gettext", "missing-gettext,gettext-variable,gettext-placeholders,gettext-repr"
        )
