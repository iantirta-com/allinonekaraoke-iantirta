...
from sigil.addons.account.tests.common import AccountTestMockOnlineSyncCommon
...
class BaseTestUi(AccountTestMockOnlineSyncCommon):
    ...
    def main_flow_tour(self):
        ...
        self.start_tour("/sigil", 'main_flow_tour', login="admin", timeout=180)


@sigil.tests.tagged('post_install', '-at_install', 'is_tour')
class TestUi(BaseTestUi):
    ...
    def test_company_switch_access_error(self):
        ...
        with mute_logger("sigil.http"):
            self.start_tour(f"/sigil/action-{act_window.id}", "test_company_switch_access_error", login="admin", cookies={"cids": f"{company1.id}-{company2.id}"})
    ...
    def test_company_access_error_redirect(self):
        ...
        with mute_logger("sigil.http"):
            self.start_tour(f"/sigil/action-{act_window.id}/{record_p2.id}", "test_company_access_error_redirect", login="admin", cookies={"cids": f"{company1.id}"})
    ...
    def test_company_switch_access_error_debug(self):
        ...
        with mute_logger("sigil.http"):
            self.start_tour(f"/sigil/action-{act_window.id}?debug=assets", "test_company_switch_access_error", login="admin", cookies={"cids": current_companies})
