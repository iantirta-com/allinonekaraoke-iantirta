...
from sigil.tools import get_lang
from sigil.tools.urls import urljoin as url_join
...
class IapAccount(models.Model):
    ...
    def get_config_account_url(self):
        ...
        if not self.env.user.has_group('base.group_no_one'):
            return False
        if account:
            url = f"/sigil/action-iap.iap_account_action/{account.id}?menu_id={menu.id}"
        else:
            url = f"/sigil/action-iap.iap_account_action?menu_id={menu.id}"
        return url
