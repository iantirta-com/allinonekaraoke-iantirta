...
export class ClipboardPlugin extends Plugin {
    ...
    onPaste(ev) {
        ...
        if (!this.delegateTo("paste_overrides", selection, ev.clipboardData)) {
            this.handlePasteUnsupportedHtml(selection, ev.clipboardData) ||
                this.handlePasteSigilEditorHtml(ev.clipboardData) ||
     ...
     * @param {DataTransfer} clipboardData
     */
    handlePasteSigilEditorHtml(clipboardData) {
        const sigilEditorHtml = clipboardData.getData("application/vnd.sigil.sigil-editor");
        const textContent = clipboardData.getData("text/plain");
        if (ONLY_LINK_REGEX.test(textContent)) {
            return false;
        }
        if (sigilEditorHtml) {
            const fragment = parseHTML(this.document, sigilEditorHtml);
    ...
    async onDrop(ev) {
        ...
        const sigilEditorHtml = ev.dataTransfer.getData("application/vnd.sigil.sigil-editor");
        const fileTransferItems = !sigilEditorHtml && getImageFiles(dataTransfer);
        const htmlTransferItem = [...dataTransfer.items].find((item) => item.type === "text/html");
        if (fileTransferItems.length || htmlTransferItem || sigilEditorHtml) {
