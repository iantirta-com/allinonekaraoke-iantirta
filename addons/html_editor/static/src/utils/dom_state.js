...
export function prepareUpdate(...args) {
    const closestRoot =
        args.length &&
        ancestors(args[0]).find((ancestor) => ancestor.classList.contains("sigil-editor-editable"));
    ...
    while (positions.length) {
        ...
        if (options.debug) {
            const editable = el && closestElement(el, ".sigil-editor-editable");
...
export function restoreState(prevStateData, debug = false) {
    ...
    if (debug) {
        const editable = closestElement(node, ".sigil-editor-editable");
        console.log(
            "%c" +
