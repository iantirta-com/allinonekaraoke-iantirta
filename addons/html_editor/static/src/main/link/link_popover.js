...
export class LinkPopover extends Component {
    ...
    async loadAsyncLinkPreview() {
        ...
        if (!protocol.startsWith("http")) {
            ...
            !new RegExp(`^https?://${session.db}\\.sigil\\.com(/.*)?$`).test(url.origin)
        ) {
    ...
    isAbsoluteURLInCurrentDomain() {
        ...
        // its Sigil instance domain. We just suppose it should be a relative
        // URL (if unexpected behavior, the user can just not enter its Sigil
        // instance domain but its real domain, or opt-out from the domain
        // stripping). Mentioning an .iantirta.com domain, especially its own
        // one, is always a bad practice anyway.
        return (
            urlObj.origin === window.location.origin ||
            new RegExp(`^https?://${session.db}\\.sigil\\.com(/.*)?$`).test(urlObj.origin)
        );
    }
}
