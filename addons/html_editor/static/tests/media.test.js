...
import { click, dblclick, press, waitFor, waitForNone } from "@sigil/hoot-dom";
...
test("Replace an image with link by a document should remove the link", async () => {
    ...
    expect(".sigil-editor-editable .o_file_box a:contains('file.txt')").toHaveCount(1);
    expect("img[src='/web/static/img/logo.png']").toHaveCount(0);
...
test("cropper should not open for external image", async () => {
    ...
    await setupEditor(
        `<p>[<img src="https://download.sigilcdn.com/icons/website/static/description/icon.png">]</p>`
    );
