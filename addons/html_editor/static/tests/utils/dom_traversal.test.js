...
describe("closestElement", () => {
    ...
    test("should not find a node which is not contained inside a .sigil-editor-editable", () => {
        const [div] = insertTestHtml(`<div><p>abc</p></div>`);
        const p = div.querySelector("p");
        let result = closestElement(p, "div");
        expect(result).toBe(div);
        const fixture = getFixture();
        fixture.classList.remove("sigil-editor-editable");
        result = closestElement(p, "div");
        expect(result).toBe(null);
    });

    test("should find a disconnected node even if not contained inside a .sigil-editor-editable element", () => {
        const [div] = insertTestHtml(`<div><p>abc</p></div>`);
        const p = div.querySelector("p");
        div.remove();
