...
test("should paste Sigil editor html as plain text inside inline code", async () => {
    await testEditor({
        contentBefore: `<p>ab<code class="o_inline_code">Inline[]Code</code>cd</p>`,
        stepFunction: async (editor) => {
            const clipboardData = new DataTransfer();
            clipboardData.setData("text/plain", "Hello Sigil self.env.cr._enable_logging()");
            clipboardData.setData(
                "application/vnd.sigil.sigil-editor",
                `<p class="o_paragraph">Hello <strong>Sigil </strong><a href="http://self.env.cr">self.env.cr</a>._enable_logging()</p>`
            );
            const pasteEvent = new ClipboardEvent("paste", {
                clipboardData,
                bubbles: true,
        ...
        contentAfter: `<p>ab<code class="o_inline_code">InlineHello Sigil self.env.cr._enable_logging()[]Code</code>cd</p>`,
    });
});
