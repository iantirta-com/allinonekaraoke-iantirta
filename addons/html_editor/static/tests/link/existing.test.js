...
import { click, waitFor } from "@sigil/hoot-dom";
...
test("when label === url popover label input should be empty", async () => {
    await setupEditor('<p>abc <a href="http://iantirta.com">http://odo[]o.com</a> def</p>');
...
test("when label === url changing url should change label", async () => {
    const { el } = await setupEditor(
        '<p>abc <a href="http://iantirta.com">http://odo[]o.com</a> def</p>'
    );
    await waitFor(".o-we-linkpopover");
    await click(".o_we_edit_link");
    await waitFor(".o_we_label_link");
