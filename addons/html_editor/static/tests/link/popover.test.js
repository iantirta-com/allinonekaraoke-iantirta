...
describe("Link creation", () => {
    describe("Creation by space", () => {
        ...
        test("typing invalid URL + space should not convert to link", async () => {
            const { editor, el } = await setupEditor("<p>[]</p>");
            await insertText(editor, "www.sigil");
            await insertText(editor, " ");
            expect(cleanLinkArtifacts(getContent(el))).toBe("<p>www.sigil []</p>");
        });
        test("typing uppercase URL + space should convert to link", async () => {
            const { editor, el } = await setupEditor("<p>[]</p>");
            await insertText(editor, "http://SIGIL.COM");
            await insertSpace(editor);
            expect(cleanLinkArtifacts(getContent(el))).toBe(
                '<p><a href="http://SIGIL.COM">http://SIGIL.COM</a>&nbsp;[]</p>'
            );
        });
    });
...
describe("link preview", () => {
    test("test internal link preview", async () => {
        ...
        await click(".o-we-command-name:first");
        await contains(".o-we-linkpopover input.o_we_href_input_link").fill(
            window.location.origin + "/sigil/project/1/tasks/8"
        );
        await animationFrame();
    ...
    test("test external link preview", async () => {
        onRpc("/html_editor/link_preview_external", () => ({
            og_description:
                "From ERP to CRM, eCommerce and CMS. Download Sigil or use it in the cloud. Grow Your Business.",
            og_image: "https://www.iantirta.com/web/image/41207129-1abe7a15/homepage-seo.png",
            og_title: "Open Source ERP and CRM | Sigil",
            og_type: "website",
            og_site_name: "Sigil",
            source_url: "http://iantirta.com/",
        }));
        const { editor } = await setupEditor(`<p>[]<br></p>`);
        await insertText(editor, "/link");
        await animationFrame();
        await click(".o-we-command-name:first");
        await contains(".o-we-linkpopover input.o_we_href_input_link").fill("http://iantirta.com/");
        await animationFrame();
        expect(".o_we_replace_title_btn").toHaveCount(1);
        expect(".o_extra_info_card").toHaveCount(1);
        expect(".o_we_url_link").toHaveText("Open Source ERP and CRM | Sigil");
    ...
    test("test internal metadata cached correctly", async () => {
        ...
        onRpc("/sigil/cachetest/8", () => "");
        const { editor } = await setupEditor(`<p>abc[]</p>`);
        await insertText(editor, "/link");
        await animationFrame();
        await click(".o-we-command-name:first");
        await contains(".o-we-linkpopover input.o_we_href_input_link").fill(
            window.location.origin + "/sigil/cachetest/8"
        );
        await animationFrame();
    ...
    test("should use cached metadata even if protocol changes", async () => {
        ...
        onRpc("/sigil/cachetest/8", (request) => {
        ...
        const testUrl = `${wrongProtocol}//${window.location.host}/sigil/cachetest/8`;
    ...
    test("test external metadata cached correctly", async () => {
        const title = "Open Source ERP and CRM | Sigil";
        const description =
            "From ERP to CRM, eCommerce and CMS. Download Sigil or use it in the cloud. Grow Your Business.";
        onRpc("/html_editor/link_preview_external", () => {
...
describe("label is a valid URL", () => {
    ...
    test("popover should display href URL even if label is a valid URL and differs from href", async () => {
        await setupEditor('<p><a href="https://iantirta.com/">googl[]e.com</a></p>');
        await waitFor(".o-we-linkpopover", { timeout: 1500 });
        expect(queryFirst(".o-we-linkpopover a").href).toBe("https://iantirta.com/");
        await click(".o_we_edit_link");
        await waitFor(".o_we_href_input_link");
        expect("input.o_we_href_input_link").toHaveValue("https://iantirta.com/");
    });
