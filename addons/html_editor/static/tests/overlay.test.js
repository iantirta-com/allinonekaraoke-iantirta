import { beforeEach, expect, test, describe, getFixture } from "@sigil/hoot";
import { setSelection } from "./_helpers/selection";
import { click, hover, queryOne, waitFor, waitForNone } from "@sigil/hoot-dom";
import {
    contains,
    defineModels,
    fields,
...
import { Component, onMounted, onWillUnmount, xml } from "@sigil/owl";
...
test("Toolbar should be visible after scroll bar is added", async () => {
    ...
    const p = queryOne(".sigil-editor-editable p");
...
test("Toolbar should not overflow scroll container at the bottom", async () => {
    ...
    const lastP = queryOne(".sigil-editor-editable p:last-child");
...
describe("powerbox", () => {
    ...
    test("Powerbox should be visible in a editable with small height", async () => {
        ...
        setSelection({ anchorNode: queryOne(".sigil-editor-editable p"), anchorOffset: 1 });
        insertText(editor, "/");
    ...
    test("Powerbox should be visible in a editable with small height (2)", async () => {
        ...
        const thirdP = queryOne(".sigil-editor-editable p:nth-child(3)");
...
test("Table column control should always be displayed on top of the table", async () => {
    ...
    const scrollableElement = queryOne(".o_content");
    const table = queryOne(".sigil-editor-editable table");
    await hover(".sigil-editor-editable td");
