...
import { advanceTime, animationFrame, tick, waitUntil } from "@sigil/hoot-dom";
import { Component, xml, markup } from "@sigil/owl";
...
class PeerTest {
    ...
    async focus() {
        return this.plugins["collaborationSigil"].joinPeerToPeer();
    }
    ...
    async writeToServer() {
        ...
        const lastId = this.plugins.collaborationSigil.getLastHistoryStepId(
        ...
        for (const peer of Object.values(this.peers)) {
            ...
            peer.onlineMutex.exec(async () =>
                peer.plugins.collaborationSigil.onServerLastIdUpdate(String(lastId))
            );
        }
    }
    async setOnline() {
...
class Wysiwygs extends Component {
    ...
    onLoad(peerId, editor) {
        ...
        editor.startPlugins = () => {
            ...
            patch(plugins["collaborationSigil"], {
                getMetadata() {
...
describe("Stale detection & recovery", () => {
    describe("detect stale while unfocused", () => {
        test("should do nothing until focus", async () => {
            ...
            expect(peers.p1.plugins.collaborationSigil.isDocumentStale).toBe(false, {
            ...
            expect(peers.p2.plugins.collaborationSigil.isDocumentStale).toBe(false, {
            ...
            expect(peers.p3.plugins.collaborationSigil.isDocumentStale).toBe(true, {
            ...
            expect(peers.p3.plugins.collaborationSigil.isDocumentStale).toBe(false, {
    ...
    describe("detect stale while focused", () => {
        describe("recover from missing steps", () => {
            test("should recover from missing steps", async () => {
                ...
                const p3Spies = makeSpies(peers.p3.plugins.collaborationSigil, [
                ...
                expect(peers.p1.plugins.collaborationSigil.historyShareId).toBe(
                    peers.p2.plugins.collaborationSigil.historyShareId,
                    {
                        message: "p1 and p2 should have the same historyShareId",
                    }
                );
                expect(peers.p1.plugins.collaborationSigil.historyShareId).toBe(
                    peers.p3.plugins.collaborationSigil.historyShareId,
                    {
        ...
        describe("recover from snapshot", () => {
            test("should wait for all peer to recover from snapshot", async () => {
                ...
                const p2Spies = makeSpies(peers.p2.plugins.collaborationSigil, [
                    "recoverFromStaleDocument",
                    "resetFromServerAndResyncWithPeers",
                    "processMissingSteps",
                    "applySnapshot",
...
describe("Disconnect & reconnect", () => {
    test("should sync history when disconnecting and reconnecting to internet", async () => {
        ...
        const p1PromiseForMissingStep = new Promise((resolve) => {
            patch(peers.p2.plugins.collaborationSigil, {
        ...
        const p2PromiseForMissingStep = new Promise((resolve) => {
            patch(peers.p1.plugins.collaborationSigil, {
                async processMissingSteps() {
                    super.processMissingSteps(...arguments);
                    resolve();
                },
...
describe("History steps Ids", () => {
    ...
    test("should clear history step ids when resetting from server", async () => {
        ...
        expect(peers.p2.plugins.collaborationSigil.isDocumentStale).toBe(true, {
