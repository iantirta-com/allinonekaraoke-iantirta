...
import { beforeEach, describe, expect, getFixture, test } from "@sigil/hoot";
import { click, queryFirst, waitFor } from "@sigil/hoot-dom";
    ...
    test("Can mount and destroy recursive embedded components in any order", async () => {
        ...
        expect(
            [...fixture.querySelectorAll("[data-embedded]")].filter(
                (elem) => !elem.closest(".sigil-editor-editable")
            )
        ).toEqual([]);
    });
