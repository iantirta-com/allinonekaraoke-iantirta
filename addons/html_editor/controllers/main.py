...
from sigil import _, http, tools, SUPERUSER_ID
from sigil.addons.html_editor.tools import get_video_url_data
from sigil.exceptions import UserError, MissingError, AccessError
from sigil.fields import Domain
from sigil.http import request
from sigil.tools.image import image_process, image_data_uri, binary_to_image, get_webp_size
from sigil.tools.mimetypes import guess_mimetype
from sigil.tools.misc import file_open
from sigil.addons.iap.tools import iap_tools
from sigil.addons.mail.tools import link_preview
from lxml import html, etree

from ..models.ir_attachment import SUPPORTED_IMAGE_MIMETYPES

DEFAULT_LIBRARY_ENDPOINT = 'https://media-api.iantirta.com'
DEFAULT_OLG_ENDPOINT = 'https://olg.api.iantirta.com'
    ...
    def link_preview_metadata_internal(self, preview_url):
        try:
            ...
            if not (
                ...
                and (
                    parsed_preview_url.path.startswith("/sigil")
            ...
            if (action_name.startswith('m-') or '.' in action_name) and action_name in request.env and not request.env[action_name]._abstract:
                # if path format is `sigil/<model>/<record_id>` so we use `action_name` as model name
