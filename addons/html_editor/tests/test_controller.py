...
import sigil.tests
from sigil.tests.common import HttpCase, new_test_user
from sigil.tools.json import scriptsafe as json_safe
from unittest.mock import patch
from sigil.addons.mail.tools import link_preview


@sigil.tests.tagged('-at_install', 'post_install')
class TestController(HttpCase):
    ...
    def test_05_internal_link_preview(self):
        ...
        response_without_preview_name = self.url_open(
            ...
            data=json_safe.dumps({
                "params": {
                    "preview_url": _get_full_url(f"/sigil/users/{self.portal_user.id}"),
                }
            }),
        ...
        response_wrong_action = self.url_open(
            ...
            data=json_safe.dumps({
                "params": {
                    "preview_url": _get_full_url("/sigil/actionInvalid/1"),
                }
            }),
            headers=self.headers
        )
        ...
        response_wrong_record = self.url_open(
            ...
            data=json_safe.dumps({
                "params": {
                    "preview_url": _get_full_url("/sigil/users/9999"),
                }
            }),
            headers=self.headers
        )
        ...
        # Attempt to retrieve metadata for path format `sigil/<model>/<record_id>`
        response_model_record = self.url_open(
            '/html_editor/link_preview_internal',
            data=json_safe.dumps({
                "params": {
                    "preview_url": _get_full_url(f"/sigil/res.users/{self.portal_user.id}"),
                }
            }),
            headers=self.headers
        )
        ...
        response_abstract_model = self.url_open(
            ...
            data=json_safe.dumps({
                "params": {
                    "preview_url": _get_full_url("/sigil/mail.thread/1"),
                }
            }),
            headers=self.headers
        )
