...
from sigil.tests import new_test_user, tagged
...
class TestIrWebsocket(WebsocketCase):
    def test_only_allow_string_channels_from_frontend(self):
        with self.assertLogs("sigil.addons.bus.websocket", level="ERROR") as log:
            ws = self.websocket_connect()
            self.subscribe(ws, [("sigil", "discuss.channel", 5)], self.env["bus.bus"]._bus_last_id())
    ...
    def test_build_bus_channel_list(self):
        ...
        with patch("sigil.addons.bus.models.ir_websocket.wsrequest", new=mock_wsrequest):
