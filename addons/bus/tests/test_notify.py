...
from ..models.bus import json_dump, get_notify_payloads, NOTIFY_PAYLOAD_MAX_LENGTH, SIGIL_NOTIFY_FUNCTION
...
class NotifyTests(TransactionCase):
    ...
    def test_postcommit(self):
        ...
        if SIGIL_NOTIFY_FUNCTION != "pg_notify":
            return
        channels = []
        ...
        def single_listen():
            nonlocal channels
            with (
                sigil.sql_db.db_connect("postgres").cursor() as cr,
