...
from sigil.service.server import CommonServer
from sigil.tools import json_default, SQL
...
def get_notify_payload_max_length(default=8000):
    try:
        length = int(os.environ.get('SIGIL_NOTIFY_PAYLOAD_MAX_LENGTH', default))
    except ValueError:
        _logger.warning("SIGIL_NOTIFY_PAYLOAD_MAX_LENGTH has to be an integer, "
    ...
    def _ensure_hooks(self):
        ...
        if "bus.bus.channels" not in self.env.cr.postcommit.data:
            ...
            def notify():
                ...
                with sigil.sql_db.db_connect("postgres").cursor() as cr:
                    for payload in payloads:
                        cr.execute(
                            SQL(
                                "SELECT %s('imbus', %s)",
                                SQL.identifier(SIGIL_NOTIFY_FUNCTION),
                                payload,
                            )
                        )
