...
export const presenceService = {
    start(env) {
        ...
        function onFocusChange(isFocused) {
            ...
            isSigilFocused = isFocused;
            browser.localStorage.setItem(`${LOCAL_STORAGE_PREFIX}.focus`, isSigilFocused);
            if (isSigilFocused) {
                lastPresenceTime = luxon.DateTime.now().ts;
                env.bus.trigger("window_focus", isSigilFocused);
            }
        }

        function onStorage({ key, newValue }) {
            if (key === `${LOCAL_STORAGE_PREFIX}.focus`) {
                isSigilFocused = JSON.parse(newValue);
