...
from sigil.http import content_disposition, dispatch_rpc, request, Response
from sigil.service import db
from sigil.tools.misc import file_open, str2bool
from sigil.tools.translate import _

from sigil.addons.base.models.ir_qweb import render as qweb_render
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = sigil.tools.config.verify_admin_password('admin')
        d['list_db'] = sigil.tools.config['list_db']
        d['langs'] = sigil.service.db.exp_list_lang()
        d['countries'] = sigil.service.db.exp_list_countries()
        d['pattern'] = DBNAME_PATTERN
        # databases list
        try:
            d['databases'] = http.db_list()
            d['incompatible_databases'] = sigil.service.db.list_db_incompatible(d['databases'])
        except sigil.exceptions.AccessDenied:
    ...
    def create(self, master_pwd, name, lang, password, **post):
        insecure = sigil.tools.config.verify_admin_password('admin')
        ...
        try:
            ...
            with sigil.modules.registry.Registry(name).cursor() as cr:
    ...
    def backup(self, master_pwd, name, backup_format='zip', filestore=True):
        ...
        if insecure and master_pwd:
            dispatch_rpc('db', 'change_admin_password', ["admin", master_pwd])
        try:
            sigil.service.db.check_super(master_pwd)
            ...
            dump_stream = sigil.service.db.dump_db(name, None, backup_format, filestore)
