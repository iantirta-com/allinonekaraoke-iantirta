...
from sigil.tools.translate import JAVASCRIPT_TRANSLATION_COMMENT
...
def get_action_triples(env, path, *, start_pos=0):
    """
    Extract the triples (active_id, action, record_id) from a "/sigil"-like path.
...
def _get_login_redirect_url(uid, redirect=None):
    ...
    if request.session.uid:  # fully logged
        return redirect or ('/sigil' if is_user_internal(request.session.uid)
