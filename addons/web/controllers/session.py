...
from sigil.modules import module
from sigil.exceptions import AccessError, UserError, AccessDenied
...
class Session(http.Controller):
    ...
    def get_session_info(self):
        # Crapy workaround for unupdatable Sigil Mobile App iOS (Thanks Apple :@)
    ...
    def authenticate(self, db, login, password, base_location=None):
        ...
        with ExitStack() as stack:
            if not request.db or request.db != db:
                ...
                cr = stack.enter_context(sigil.modules.registry.Registry(db).cursor())
            ...
            if auth_info['uid'] != request.session.uid:
                # Crapy workaround for unupdatable Sigil Mobile App iOS (Thanks Apple :@) and Android
    ...
    def account(self):
        ...
        return 'https://accounts.iantirta.com/oauth2/auth?' + url_encode(params)
