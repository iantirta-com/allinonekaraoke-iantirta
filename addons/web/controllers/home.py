...
from sigil.tools.misc import hmac
...
class Home(http.Controller):
    ...
    def index(self, s_action=None, db=None, **kw):
        ...
        return request.redirect_query('/sigil', query=request.params)

    def _web_client_readonly(self, rule, args):
        return False

    # ideally, this route should be `auth="user"` but that don't work in non-monodb mode.
    @http.route(['/web', '/sigil', '/sigil/<path:subpath>', '/scoped_app/<path:subpath>'], type='http', auth="none", readonly=_web_client_readonly)
    ...
    def web_login(self, redirect=None, **kw):
        ...
        if request.httprequest.method == 'POST':
            ...
            except sigil.exceptions.AccessDenied as e:
                if e.args == sigil.exceptions.AccessDenied().args:
    ...
    def switch_to_admin(self):
        ...
        if request.env.user._is_system():
            uid = request.session.uid = sigil.SUPERUSER_ID
    ...
    def health(self, db_server_status=False):
        ...
        if db_server_status:
            try:
                sigil.sql_db.db_connect('postgres').cursor().close()
