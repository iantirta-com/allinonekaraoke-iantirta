...
try:
    ...
    from sigil.tools._vendor.send_file import send_file

import sigil
import sigil.modules.registry
from sigil import SUPERUSER_ID, _, http, api
from sigil.addons.base.models.assetsbundle import ANY_UNIQUE
from sigil.exceptions import AccessError, UserError
from sigil.http import request, Response
from sigil.tools import file_open, file_path, replace_exceptions, str2bool
from sigil.tools.image import image_guess_size_from_field_name
from sigil.tools.mimetypes import guess_mimetype

_logger = logging.getLogger(__name__)

BAD_X_SENDFILE_ERROR = """\
Sigil is running with --x-sendfile but is receiving /web/filestore requests.

With --x-sendfile enabled, NGINX should be serving the
/web/filestore route, however Sigil is receiving the
request.
...
class Binary(http.Controller):
    ...
    def content_filestore(self, _path):
        if sigil.tools.config['x_sendfile']:
            # pylint: disable=logging-format-interpolation
            _logger.error(BAD_X_SENDFILE_ERROR.format(
                data_dir=sigil.tools.config['data_dir']
            ))
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        imgext = '.png'
        dbname = request.db
        uid = (request.session.uid if dbname else None) or sigil.SUPERUSER_ID
        ...
        if not dbname:
            ...
            try:
                ...
                _logger.warning("While retrieving the company logo, using the Sigil logo instead", exc_info=True)
