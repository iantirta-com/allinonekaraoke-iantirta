...
from sigil import http, modules
from sigil.exceptions import AccessError
from sigil.http import request
from sigil.tools import file_open, file_path
...
class WebManifest(http.Controller):
        ...
        for module in module_ids:
            ...
            if data:
                shortcuts.append({
                    ...
                    'url': '/sigil?menu_id=%s' % data.mapped('res_id')[0],
    ...
    def _get_webmanifest(self):
        web_app_name = request.env['ir.config_parameter'].sudo().get_param('web.web_app_name', 'Sigil')
        manifest = {
            'name': web_app_name,
            'scope': '/sigil',
            'start_url': '/sigil',
        ...
        manifest['icons'] = [{
            'src': '/web/static/img/sigil-icon-%s.png' % size,
            'sizes': size,
            'type': 'image/png',
    ...
    def service_worker(self):
        response = request.make_response(
            ...
            [
                ...
                ('Service-Worker-Allowed', '/sigil'),
            ]
        )
        return response

    def _get_service_worker_content(self):
        """ Returns a ServiceWorker javascript file scoped for the backend (aka. '/sigil')
        """
    ...
    def _icon_path(self):
        return 'web/static/img/sigil-icon-192x192.png'

    @http.route('/sigil/offline', type='http', auth='public', methods=['GET'], readonly=True)
    def offline(self):
        """ Returns the offline page delivered by the service worker """
        return request.render('web.webclient_offline', {
            'sigil_icon': base64.b64encode(file_open(self._icon_path(), 'rb').read())
        })
    ...
    def scoped_app(self, app_id, path='', app_name=''):
        ...
        scoped_app_values = {
            'app_id': app_id,
            'apple_touch_icon': '/web/static/img/sigil-icon-ios.png',
            'app_name': app_name,
            'path': path,
    ...
    def scoped_app_manifest(self, app_id, path, app_name=''):
        """ Returns a WebManifest dedicated to the scope of the given app. A custom scope and start
            url are set to make sure no other installed PWA can overlap the scope (e.g. /sigil)
        """
        path = unquote(path)
        app_name = unquote(app_name) if app_name else self._get_scoped_app_name(app_id)
        webmanifest = {
