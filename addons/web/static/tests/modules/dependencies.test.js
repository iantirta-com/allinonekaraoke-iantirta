...
function invalidImportsFrom(folder, allowedFolders) {
    ...
    const modulesToCheck = Array.from(sigil.loader.modules.keys()).filter((module) =>
        module.startsWith(`@web/${folder}/`)
    );
    const invalidDeps = {};
    for (const module of modulesToCheck) {
        const invalid = sigil.loader.factories.get(module).deps.filter((dep) => {
            // owl and @web/session are allowed everywhere
            if (dep === "@sigil/owl" || dep === "@web/session") {
                return false;
            }
