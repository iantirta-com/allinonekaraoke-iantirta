...
import { describe, test, expect } from "@sigil/hoot";
...
import { click, freezeTime, queryAllTexts } from "@sigil/hoot-dom";
import {
    ClientErrorDialog,
    Error504Dialog,
    ErrorDialog,
...
test("WarningDialog", async () => {
    ...
    await mountWithCleanup(WarningDialog, {
        env,
        props: {
            exceptionName: "sigil.exceptions.UserError",
            message: "...",
...
test("RedirectWarningDialog", async () => {
    ...
    expect(".o_dialog").toHaveCount(1);
    expect("header .modal-title").toHaveText("Sigil Warning");
    expect("main").toHaveText("Some strange unreadable message");
...
test("SessionExpiredDialog", async () => {
    ...
    expect(".o_dialog").toHaveCount(1);
    expect(".o_dialog").toHaveCount(1);
    expect("header .modal-title").toHaveText("Sigil Session Expired");
    expect("main p").toHaveText(
        "Your Sigil session expired. The current page is about to be refreshed."
    );
    expect(".o_dialog footer button").toHaveText("Close");
