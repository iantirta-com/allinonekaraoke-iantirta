import { describe, expect, getFixture, test } from "@sigil/hoot";
import { click, on } from "@sigil/hoot-dom";
import { mockMatchMedia, tick } from "@sigil/hoot-mock";
...
describe("stateToUrl", () => {
    test("encodes URI compatible strings", (assert) => {
        expect(router.stateToUrl({})).toBe("/sigil");
        expect(router.stateToUrl({ a: "11", b: "summer wine" })).toBe("/sigil?a=11&b=summer%20wine");
        expect(router.stateToUrl({ b: "2", c: "", e: "kloug,gloubi" })).toBe(
            "/sigil?b=2&c=&e=kloug%2Cgloubi"
        );
    });
    ...
    test("backwards compatibility: no action stack, action encoded in path", (assert) => {
        expect(router.stateToUrl({})).toBe("/sigil");
        // action
        expect(router.stateToUrl({ action: "some-path" })).toBe("/sigil/some-path");
        expect(router.stateToUrl({ active_id: 5, action: "some-path" })).toBe("/sigil/5/some-path");
        expect(router.stateToUrl({ active_id: "some-active_id", action: "some-path" })).toBe(
            "/sigil/some-path?active_id=some-active_id",
            { message: "only numeric active_id are encoded in path" }
        );
        expect(router.stateToUrl({ action: "some-path", resId: 2 })).toBe("/sigil/some-path/2");
        expect(router.stateToUrl({ action: "some-path", resId: "some-resId" })).toBe(
            "/sigil/some-path?resId=some-resId",
        ...
        expect(router.stateToUrl({ active_id: 5, action: "some-path", resId: "new" })).toBe(
            "/sigil/5/some-path/new"
        );
        expect(router.stateToUrl({ action: 1, resId: 2 })).toBe("/sigil/action-1/2", {
            message: "action id instead of path/tag",
        });
        expect(router.stateToUrl({ action: "module.xml_id", resId: 2 })).toBe(
            "/sigil/action-module.xml_id/2",
            { message: "action xml_id instead of path/tag" }
        );
        // model
        expect(router.stateToUrl({ model: "some.model" })).toBe("/sigil/some.model");
        expect(router.stateToUrl({ model: "some.model", resId: 2 })).toBe("/sigil/some.model/2");
        expect(router.stateToUrl({ active_id: 5, model: "some.model" })).toBe("/sigil/5/some.model");
        expect(router.stateToUrl({ active_id: 5, model: "some.model", resId: 2 })).toBe(
            "/sigil/5/some.model/2"
        );
        expect(router.stateToUrl({ active_id: 5, model: "some.model", resId: "new" })).toBe(
            "/sigil/5/some.model/new"
        );
        expect(
            router.stateToUrl({ active_id: 5, model: "some.model", view_type: "some_viewtype" })
        ).toBe("/sigil/5/some.model?view_type=some_viewtype");
        // edge cases
        expect(
            router.stateToUrl({
                active_id: 5,
        ...
        ).toBe("/sigil/5/some-path/2?some_key=some_value", {
        ...
        ).toBe("/sigil/5/some-path/2", { message: "action has priority on model" });
        expect(
            router.stateToUrl({ active_id: 5, model: "some.model", resId: 2, view_type: "list" })
        ).toBe(
            "/sigil/5/some.model/2?view_type=list",
    ...
    test("actionStack: one action", () => {
        expect(router.stateToUrl({ actionStack: [] })).toBe("/sigil");
        // action
        ...
        ).toBe("/sigil/5/some-path/2");
        expect(
            router.stateToUrl({
                actionStack: [{ active_id: 5, action: "some-path", resId: "new" }],
            })
        ).toBe("/sigil/5/some-path/new");
        ...
        ).toBe("/sigil/5/some.model/2");
        expect(
            router.stateToUrl({
                actionStack: [{ active_id: 5, model: "some.model", resId: "new" }],
            })
        ).toBe("/sigil/5/some.model/new");
        expect(
            router.stateToUrl({
                actionStack: [{ active_id: 5, model: "some.model", view_type: "some_viewtype" }],
            })
        ).toBe("/sigil/5/some.model", { message: "view_type is ignored in the action stack" });
        expect(
            router.stateToUrl({
    ...
    test("actionStack: multiple actions", () => {
        ...
        ).toBe("/sigil/some-path/other-path");
        expect(
            router.stateToUrl({
                actionStack: [{ active_id: 5, action: "some-path" }, { action: "other-path" }],
            })
        ).toBe("/sigil/5/some-path/other-path");
        expect(
            router.stateToUrl({
        ...
        ).toBe("/sigil/5/some-path/2/other-path");
        expect(
            router.stateToUrl({
                actionStack: [
                    { action: "some-path" },
        ...
        ).toBe("/sigil/some-path/5/other-path/2");
        expect(
            router.stateToUrl({
                actionStack: [
                    { action: "some-path" },
        ...
        ).toBe("/sigil/some-path/5/other-path/new");
        expect(
            router.stateToUrl({
                actionStack: [
        ...
        ).toBe("/sigil/some-path/5/other-path", {
            message:
        ...
        ).toBe("/sigil/action-1/5/action-6/2", { message: "numerical actions" });
        expect(
            router.stateToUrl({
                actionStack: [
        ...
        ).toBe("/sigil/5/some-path/some-path", {
        ...
        ).toBe("/sigil/some-path/7/some-path", {
            message:
        ...
        ).toBe("/sigil/some-path/2/some-path");
        expect(
            router.stateToUrl({
                actionStack: [
        ...
        ).toBe("/sigil/7/some-path/7/some-path", {
            message:
        ...
        ).toBe("/sigil/5/some-path/2/some-path");
        expect(
            router.stateToUrl({
                actionStack: [
                    { action: "some-path" },
        ...
        ).toBe("/sigil/some-path/5/some-path/2", {
            message:
        ...
        ).toBe("/sigil/some-path/5/some-path/new");
        expect(
            router.stateToUrl({
                actionStack: [
                    { action: "some-path", resId: 5 },
        ...
        ).toBe("/sigil/some-path/5/some-path", {
            message:
                "action with resId followed by action with same value as active_id is not duplicated",
        });
        expect(
        ...
        ).toBe("/sigil/5/some-path/5/some-path/2", {
        ...
        ).toBe("/sigil/action-1/5/action-1/2", { message: "numerical actions" });
        expect(
            router.stateToUrl({
                actionStack: [
                    { action: "module.xml_id" },
        ...
        ).toBe("/sigil/5/some.model/2/other.model");
        expect(
            router.stateToUrl({
                actionStack: [
                    { model: "some.model" },
        ...
        ).toBe("/sigil/some.model/5/other.model/2");
        expect(
            router.stateToUrl({
                actionStack: [
                    { model: "some.model" },
        ...
        ).toBe("/sigil/some.model/5/other.model/new");
        expect(
            router.stateToUrl({
                actionStack: [
        ...
        ).toBe("/sigil/some.model/5/other.model", {
            message:
                "action with resId followed by action with same value as active_id is not duplicated",
        });
        expect(
        ...
        ).toBe("/sigil/5/some-path/2/some.model");
        expect(
            router.stateToUrl({
                actionStack: [
                    { action: "some-path" },
        ...
        ).toBe("/sigil/some-path/5/some.model/2");
        expect(
            router.stateToUrl({
                actionStack: [
                    { action: "some-path" },
        ...
        ).toBe("/sigil/some-path/5/some.model/new");
        expect(
            router.stateToUrl({
                actionStack: [
                    { action: "some-path", resId: 5 },
        ...
        ).toBe("/sigil/some-path/5/some.model", {
            message:
                "action with resId followed by action with same value as active_id is not duplicated",
        });
        expect(
        ...
        ).toBe("/sigil/action-1/5/m-model_no_dot/2");
        expect(
            router.stateToUrl({
                actionStack: [
        ...
        ).toBe("/sigil/5/some.model/2/other-path");
        expect(
            router.stateToUrl({
                actionStack: [
                    { model: "some.model" },
        ...
        ).toBe("/sigil/some.model/5/other-path/2");
        expect(
            router.stateToUrl({
                actionStack: [
                    { model: "some.model" },
        ...
        ).toBe("/sigil/some.model/5/other-path/new");
        expect(
            router.stateToUrl({
                actionStack: [
                    { model: "some.model", resId: 5 },
        ...
        ).toBe("/sigil/some.model/5/other-path", {
            message:
                "action with resId followed by action with same value as active_id is not duplicated",
        });
        expect(
        ...
        ).toBe("/sigil/m-model_no_dot/5/action-1/2");
        expect(
            router.stateToUrl({
                actionStack: [
                    { model: "model_no_dot", resId: 5 },
        ...
        ).toBe("/sigil/m-model_no_dot/5/action-module.xml_id/2");
        ...
        ).toBe("/sigil/some-path/5/other-path/2?some_key=some_value", {
            message:
                "pieces of state unrelated to actions are added as query string even with actionStack",
        });
        expect(
        ...
        ).toBe("/sigil/some-path/5/other-path/2", { message: "action has priority on model" });
        expect(
            router.stateToUrl({
                actionStack: [
                    { action: "some-path", resId: 5 },
        ...
        ).toBe("/sigil/some-path/5/some.model/2?view_type=list", {
            message: "view_type and resId aren't incompatible",
        });
        expect(
            router.stateToUrl({
        ...
        ).toBe("/sigil/some-path/2/5/other-path", {
            message:
        ...
        ).toBe("/sigil/some.model/2/5/other.model", {
            message:
                "action with resId followed by action with different active_id gets both ids in a row",
        });
        expect(
        ...
        ).toBe("/sigil/other-path/5/some-path/2", {
            message:
...
describe("urlToState", () => {
    test("deserialize queryString", () => {
        expect(_urlToState("/sigil?a=11&g=summer%20wine")).toEqual({
            a: 11,
            g: "summer wine",
        });
        expect(_urlToState("/sigil?g=2&c=&e=kloug%2Cgloubi")).toEqual({
            g: 2,
            c: "",
            e: "kloug,gloubi",
        });
    ...
    test("deserialize single action", () => {
        ...
        expect(_urlToState("/sigil")).toEqual({});
        // action
        expect(_urlToState("/sigil/some-path")).toEqual({
            action: "some-path",
            actionStack: [{ action: "some-path" }],
        });
        expect(_urlToState("/sigil/5/some-path")).toEqual({
            active_id: 5,
            action: "some-path",
            actionStack: [{ active_id: 5, action: "some-path" }],
        });
        expect(_urlToState("/sigil/some-path/2")).toEqual(
            {
                action: "some-path",
                resId: 2,
                actionStack: [{ action: "some-path" }, { action: "some-path", resId: 2 }],
        ...
        expect(_urlToState("/sigil/some-path/new")).toEqual(
            {
                action: "some-path",
                resId: "new",
        ...
        expect(_urlToState("/sigil/5/some-path/2")).toEqual({
            active_id: 5,
            action: "some-path",
            resId: 2,
            actionStack: [
        ...
        expect(_urlToState("/sigil/action-1/2")).toEqual({
            action: 1,
            resId: 2,
            actionStack: [{ action: 1 }, { action: 1, resId: 2 }],
        });
        expect(_urlToState("/sigil/action-module.xml_id/2")).toEqual({
            action: "module.xml_id",
            resId: 2,
            actionStack: [{ action: "module.xml_id" }, { action: "module.xml_id", resId: 2 }],
        });
        // model
        expect(_urlToState("/sigil/some.model")).toEqual({
            model: "some.model",
            actionStack: [{ model: "some.model" }],
        });
        expect(_urlToState("/sigil/some.model/2")).toEqual(
            {
                model: "some.model",
                resId: 2,
        ...
        expect(_urlToState("/sigil/some.model/new")).toEqual(
            {
                model: "some.model",
                resId: "new",
        ...
        expect(_urlToState("/sigil/5/some.model")).toEqual({
            active_id: 5,
            model: "some.model",
            actionStack: [{ active_id: 5, model: "some.model" }],
        });
        expect(_urlToState("/sigil/5/some.model/2")).toEqual({
            active_id: 5,
            model: "some.model",
            resId: 2,
            actionStack: [{ active_id: 5, model: "some.model", resId: 2 }],
        });
        expect(_urlToState("/sigil/5/some.model?view_type=some_viewtype")).toEqual(
            {
                active_id: 5,
                model: "some.model",
                view_type: "some_viewtype",
        ...
        expect(_urlToState("/sigil/m-model_no_dot/2")).toEqual({
            model: "model_no_dot",
            resId: 2,
            actionStack: [{ model: "model_no_dot", resId: 2 }],
        });
        // edge cases
        expect(_urlToState("/sigil/5/some-path/2?some_key=some_value")).toEqual(
            {
                active_id: 5,
                action: "some-path",
                resId: 2,
        ...
        expect(_urlToState("/sigil/5/some.model/2?view_type=list")).toEqual(
            {
                active_id: 5,
                model: "some.model",
                resId: 2,
    ...
    test("deserialize multiple actions", () => {
        // action
        expect(_urlToState("/sigil/some-path/other-path")).toEqual({
            action: "other-path",
            actionStack: [{ action: "some-path" }, { action: "other-path" }],
        });
        expect(_urlToState("/sigil/5/some-path/other-path")).toEqual({
            action: "other-path",
            actionStack: [{ active_id: 5, action: "some-path" }, { action: "other-path" }],
        });
        expect(_urlToState("/sigil/some-path/2/other-path")).toEqual({
            action: "other-path",
            active_id: 2,
            actionStack: [
                { action: "some-path" },
        ...
        expect(_urlToState("/sigil/some-path/other-path/2")).toEqual({
            action: "other-path",
            resId: 2,
            actionStack: [
                { action: "some-path" },
        ...
        expect(_urlToState("/sigil/5/some-path/2/other-path")).toEqual({
            action: "other-path",
            active_id: 2,
            actionStack: [
                { active_id: 5, action: "some-path" },
        ...
        expect(_urlToState("/sigil/some-path/5/other-path/2")).toEqual({
            active_id: 5,
            action: "other-path",
            resId: 2,
            actionStack: [
        ...
        expect(_urlToState("/sigil/some-path/5/other-path/new")).toEqual({
            active_id: 5,
            action: "other-path",
            resId: "new",
            actionStack: [
        ...
        expect(_urlToState("/sigil/action-1/5/action-6/2")).toEqual({
            active_id: 5,
            action: 6,
            resId: 2,
            actionStack: [
        ...
        expect(_urlToState("/sigil/action-module.xml_id/5/action-module.other_xml_id/2")).toEqual({
            active_id: 5,
        ...
        // model
        expect(_urlToState("/sigil/some.model/other.model")).toEqual(
            {
                model: "other.model",
        ...
        expect(_urlToState("/sigil/5/some.model/other.model")).toEqual(
            {
                model: "other.model",
                actionStack: [{ model: "other.model" }],
            },
        ...
        expect(_urlToState("/sigil/some.model/7/other.model")).toEqual({
            active_id: 7,
            model: "other.model",
            actionStack: [
        ...
        expect(_urlToState("/sigil/some.model/other.model/2")).toEqual({
            model: "other.model",
            resId: 2,
            actionStack: [{ model: "other.model", resId: 2 }],
        });
        expect(_urlToState("/sigil/5/some.model/2/other.model")).toEqual({
            active_id: 2,
            model: "other.model",
            actionStack: [
                { active_id: 5, model: "some.model", resId: 2 },
        ...
        expect(_urlToState("/sigil/some.model/5/other.model/2")).toEqual({
            active_id: 5,
            model: "other.model",
            resId: 2,
            actionStack: [
        ...
        expect(_urlToState("/sigil/some.model/5/other.model/new")).toEqual({
            active_id: 5,
            model: "other.model",
            resId: "new",
            actionStack: [
        ...
        expect(_urlToState("/sigil/m-model_no_dot/5/m-no_dot_model/2")).toEqual({
            active_id: 5,
            model: "no_dot_model",
            resId: 2,
            actionStack: [
        ...
        // action + model
        expect(_urlToState("/sigil/some-path/some.model")).toEqual({
            model: "some.model",
            actionStack: [{ action: "some-path" }, { model: "some.model" }],
        });
        expect(_urlToState("/sigil/5/some-path/some.model")).toEqual({
            model: "some.model",
            actionStack: [{ active_id: 5, action: "some-path" }, { model: "some.model" }],
        });
        expect(_urlToState("/sigil/some-path/7/some.model")).toEqual({
            active_id: 7,
            model: "some.model",
            actionStack: [
                { action: "some-path" },
        ...
        expect(_urlToState("/sigil/some-path/some.model/2")).toEqual({
            model: "some.model",
            resId: 2,
            actionStack: [{ action: "some-path" }, { model: "some.model", resId: 2 }],
        });
        expect(_urlToState("/sigil/5/some-path/2/some.model")).toEqual({
            active_id: 2,
            model: "some.model",
            actionStack: [
                { active_id: 5, action: "some-path" },
        ...
        expect(_urlToState("/sigil/some-path/5/some.model/2")).toEqual({
            active_id: 5,
            model: "some.model",
            resId: 2,
            actionStack: [
        ...
        expect(_urlToState("/sigil/some-path/5/some.model/new")).toEqual({
            active_id: 5,
            model: "some.model",
            resId: "new",
            actionStack: [
        ...
        expect(_urlToState("/sigil/action-1/5/m-model_no_dot/2")).toEqual({
            active_id: 5,
            model: "model_no_dot",
            resId: 2,
            actionStack: [
        ...
        expect(_urlToState("/sigil/action-module.xml_id/5/m-model_no_dot/2")).toEqual({
            active_id: 5,
            model: "model_no_dot",
            resId: 2,
            actionStack: [
        ...
        // model + action
        expect(_urlToState("/sigil/some.model/other-path")).toEqual({
            action: "other-path",
            actionStack: [{ action: "other-path" }],
        });
        expect(_urlToState("/sigil/5/some.model/other-path")).toEqual({
            action: "other-path",
            actionStack: [{ action: "other-path" }],
        });
        expect(_urlToState("/sigil/some.model/2/other-path")).toEqual({
            active_id: 2,
            action: "other-path",
            actionStack: [
                { model: "some.model", resId: 2 },
        ...
        expect(_urlToState("/sigil/some.model/other-path/2")).toEqual({
            action: "other-path",
            resId: 2,
            actionStack: [{ action: "other-path" }, { action: "other-path", resId: 2 }],
        });
        expect(_urlToState("/sigil/5/some.model/2/other-path")).toEqual({
            active_id: 2,
            action: "other-path",
            actionStack: [
                { active_id: 5, model: "some.model", resId: 2 },
        ...
        expect(_urlToState("/sigil/some.model/5/other-path/2")).toEqual({
            active_id: 5,
            action: "other-path",
            resId: 2,
            actionStack: [
        ...
        expect(_urlToState("/sigil/some.model/5/other-path/new")).toEqual({
            active_id: 5,
            action: "other-path",
            resId: "new",
            actionStack: [
        ...
        expect(_urlToState("/sigil/m-model_no_dot/5/action-1/2")).toEqual({
            active_id: 5,
            action: 1,
            resId: 2,
            actionStack: [
        ...
        expect(_urlToState("/sigil/m-model_no_dot/5/action-module.xml_id/2")).toEqual({
            active_id: 5,
            action: "module.xml_id",
            resId: 2,
            actionStack: [
        ...
        // edge cases
        expect(_urlToState("/sigil/some-path/5/other-path/2?some_key=some_value")).toEqual({
            active_id: 5,
            action: "other-path",
            resId: 2,
            actionStack: [
        ...
        expect(_urlToState("/sigil/some-path/5/some.model?view_type=list")).toEqual(
            {
                active_id: 5,
                model: "some.model",
                actionStack: [
        ...
        expect(_urlToState("/sigil/some-path/5/some.model/2?view_type=list")).toEqual(
            {
                active_id: 5,
                model: "some.model",
                resId: 2,
        ...
        expect(_urlToState("/sigil/some-path/2/5/other-path")).toEqual(
            {
                active_id: 5,
                action: "other-path",
                actionStack: [
        ...
        expect(_urlToState("/sigil/some.model/2/5/other.model")).toEqual(
            {
                active_id: 5,
                model: "other.model",
                actionStack: [
...
describe("pushState", () => {
    ...
    test("can hide keys", async () => {
        ...
        await tick();
        expect(router.current).toEqual({ k1: 2, k2: 3 });
        expect(browser.location.href).toBe("https://www.hoot.test/sigil?k2=3");
    });
    test("different order of keys shouldn't push a new state", async () => {
        ...
        expect(browser.location.href).toBe("https://www.hoot.test/sigil?k1=2&z=1&a=2");
...
describe("History", () => {
    test("properly handles history.back and history.forward", async () => {
        ...
        router.pushState({ k1: 1 });
        await tick();
        expect(browser.location.href).toBe("https://www.hoot.test/sigil?k1=1");

        router.pushState({ k2: 2 });
        await tick();
        expect(browser.location.href).toBe("https://www.hoot.test/sigil?k1=1&k2=2");

        router.pushState({ k3: 3 }, { replace: true });
        await tick();
        expect(browser.location.href).toBe("https://www.hoot.test/sigil?k3=3");
        ...
        await tick();
        expect(browser.location.href).toBe("https://www.hoot.test/sigil?k4=3");
    ...
    test("unserialized parts of action stack are preserved when going back/forward", async () => {
        ...
        await tick();
        expect(browser.location.href).toBe("https://www.hoot.test/sigil/some-path");
        expect(router.current).toEqual({
            actionStack: [{ action: "some-path", displayName: "A cool display name" }],
        });
        router.pushState({
        ...
        await tick();
        expect(browser.location.href).toBe("https://www.hoot.test/sigil/other-path");
        expect(router.current).toEqual({
            actionStack: [{ action: "other-path", displayName: "A different display name" }],
        });
    ...
    test("properly handles history.back with hidden keys", async () => {
        ...
        expect(browser.location.href).toBe("https://www.hoot.test/sigil?k2=2");
...
describe("Scoped apps", () => {
    test("url location is changed to /sigil if the client is not used in a standalone scoped app", async () => {
        Object.assign(browser.location, { pathname: "/scoped_app/some-path" });
        createRouter();
        router.pushState({ app_name: "some_app", path: "scoped_app/some_path" });
        await tick();
        expect(browser.location.href).toBe("https://www.hoot.test/sigil/some-path?app_name=some_app&path=scoped_app%2Fsome_path");
    });
...
describe("Retrocompatibility", () => {
    test("parse an url with hash (key/values)", async () => {
        ...
        expect(browser.location.pathname).toBe("/sigil");
    });
    ...
    test("parse an url with hash (anchor link) and query string", async () => {
        redirect("/sigil?a=114&g=c.e&f=1#anchor");
        browser.location.hash = "#anchor";
...
describe("internal links", () => {
    ...
    test("click on internal link with different protocol does a loadState", async () => {
        ...
        const fixture = getFixture();
        const link = document.createElement("a");
        link.href = "http://" + browser.location.host + "/sigil/some-action/2";
        fixture.appendChild(link);
    ...
    test("click on internal link with hash (key/values)", async () => {
        ...
        const fixture = getFixture();
        const link = document.createElement("a");
        link.href = "/sigil/1/action-114/22";
        fixture.appendChild(link);
    ...
    test("click on internal link with hash (anchor)", async () => {
        ...
        const fixture = getFixture();
        const link = document.createElement("a");
        link.href = "/sigil/1/action-114/22#anchorId";
        fixture.appendChild(link);
        ...
        await tick();
        expect.verifySteps([
            "pushState: https://www.hoot.test/sigil/1/action-114/22#anchorId",
            "click",
        ]);
        expect(router.current).toEqual({
            action: 114,
