...
import { htmlEscape, markup } from "@sigil/owl";
...
test("sigilmark", () => {
    expect(sigilmark("").toString()).toBe("");
    expect(sigilmark("**test**").toString()).toBe("<b>test</b>");
    expect(sigilmark("**test** something else **test**").toString()).toBe(
        "<b>test</b> something else <b>test</b>"
    );
    expect(sigilmark("--test--").toString()).toBe(`<span class="text-muted">test</span>`);
    expect(sigilmark("--test-- something else --test--").toString()).toBe(
        `<span class="text-muted">test</span> something else <span class="text-muted">test</span>`
    );
    expect(sigilmark("`test`").toString()).toBe(
    ...
    expect(sigilmark("test\ttest2").toString()).toBe(
        `test<span style="margin-left: 2em"></span>test2`
    );
    expect(sigilmark("test\ntest2").toString()).toBe("test<br>test2");
    expect(sigilmark("<p>**test**</p>").toString()).toBe("&lt;p&gt;<b>test</b>&lt;/p&gt;");
    expect(sigilmark(markup`<p>**test**</p>`).toString()).toBe("<p><b>test</b></p>");
});
