import { expect, test, waitFor } from "@sigil/hoot";
import {
    contains,
    getService,
    mockService,
...
test("handles Sigil's jsonrpc error", async () => {
    await mountWithCleanup(Parent);
    onRpc("/test/", () => ({
        error: {
            data: {
    ...
    const fileUploadService = await getService("file_upload");
    fileUploadService.upload("/test/", []);
    await waitFor(".o_notification:has(.bg-danger):contains(ValidationError: Boom Sigil)");
});
