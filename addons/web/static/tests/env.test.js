import { after, beforeEach, describe, expect, getFixture, test } from "@sigil/hoot";
 ...
 * @param {(env: import("@web/env").SigilEnv, dependencies: Record<string, any>) => any} factory
 */
...
test(`mountComponent creates an env and sets the application as root when no env is provided`, async () => {
    ...
    after(() => {
        delete sigil.__WOWL_DEBUG__;
    });
    const { env } = app;
    expect(env.services).toEqual({ my_service: "a" });
    expect(sigil.__WOWL_DEBUG__).toEqual({ root: app.root.component });
...
test(`mountComponent uses the env when provided and doesn't start the services`, async () => {
    ...
    expect(sigil.__WOWL_DEBUG__).toBe(undefined);
    expect(getFixture()).toHaveText("Root");
});
