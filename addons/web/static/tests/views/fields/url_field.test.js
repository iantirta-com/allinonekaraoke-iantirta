import { expect, getFixture, test } from "@sigil/hoot";
import { queryAllAttributes, queryAllTexts, queryFirst, click, middleClick } from "@sigil/hoot-dom";
...
test("UrlField in form view", async () => {
    ...
    await fieldInput("url").edit("https://www.iantirta.com");
    expect(`.o_field_widget input[type="text"]`).toHaveValue("https://www.iantirta.com");
});
...
test("onchange scenario", async () => {
    ...
    Product._records = [{ id: 1, url: "iantirta.com", url_source: "another.com" }];
    await mountView({
        type: "form",
        resModel: "product",
        resId: 1,
    ...
    expect(".o_field_widget[name=url]").toHaveText("iantirta.com");
...
test("with non falsy, but non url value", async () => {
    Product._fields.url.default = "sigil://hello";
    await mountView({
        type: "form",
        resModel: "product",
        arch: `<form><field name="url" widget="url"/></form>`,
    });
    expect(".o_field_widget[name=url] a").toHaveAttribute("href", "http://sigil://hello");
});
