/** @sigil-module alias=@web/../tests/helpers/mock_server default=false */
...
export function makeServerError({
    ...
    return makeErrorFromResponse({
        code: code || 0,
        message: message || "Sigil Server Error",
        data: {
            name: errorName || `sigil.exceptions.${type || "UserError"}`,
            debug: "traceback",
            arguments: args || [],
            context: context || {},
            subType,
    ...
    mockLoadAction({ action_id } = {}) {
        ...
        if (!action) {
            throw makeServerError({
                errorName: "sigil.addons.web.controllers.action.MissingActionError",
