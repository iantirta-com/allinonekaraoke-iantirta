import { after, beforeEach, expect, getFixture, test } from "@sigil/hoot";
import {
    click,
    edit,
    on,
...
import { animationFrame, Deferred, mockSendBeacon, mockTouch, runAllTimers } from "@sigil/hoot-mock";
import {
    clickModalButton,
    clickSave,
    defineActions,
...
test("settings views does not write the id on the url", async () => {
    ...
    await getService("action").doAction(1);
    await runAllTimers();
    expect(browser.location.pathname).toBe("/sigil/settings");
    expect(".o_field_boolean input").toHaveProperty("disabled", false);
    await click(".o_field_boolean input");
    await animationFrame();
...
test("Open settings from url, with setting id anchor", async () => {
    ...
    redirect("/sigil/settings#setting_id");
