...
import { click, queryAllAttributes, queryAllProperties, queryAllTexts } from "@sigil/hoot-dom";
...
import { sigilAccountItem, preferencesItem } from "@web/webclient/user_menu/user_menu_items";
...
test("click on sigil account item", async () => {
    patchWithCleanup(browser, {
        open: (url) => expect.step(`open ${url}`),
    });
    userMenuRegistry.add("sigil_account", sigilAccountItem);
    await mountWithCleanup(UserMenu);
    onRpc("/web/session/account", () => "https://account-url.com");
    stepAllNetworkCalls();
    await contains("button.dropdown-toggle").click();
    expect(".o-dropdown--menu .dropdown-item").toHaveCount(1);
    expect(".o-dropdown--menu .dropdown-item").toHaveText("My Sigil.com Account");
