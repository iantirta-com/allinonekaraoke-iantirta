import { beforeEach, destroy, expect, test } from "@sigil/hoot";
import { queryAll, queryAllAttributes, queryAllTexts, resize } from "@sigil/hoot-dom";
import { advanceTime, animationFrame, runAllTimers } from "@sigil/hoot-mock";
import {
    clearRegistry,
    contains,
    defineMenus,
...
import { Component, onRendered, xml } from "@sigil/owl";
...
test("href attribute on apps menu items", async () => {
    ...
    await mountWithCleanup(NavBar);
    await contains(".o_navbar_apps_menu button.dropdown-toggle").click();
    expect(".o-dropdown--menu .dropdown-item").toHaveAttribute("href", "/sigil/action-339");
});
...
test("href attribute with path on apps menu items", async () => {
    ...
    await mountWithCleanup(NavBar);
    await contains(".o_navbar_apps_menu button.dropdown-toggle").click();
    expect(".o-dropdown--menu .dropdown-item").toHaveAttribute("href", "/sigil/my-path");
});
