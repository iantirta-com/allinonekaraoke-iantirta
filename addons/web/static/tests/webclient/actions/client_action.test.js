import { beforeEach, expect, test } from "@sigil/hoot";
import { animationFrame, runAllTimers } from "@sigil/hoot-mock";
...
test("test reload client action", async () => {
    ...
    await runAllTimers();
    expect.verifySteps([
        "replaceState /sigil?test=42",
        "window_reload",
        "pushState /sigil/action-2",
        "window_reload",
        "pushState /sigil?menu_id=1",
        "window_reload",
        "pushState /sigil/action-1?menu_id=2",
        "window_reload",
    ]);
});
...
test("test display_exception client action", async () => {
    ...
    getService("action").doAction({
        ...
        params: {
            ...
            data: {
                name: `sigil.exceptions.UserError`,
                debug: "traceback",
                arguments: [],
                context: {},
