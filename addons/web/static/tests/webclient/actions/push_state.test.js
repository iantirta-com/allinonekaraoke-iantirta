...
import { queryAllTexts } from "@sigil/hoot-dom";
import { Deferred, animationFrame } from "@sigil/hoot-mock";
...
test(`actions can push state`, async () => {
    ...
    await animationFrame();
    expect(browser.location.href).toBe("http://example.com/sigil/client_action_pushes");
    ...
    expect(browser.location.href).toBe(
        "http://example.com/sigil/client_action_pushes?arbitrary=actionPushed"
    );
...
test(`properly push state`, async () => {
    ...
    await animationFrame();
    expect(browser.location.href).toBe("http://example.com/sigil/action-4/action-8");
    expect(browser.history.length).toBe(3);
    expect(router.current).toEqual({
        action: 8,
        actionStack: [
    ...
    await animationFrame();
    expect(browser.location.href).toBe("http://example.com/sigil/action-4/action-8/4");
    expect(browser.history.length).toBe(4);
    expect(router.current).toEqual({
        action: 8,
        actionStack: [
...
test(`do not push state when action fails`, async () => {
    ...
    await getService("action").doAction(8);
    await animationFrame();
    expect(browser.location.href).toBe("http://example.com/sigil/action-8");
    expect(browser.history.length).toBe(2);
    expect(router.current).toEqual({
        action: 8,
        actionStack: [
...
test(`properly push globalState`, async () => {
    ...
    expect(browser.location.href).toBe("http://example.com/sigil/action-4/2");
    expect(router.current).toEqual({
        action: 4,
        actionStack: [
            {
