...
import { queryAllAttributes, queryAllTexts, queryFirst, runAllTimers } from "@sigil/hoot-dom";
import { animationFrame } from "@sigil/hoot-mock";
import { Component, onMounted, xml } from "@sigil/owl";
import {
    contains,
    defineActions,
    defineMenus,
...
function _t() {
    sigil.translationContext = "web";
    const translatedTerm = basic_t(...arguments);
    sigil.translationContext = null;
...
describe(`new urls`, () => {
    test(`action loading`, async () => {
        ...
        expect(browser.location.href).toBe("http://example.com/sigil/action-1001", {
    ...
    test(`action loading, when action not found, load previous`, async () => {
        redirect("/sigil/action-1001/action-666");
        logHistoryInteractions();
        ...
        expect.verifySteps(["pushState http://example.com/sigil/action-1001"]);
    });

    test(`menu loading`, async () => {
        redirect("/sigil?menu_id=2");
        logHistoryInteractions();
        ...
        expect(browser.location.href).toBe("http://example.com/sigil/action-1002", {
            message: "url now points to the default action of the menu",
        });
        expect.verifySteps(["pushState http://example.com/sigil/action-1002"]);
    });

    test(`action and menu loading`, async () => {
        redirect("/sigil/action-1001?menu_id=2");
        logHistoryInteractions();
    ...
    test(`initial loading with action tag`, async () => {
        redirect("/sigil/__test__client__action__");
        logHistoryInteractions();
        stepAllNetworkCalls();
        ...
        await mountWithCleanup(WebClient, { env });
        expect(browser.location.href).toBe("http://example.com/sigil/__test__client__action__", {
            message: "url did not change",
        });
        expect.verifySteps([]);
    });
    ...
    test(`fallback on home action if no action found`, async () => {
        ...
        expect(browser.location.href).toBe("http://example.com/sigil");
    ...
    test(`correctly sends additional context`, async () => {
        ...
        await mountWebClient();
        expect(browser.location.href).toBe("http://example.com/sigil/4/action-1001", {
            message: "url did not change",
        });
        expect.verifySteps([
            "/web/action/load",
    ...
    test(`supports action as xmlId`, async () => {
        redirect("/sigil/action-wowl.client_action");
        logHistoryInteractions();
        ...
        expect(browser.location.href).toBe(
            ...
            "http://example.com/sigil/action-1099",
            { message: "url did not change" }
        );
        expect.verifySteps(["pushState http://example.com/sigil/action-1099"]);
    });
    ...
    test(`supports opening action in dialog`, async () => {
        ...
        expect(`.o_menu_brand`).toHaveCount(0);
        expect(browser.location.href).toBe("http://example.com/sigil/action-wowl.client_action", {
    ...
    test(`should not crash on invalid state`, async () => {
        redirect("/sigil/m-partner?view_type=list");
        logHistoryInteractions();
        stepAllNetworkCalls();
        ...
        expect.verifySteps(["/web/webclient/translations", "/web/webclient/load_menus"]);
        expect(browser.location.href).toBe("http://example.com/sigil/m-partner?view_type=list", {
    ...
    test(`properly load client actions`, async () => {
        ...
        expect(browser.location.href).toBe("http://example.com/sigil/HelloWorldTest", {
            message: "the url did not change",
        });
        expect.verifySteps([
    ...
    test(`properly load client actions with path`, async () => {
        ...
        expect(browser.location.href).toBe("http://example.com/sigil/my-action");
        expect.verifySteps([
            "/web/webclient/translations",
            "/web/webclient/load_menus",
            "pushState http://example.com/sigil/my-action",
        ]);
    });
    ...
    test(`properly load client actions with resId`, async () => {
        ...
        redirect("/sigil/HelloWorldTest/12");
        logHistoryInteractions();
        stepAllNetworkCalls();
        ...
        expect(browser.location.href).toBe("http://example.com/sigil/HelloWorldTest/12", {
            message: "the url did not change",
        });
    ...
    test(`properly load client actions with updateActionState`, async () => {
        ...
        expect.verifySteps([
            "/web/webclient/translations",
            "/web/webclient/load_menus",
            "pushState http://example.com/sigil/HelloWorldTest/12",
        ]);
    });
    ...
    test(`properly load client actions with resId and path (1)`, async () => {
        ...
        expect(browser.location.href).toBe("http://example.com/sigil/my_client/12");
        // Breadcrumb should have only one item, the client action don't have a LazyController (a multi-record view)
        expect(queryAllTexts`.breadcrumb-item, .o_breadcrumb .active`).toEqual([
            "Client Action DisplayName",
        ]);
        expect.verifySteps([
            ...
            "/web/webclient/load_menus",
            "resId:12",
            "pushState http://example.com/sigil/my_client/12",
        ]);
    });
    ...
    test(`properly load client actions with LazyTranslatedString displayName`, async () => {
        ...
        expect(browser.location.href).toBe("http://example.com/sigil/my_client");
        // Breadcrumb should have only one item, the client action don't have a LazyController (a multi-record view)
        expect(queryAllTexts`.breadcrumb-item, .o_breadcrumb .active`).toEqual([
    ...
    test(`properly load act window actions`, async () => {
        ...
        expect(browser.location.href).toBe("http://example.com/sigil/action-1", {
            message: "the url did not change",
        });
        expect.verifySteps([
            "/web/webclient/translations",
    ...
    test(`properly load records`, async () => {
        redirect("/sigil/m-partner/2");
        logHistoryInteractions();
        stepAllNetworkCalls();

        await mountWebClient();
        expect(`.o_form_view`).toHaveCount(1);
        expect(browser.location.href).toBe("http://example.com/sigil/m-partner/2", {
            message: "the url did not change",
        });
    ...
    test(`properly load default record`, async () => {
        redirect("/sigil/action-3/new");
        logHistoryInteractions();
        stepAllNetworkCalls();

        await mountWebClient();
        expect(`.o_form_view`).toHaveCount(1);
        expect(browser.location.href).toBe("http://example.com/sigil/action-3/new", {
            message: "the url did not change",
        });
        expect.verifySteps([
            "/web/webclient/translations",
    ...
    test(`load requested view for act window actions`, async () => {
        redirect("/sigil/action-3?view_type=kanban");
        logHistoryInteractions();
        stepAllNetworkCalls();
        ...
        expect(`.o_kanban_view`).toHaveCount(1);
        expect(browser.location.href).toBe("http://example.com/sigil/action-3?view_type=kanban", {
            message: "the url did not change",
        });
        expect.verifySteps([
            "/web/webclient/translations",
    ...
    test(`lazy load multi record view if mono record one is requested`, async () => {
        ...
        expect(browser.location.href).toBe("http://example.com/sigil/action-3/2", {
            message: "the url did not change",
        });
        expect.verifySteps([
            "/web/webclient/translations",
        ...
        expect(browser.location.href).toBe("http://example.com/sigil/action-3");
        expect.verifySteps(["pushState http://example.com/sigil/action-3"]);
    });
    ...
    test(`go back with breadcrumbs after doAction`, async () => {
        ...
        await animationFrame(); // pushState is debounced
        expect(browser.location.href).toBe("http://example.com/sigil/action-4");
        expect.verifySteps(["pushState http://example.com/sigil/action-4"]);
        expect(queryAllTexts`.breadcrumb-item, .o_breadcrumb .active`).toEqual([
            "Partners Action 4",
        ]);
        ...
        await animationFrame(); // pushState is debounced
        expect(browser.location.href).toBe("http://example.com/sigil/action-4/action-3/2");
        // pushState was called only once
        expect.verifySteps([
            "Update the state without updating URL, nextState: actionStack,action,globalState",
            "pushState http://example.com/sigil/action-4/action-3/2",
        ]);
        ...
        expect.verifySteps([
            ...
            "pushState http://example.com/sigil/action-4",
        ]);
    });
    ...
    test(`lazy loaded multi record view with failing mono record one`, async () => {
        ...
        await animationFrame(); // pushState is debounced
        expect(browser.location.href).toBe("http://example.com/sigil/action-3/action-1");
        expect.verifySteps([
            "Update the state without updating URL, nextState: actionStack,action,globalState",
            "pushState http://example.com/sigil/action-3/action-1",
        ]);
    ...
    test(`should push the correct state at the right time`, async () => {
        ...
        expect(browser.location.href).toBe("http://example.com/sigil/action-3/1");
        // should push the state if it changes afterwards
        expect.verifySteps([
            "Update the state without updating URL, nextState: actionStack,action,globalState",
            "pushState http://example.com/sigil/action-3/1",
        ]);
    });
    ...
    test(`load state supports being given menu_id alone`, async () => {
        ...
        expect.verifySteps([
            ...
            "web_search_read",
            "has_group",
            "pushState http://example.com/sigil/action-1",
        ]);
    });
    ...
    test(`load state: in a form view, no id in initial state`, async () => {
        ...
        expect(browser.location.href).toBe("http://example.com/sigil/action-999/new");
    });
    ...
    test(`load state: in a form view, wrong id in the state`, async () => {
        ...
        redirect("/sigil/action-1000/999");
        logHistoryInteractions();

        await mountWebClient();
        expect(`.o_list_view`).toHaveCount(1);
        expect(`.o_notification_body`).toHaveCount(1, { message: "should have a notification" });
        expect(browser.location.href).toBe("http://example.com/sigil/action-1000", {
            message: "url reflects that we are not on the record",
        });
        expect.verifySteps(["pushState http://example.com/sigil/action-1000"]);
        expect.verifyErrors([
    ...
    test(`server action loading with id`, async () => {
        ...
        await mountWebClient();
        expect(browser.location.href).toBe("http://example.com/sigil/action-2/2", {
            message: "url did not change",
        });
    ...
    test("server action returning act_window", async () => {
        ...
        redirect("/sigil/my-path/2");
        logHistoryInteractions();
        await mountWebClient();
        expect(browser.location.href).toBe("http://example.com/sigil/my-path/2", {
            message: "url did not change",
        });
        expect(router.current).toEqual({
            action: "my-path",
    ...
    test(`state with integer active_ids should not crash`, async () => {
        redirect("/sigil/action-2?active_ids=3");
        logHistoryInteractions();
        ...
        await mountWebClient();
        expect(browser.location.href).toBe("http://example.com/sigil/action-2?active_ids=3", {
            message: "url did not change",
        });
    ...
    test(`initial action crashes`, async () => {
        ...
        redirect("/sigil/__test__client__action__?menu_id=1");
        logHistoryInteractions();

        await mountWebClient();
        expect.verifySteps(["clientAction setup"]);
        expect(browser.location.href).toBe(
            "http://example.com/sigil/__test__client__action__?menu_id=1",
            {
                message: "url did not change",
            }
        );
    ...
    test("all actions crashes", async () => {
        expect.errors(2);
        redirect("/sigil/m-partner/2/m-partner/1");
        logHistoryInteractions();
        stepAllNetworkCalls();
        onRpc("web_read", () => Promise.reject());
    ...
    test(`initial loading with multiple path segments loads the breadcrumbs`, async () => {
        ...
        redirect("/sigil/partners/2/action-28/1");
        logHistoryInteractions();
        stepAllNetworkCalls();
        ...
        expect(browser.location.href).toBe("http://example.com/sigil/partners/2/action-28/1", {
            message: "url did not change",
        });
        expect.verifySteps([
            "/web/action/load_breadcrumbs",
    ...
    test(`don't load controllers when load action new`, async () => {
        ...
        Partner._views["form"] = /* xml */ `
            <form string="Partner">
                <sheet>
                    <a href="http://example.com/sigil/action-5" class="clickMe">clickMe</a>
                    <group>
                        <field name="display_name"/>
                        <field name="foo"/>
                    </group>
        ...
        expect.verifySteps([
            "web_search_read",
            "has_group",
            "pushState http://example.com/sigil/action-3",
        ]);
    });
    ...
    test("properly load previous action when error", async () => {
        ...
        expect.errors(1);
        redirect("/sigil/action-3/1/m-partner");
        logHistoryInteractions();
        stepAllNetworkCalls();
        onRpc("web_read", () => Promise.reject());
    ...
    test("properly reload dynamic actions from sessionStorage (action without id)", async () => {
        ...
        expect(browser.location.href).toBe("http://example.com/sigil/m-partner/1/m-partner");
    ...
    test("properly reload dynamic actions from sessionStorage (action with id)", async () => {
        ...
        expect(browser.location.href).toBe("http://example.com/sigil/action-100/1/action-200");
    ...
    test("properly reload dynamic actions from sessionStorage (form view)", async () => {
        ...
        expect(browser.location.href).toBe("http://example.com/sigil/m-partner/1");
    ...
    test("menu jumping fix: multiple menus sharing same action", async () => {
        ...
        redirect("/sigil/action-9001?menu_id=100");
        logHistoryInteractions();
        ...
        expect.verifySteps([
            ...
            "set current_lang-en",
            "pushState http://example.com/sigil/action-9001",
    ...
    test(`properly reload breadcrumb (state)`, async () => {
        ...
        expect(`.o_form_view`).toHaveCount(1);
        expect(browser.location.href).toBe(
            "http://example.com/sigil/action-200/5/action-300/action-100/1"
        );
        expect(queryAllTexts`.breadcrumb-item, .o_breadcrumb .active`).toEqual([
            "Kanban Partners",
    ...
    test(`reload without lang change`, async () => {
        ...
        expect(router.stateToUrl(router.current)).toBe("/sigil/action-100/1/action-200");
