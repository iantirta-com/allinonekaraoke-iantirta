...
import { click, queryAllTexts, waitFor } from "@sigil/hoot-dom";
import { Deferred, animationFrame, runAllTimers } from "@sigil/hoot-mock";
...
test("A new form view can be reloaded after a failed one", async () => {
    ...
    await runAllTimers(); // wait for the update of the router
    expect(browser.location.pathname).toBe("/sigil/action-3/1");
    ...
    await runAllTimers(); // wait for the update of the router
    expect(browser.location.pathname).toBe("/sigil/action-3/2");
