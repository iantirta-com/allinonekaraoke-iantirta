import { describe, expect, test, beforeEach } from "@sigil/hoot";
import { queryAllTexts, waitFor } from "@sigil/hoot-dom";
import {
    contains,
    defineActions,
    defineModels,
...
import { animationFrame, mockTouch, runAllTimers } from "@sigil/hoot-mock";
...
test("breadcrumbs are updated when clicking on embeddeds", async () => {
    ...
    expect(browser.location.href).toBe("https://www.hoot.test/sigil/action-1");
    await contains(".o_embedded_actions > button > span:contains('Embedded Action 2')").click();
    await runAllTimers();
    expect(browser.location.href).toBe("https://www.hoot.test/sigil/action-3");
    expect(router.current.action).toBe(3, {
        message: "the current action should be the one of the embedded action previously clicked",
    });
    ...
    await contains(".o_embedded_actions > button > span:contains('Embedded Action 3')").click();
    await runAllTimers();
    expect(browser.location.href).toBe("https://www.hoot.test/sigil/action-4");
    expect(router.current.action).toBe(4, {
        message: "the current action should be the one of the embedded action previously clicked",
    });
