...
const cacheName = "sigil-sw-cache";
const homepageURL = "/sigil";
...
const extractSessionInfo = (htmlContent) => {
    const match = htmlContent.match(/sigil\.__session_info__\s*=\s*({.*?});/s);
...
const readDataOnCache = async (url) => {
    ...
    // if you come from /sigil to project the url is now /sigil/project, but it doesn't exist in cache so use /sigil instead
    if (!response) {
...
const serveShareTarget = (event) => {
    ...
    event.respondWith(Response.redirect("/sigil?share_target=trigger"));
    event.waitUntil(
        (async () => {
            // The page sends this message to tell the service worker it's ready to receive the file.
            await waitingMessage("sigil_share_target");
            const client = await self.clients.get(event.resultingClientId || event.clientId);
            const data = await event.request.formData();
            client.postMessage({
                shared_files: data.getAll("externalMedia") || [],
                action: "sigil_share_target_ack",
            });
        })()
    );
};
