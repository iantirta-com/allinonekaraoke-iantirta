...
function SigilEvent(target, name, data) {
    this.target = target;
    this.name = name;
...
SigilEvent.prototype.stopPropagation = function () {
    this.stopped = true;
};
SigilEvent.prototype.is_stopped = function () {
    return this.stopped;
};
...
const EventDispatcherMixin = Object.assign({}, ParentedMixin, {
     ...
     * Sigil where developers may want to replace existing callbacks with theirs.
     *
    ...
    "trigger_up": function (name, info) {
        var event = new SigilEvent(this, name, info);
