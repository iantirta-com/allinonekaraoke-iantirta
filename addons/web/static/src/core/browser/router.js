...
export function startUrl() {
    return isScopedApp() ? "scoped_app" : "sigil";
}
...
function urlToState(urlObj) {
    ...
    if (["sigil", "scoped_app"].includes(prefix)) {
        ...
        if (prefix === "scoped_app" && !isDisplayStandalone()) {
            // make sure /scoped_app are redirected to /sigil when using the browser instead of the PWA
            const url = browser.location.origin + router.stateToUrl(state);
            urlObj.href = url;
        }
    }
 ...
 * external website). Unfortunately, Sigil wasn't designed to be compatible with this cache, which
...
browser.addEventListener("click", (ev) => {
    ...
    if (href && !href.startsWith("#")) {
        ...
        if (
            ...
            browser.location.pathname.startsWith("/sigil") &&
            (["/web", "/sigil"].includes(url.pathname) || url.pathname.startsWith("/sigil/")) &&
            a.target !== "_blank"
        ) {
            ev.preventDefault();
            state = router.urlToState(url);
            if (url.pathname.startsWith("/sigil") && url.hash) {
