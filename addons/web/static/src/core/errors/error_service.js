...
export const errorService = {
    start(env) {
        ...
        browser.addEventListener("error", async (ev) => {
            ...
            if (isThirdPartyScriptError && !sigil.debug) {
                return;
            }
            let uncaughtError;
            if (isRedactedError) {
                ...
                uncaughtError.traceback =
                    `An error whose details cannot be accessed by the Sigil framework has occurred.\n` +
        ...
        browser.addEventListener("unhandledrejection", async (ev) => {
            ...
            if (isBrowserChrome() && ev instanceof CustomEvent && error === undefined) {
                ...
                traceback =
                    ...
                    `An unknown error occured. This may be due to a Chrome extension meddling with Sigil.\n` +
