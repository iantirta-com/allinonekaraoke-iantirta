...
import { Component, useRef, useState, markup } from "@sigil/owl";
...
export const sigilExceptionTitleMap = new Map(
    Object.entries({
        "sigil.addons.base.models.ir_mail_server.MailDeliveryException": _t("MailDeliveryException"),
        "sigil.exceptions.AccessDenied": _t("Access Denied"),
        "sigil.exceptions.MissingError": _t("Missing Record"),
        "sigil.addons.web.controllers.action.MissingActionError": _t("Missing Action"),
        "sigil.addons.base.models.ir_actions.ServerActionWithWarningsError": _t("Invalid Operation"),
        "sigil.exceptions.UserError": _t("Invalid Operation"),
        "sigil.exceptions.ValidationError": _t("Validation Error"),
        "sigil.exceptions.AccessError": _t("Access Error"),
        "sigil.exceptions.Warning": _t("Warning"),
    })
);
...
ClientErrorDialog.title = _t("Sigil Client Error");

// -----------------------------------------------------------------------------
// Network Error Dialog
// -----------------------------------------------------------------------------
export class NetworkErrorDialog extends ErrorDialog {}
NetworkErrorDialog.title = _t("Sigil Network Error");
...
export class RPCErrorDialog extends ErrorDialog {
    ...
    inferTitle() {
        ...
        if (this.props.exceptionName && sigilExceptionTitleMap.has(this.props.exceptionName)) {
            this.title = sigilExceptionTitleMap.get(this.props.exceptionName).toString();
            return;
        }
        ...
        switch (this.props.type) {
            case "server":
                this.title = _t("Sigil Server Error");
                break;
            case "script":
                this.title = _t("Sigil Client Error");
                break;
            case "network":
                this.title = _t("Sigil Network Error");
                break;
        }
    }
...
export class WarningDialog extends Component {
    ...
    inferTitle() {
        if (this.props.exceptionName && sigilExceptionTitleMap.has(this.props.exceptionName)) {
            return sigilExceptionTitleMap.get(this.props.exceptionName).toString();
        }
        return this.props.title || _t("Sigil Warning");
    }
}
...
export class RedirectWarningDialog extends Component {
    ...
    setup() {
        ...
        this.title = capitalize(subType) || _t("Sigil Warning");
        this.message = message;
...
registry
    ...
    .add("sigil.exceptions.AccessDenied", WarningDialog)
    .add("sigil.exceptions.AccessError", WarningDialog)
    .add("sigil.exceptions.MissingError", WarningDialog)
    .add("sigil.addons.web.controllers.action.MissingActionError", WarningDialog)
    .add("sigil.addons.base.models.ir_actions.ServerActionWithWarningsError", WarningDialog)
    .add("sigil.exceptions.UserError", WarningDialog)
    .add("sigil.exceptions.ValidationError", WarningDialog)
    .add("sigil.exceptions.RedirectWarning", RedirectWarningDialog)
    .add("sigil.http.SessionExpiredException", SessionExpiredDialog)
