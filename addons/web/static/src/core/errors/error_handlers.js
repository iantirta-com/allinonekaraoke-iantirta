...
/**
 * @typedef {import("../../env").SigilEnv} SigilEnv
...
export function swallowAllVisitorErrors(env, error, originalError) {
    if (!user.isInternalUser && !sigil.debug && !session.test_mode) {
        return true;
    }
}
...
if (user.isInternalUser === undefined) {
    ...
    // apparently not be present in all Sigil screens at the moment... TODO ?
