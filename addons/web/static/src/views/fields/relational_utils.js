...
import { highlightText, sigilmark } from "@web/core/utils/html";
...
export class Many2XAutocomplete extends Component {
    ...
    onQuickCreateError(error, request) {
        if (
            ...
            error.exceptionName === "sigil.exceptions.ValidationError"
        ) {
    ...
    buildRecordSuggestion(request, record) {
        ...
        return {
            ...
            label: label
                ? highlightText(request, sigilmark(label), "text-primary fw-bold")
                : _t("Unnamed"),
