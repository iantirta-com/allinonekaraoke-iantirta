...
from sigil.tests.common import BaseCase, HttpCase, tagged
...
class TestDatabaseOperations(BaseCase):
    def setUp(self):
        ...
        self.verify_admin_password_patcher = patch(
            'sigil.tools.config.verify_admin_password', self.password.__eq__,
        )
    ...
    def list_dbs_filtered(self):
        return set(db for db in sigil.service.db.list_dbs(True) if re.match(config['dbfilter'], db))
    ...
    def test_database_creation(self):
        ...
        self.assertTrue(sigil.tools.config.verify_admin_password(self.password))
        ...
        self.assertIn('/sigil', res.headers['Location'])
    ...
    def test_database_restore(self):
        ...
        with self.subTest(DEFAULT_MAX_CONTENT_LENGTH=None), \
             patch.object(sigil.http, 'DEFAULT_MAX_CONTENT_LENGTH', None):
            backup_file.seek(0)
            self.session.post(
        ...
        with self.subTest(DEFAULT_MAX_CONTENT_LENGTH=1024), \
             patch.object(sigil.http, 'DEFAULT_MAX_CONTENT_LENGTH', 1024):
            backup_file.seek(0)
            self.session.post(
                self.url('/web/database/restore'),
                data={
    ...
    def test_database_http_registries(self):
        ...
        with patch('sigil.sql_db.close_db') as close_db:
            res = self.url_open_drop(test_db_name)
        close_db.assert_called_once_with(test_db_name)

        # simulate that some customers were connected to that dropped db
        session_store = sigil.http.root.session_store
        session = session_store.new()
        session.update(sigil.http.get_default_session(), db=test_db_name)
        ...
        with self.subTest(msg="Registry.init() fails"):
            ...
            with self.assertLogs('sigil.sql_db', logging.INFO) as capture:
                res = self.session.get(self.url('/web/health'))
            self.assertEqual(res.status_code, 200)
            self.assertEqual(session_store.get(session.sid)['db'], None)
            self.assertEqual(capture.output, [
                "INFO:sigil.sql_db:Connection to the database failed",
            ])
        ...
        with self.subTest(msg="Registry.check_signaling() fails"):
            session_store.save(session)
            registries[test_db_name] = registry
            with self.assertLogs('sigil.sql_db', logging.ERROR) as capture, \
                 patch.object(Registry, '__new__', return_value=registry), \
            ...
            self.assertRegex(capture.output[0], (
                r"^ERROR:sigil\.sql_db:bad query:(?s:.*?)"
