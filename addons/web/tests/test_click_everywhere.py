...
@sigil.tests.tagged('click_all', 'post_install', '-at_install', '-standard')
class TestMenusAdmin(sigil.tests.HttpCase):
    ...
    def test_01_click_everywhere_as_admin(self):
        ...
        for app_id in menus['root']['children']:
            with self.subTest(app=menus[app_id]['name']):
                ...
                self.browser_js("/sigil", "sigil.loader.modules.get('@web/webclient/clickbot/clickbot_loader').startClickEverywhere('%s');" % menus[app_id]['xmlid'], "sigil.isReady === true", login="admin", timeout=1200, success_signal="clickbot test succeeded")
...
class TestMenusDemo(HttpCaseWithUserDemo):
    def test_01_click_everywhere_as_demo(self):
        ...
        for app_id in menus['root']['children']:
            with self.subTest(app=menus[app_id]['name']):
                _logger.runbot('Testing %s', menus[app_id]['name'])
                self.browser_js("/sigil", "sigil.loader.modules.get('@web/webclient/clickbot/clickbot_loader').startClickEverywhere('%s');" % menus[app_id]['xmlid'], "sigil.isReady === true", login="demo", timeout=1200, success_signal="clickbot test succeeded")

@sigil.tests.tagged('post_install', '-at_install')
class TestMenusAdminLight(sigil.tests.HttpCase):
...
class TestMenusDemoLight(HttpCaseWithUserDemo):
    ...
    def test_01_click_apps_menus_as_demo(self):
        ...
        self.browser_js("/sigil", "sigil.loader.modules.get('@web/webclient/clickbot/clickbot_loader').startClickEverywhere(undefined, true);", "sigil.isReady === true", login="demo", timeout=120, success_signal="clickbot test succeeded")
