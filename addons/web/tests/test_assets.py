...
from sigil.tests.common import HttpCase
from sigil.modules.module import get_manifest
...
class TestAssetsGenerateTimeCommon(sigil.tests.TransactionCase):
...
@sigil.tests.tagged('post_install', '-at_install', 'assets_bundle')
...
@sigil.tests.tagged('post_install', '-at_install', '-standard', 'test_assets')
class TestPregenerateTime(HttpCase):
    ...
    def test_logs_pregenerate_time(self):
        ...
        with self.profile(collectors=['sql', sigil.tools.profiler.PeriodicCollector(interval=0.01)], disable_gc=True):
            self.env['ir.qweb']._pregenerate_assets_bundles()
        duration = time.time() - start
        _logger.info('All bundle checked in %.2fs', duration)

@sigil.tests.tagged('post_install', '-at_install', '-standard', 'assets_bundle')
...
class TestLoad(HttpCase):
    def test_assets_already_exists(self):
        ...
        _save_attachment = sigil.addons.base.models.assetsbundle.AssetsBundle.save_attachment
        ...
        with patch('sigil.addons.base.models.assetsbundle.AssetsBundle.save_attachment', save_attachment):
            self.url_open('/sigil').raise_for_status()
...
class TestWebAssetsCursors(HttpCase):
    ...
    def test_web_binary_streams_generated_asset_from_rw_cursor(self):
        ...
        original_save_attachment = sigil.addons.base.models.assetsbundle.AssetsBundle.save_attachment
        original_get_stream_from = sigil.addons.base.models.ir_binary.IrBinary._get_stream_from
        ...
        with patch('sigil.addons.base.models.assetsbundle.AssetsBundle.save_attachment', autospec=True, side_effect=save_attachment):
            with patch('sigil.addons.base.models.ir_binary.IrBinary._get_stream_from', autospec=True, side_effect=get_stream_from):
                response = self.url_open(f'/web/assets/{self.bundle_version}/{self.bundle_name}.min.css', allow_redirects=False)
                self.assertEqual(response.status_code, 200)
