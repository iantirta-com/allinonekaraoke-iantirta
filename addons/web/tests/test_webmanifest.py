...
from sigil.tests.common import tagged
...
class WebManifestRoutesTest(HttpCaseWithUserDemo):
    ...
    def test_webmanifest(self):
        ...
        data = response.json()
        self.assertEqual(data["name"], "Sigil")
        self.assertEqual(data["scope"], "/sigil")
        self.assertEqual(data["start_url"], "/sigil")
        ...
        self.assertCountEqual(data["icons"], [
            {'src': '/web/static/img/sigil-icon-192x192.png', 'sizes': '192x192', 'type': 'image/png'},
            {'src': '/web/static/img/sigil-icon-512x512.png', 'sizes': '512x512', 'type': 'image/png'}
        ])
        ...
        for shortcut in data["shortcuts"]:
            ...
            self.assertTrue(shortcut["url"].startswith("/sigil?menu_id="))
    ...
    def test_webmanifest_scoped(self):
        ...
        self.assertEqual(data["prefer_related_applications"], False)
        self.assertCountEqual(data["icons"], [
            {'src': "/web/static/img/sigil-icon-192x192.png", 'sizes': 'any', 'type': 'image/png'}
        ])
        self.assertEqual(len(data["shortcuts"]), 0)
    ...
    def test_serviceworker(self):
        ...
        self.assertEqual(response.headers["Service-Worker-Allowed"], "/sigil")

    def test_offline_url(self):
        """
        This route returns the offline page
        """
        response = self.url_open("/sigil/offline")
        self.assertEqual(response.status_code, 200)
    ...
    def test_apple_touch_icon(self):
        ...
        response = self.url_open("/web/static/img/sigil-icon-ios.png")
        self.assertEqual(response.status_code, 200)

        document = self.url_open("/sigil")
        self.assertIn(
            '<link rel="apple-touch-icon" href="/web/static/img/sigil-icon-ios.png"/>', document.text,
