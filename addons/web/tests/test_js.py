...
class QunitCommon(sigil.tests.HttpCase):
...
@sigil.tests.tagged('post_install', '-at_install')
class HOOTCommon(sigil.tests.HttpCase):
...
class WebSuite(QunitCommon, HOOTCommon):
    ...
    def _check_forbidden_statements(self, bundle):
        ...
        self.env.ref('web.layout').write({'arch_db': '<t t-name="web.layout"><html><head><meta charset="utf-8"/><link/><script id="web.layout.sigilscript"/><meta/><t t-esc="head"/></head><body><t t-out="0"/></body></html></t>'})
