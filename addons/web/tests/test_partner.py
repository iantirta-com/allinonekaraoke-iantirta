...
from sigil.addons.base.tests.common import TransactionCaseWithUserPortal
from sigil.exceptions import AccessError
from sigil.fields import Command
from sigil.tests.common import HttpCase, tagged
...
class TestPartnerVCard(HttpCase):
    ...
    def test_check_partner_access_for_user(self):
        ...
        with mute_logger('sigil.http'):  # mute 403 warning
            res = self.url_open('/web/partner/vcard?partner_ids=%s,%s' %
                            (self.partners[0].id, self.partners[1].id))
