...
from sigil import api, models, fields, tools
from sigil.tools.misc import OrderedSet
from sigil.addons.mail.tools.link_preview import get_link_preview_from_url
...
class MailLinkPreview(models.Model):
    ...
    def _create_from_message_and_notify(self, message, request_url=None):
        ...
        if not tools.is_html_empty(message.body):
            ...
            if request_url:
                ignore_pattern = re.compile(f"{re.escape(request_url)}(sigil|web|chat)(/|$|#|\\?)")
