...
from sigil.tools import exception_to_unicode
...
class SigilIMAP4_SSL(SigilIMAP4, IMAP4_SSL):
    pass


class SigilPOP3(POP3):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._unread_messages = None
...
class SigilPOP3_SSL(SigilPOP3, POP3_SSL):
    pass
...
class FetchmailServer(models.Model):
    ...
    script = fields.Char(readonly=True, default='/mail/static/scripts/sigil-mailgate.py')
...
sigil-mailgate.py --host=HOSTNAME --port=PORT -u %(uid)d -p PASSWORD -d %(dbname)s
Example configuration for the postfix mta running locally:
/etc/postfix/virtual_aliases: @youdomain sigil_mailgate@localhost
/etc/aliases:
sigil_mailgate: "|/path/to/sigil-mailgate.py --host=localhost -u %(uid)d -p PASSWORD -d %(dbname)s"
        """ % conf
    ...
    def _connect__(self, allow_archived=False):  # noqa: PLW3201
        ...
        if connection_type == 'imap':
            ...
            connection = SigilIMAP4_SSL(server, port, timeout=MAIL_TIMEOUT) if is_ssl else SigilIMAP4(server, port, timeout=MAIL_TIMEOUT)
            self._imap_login__(connection)
        elif connection_type == 'pop':
            server, port, is_ssl = self.server, int(self.port), self.is_ssl
            connection = SigilPOP3_SSL(server, port, timeout=MAIL_TIMEOUT) if is_ssl else SigilPOP3(server, port, timeout=MAIL_TIMEOUT)
