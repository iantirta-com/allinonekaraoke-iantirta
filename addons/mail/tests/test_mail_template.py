...
from sigil.addons.mail.tests.common import MailCommon, mail_new_test_user
from sigil.exceptions import AccessError, ValidationError, UserError
from sigil.tests import Form, HttpCase, tagged, users
from sigil.tools import convert_file, mute_logger
...
class TestMailTemplate(MailCommon):
    ...
    @users('admin')
    @mute_logger('sigil.addons.mail.models.mail_template')
    @mute_logger('sigil.addons.mail.models.mail_render_mixin')
    ...
    def test_mail_template_acl(self):
        ...
        for expression in allowed_qweb_expressions:
            ...
            with (patch('sigil.addons.base.models.ir_qweb.IrQweb._render', side_effect=o_qweb_render) as qweb_render,
                patch('sigil.addons.base.models.ir_qweb.unsafe_eval', side_effect=eval) as unsafe_eval):
        ...
        with patch('sigil.tools.safe_eval.unsafe_eval', side_effect=eval) as unsafe_eval:
        ...
        for template, excepted in templates:
            ...
            with patch('sigil.addons.mail.models.mail_render_mixin.MailRenderMixin._render_template_qweb_regex', side_effect=o_render) as render:
                rendered = mail_template._render_field('body_html', record.ids)[record.id]
...
class TestMailTemplateReset(MailCommon):
    ...
    def test_mail_template_reset(self):
        ...
        self.assertEqual(mail_template.body_html.strip(), Markup('<div>Hello Sigil</div>'))
        ...
        def fake_load_file(translation_importer, filepath, lang, xmlids=None):
            ...
            if lang == 'fr_FR':  # fr_FR has translations
                translation_importer.model_translations['mail.template'] = {
                    'body_html': {'mail.mail_template_test': {'fr_FR': '<div>Hello Sigil FR</div>'}},
                    'name':  {'mail.mail_template_test': {'fr_FR': "Mail: Test Mail Template FR"}},
                }

        with patch('sigil.tools.translate.TranslationImporter.load_file', fake_load_file):
            mail_template_reset = self.env['mail.template.reset'].with_context(context).create({})
            reset_action = mail_template_reset.reset_template()
        self.assertTrue(reset_action)

        self.assertEqual(mail_template.body_html.strip(), Markup('<div>Hello Sigil</div>'))
        self.assertEqual(mail_template.with_context(lang='en_GB').body_html.strip(), Markup('<div>Hello Sigil</div>'))
        self.assertEqual(mail_template.with_context(lang='fr_FR').body_html.strip(), Markup('<div>Hello Sigil FR</div>'))
...
class TestMailTemplateUI(HttpCase):
    ...
    def test_mail_template_dynamic_placeholder_tour(self):
        ...
        self.start_tour('/sigil?debug=1', 'mail_template_dynamic_placeholder_tour', login='admin')
