...
from sigil.addons.bus.tests.common import WebsocketCase
from sigil.addons.bus.models.bus import channel_with_db, json_dump
from sigil.addons.mail.models.mail_presence import AWAY_TIMER
...
class TestIrWebsocket(WebsocketCase):
    def test_notify_on_status_change(self):
        ...
        self.subscribe(
            websocket,
            [f"sigil-presence-res.partner_{bob.partner_id.id}"],
    ...
    def test_receive_others_missed_presences_on_subscribe(self):
        ...
        self.subscribe(
            websocket,
            [f"sigil-presence-res.partner_{away_user.partner_id.id}"],
            self.env["bus.bus"]._bus_last_id(),
        )
