...
from sigil.tests.common import JsonRpcException, new_test_user, tagged
...
class TestTranslationController(HttpCaseWithUserDemo):
    ...
    def test_access_right(self):
        with self.assertRaises(JsonRpcException, msg="sigil.http.SessionExpiredException"):
        ...
        with self.assertRaises(JsonRpcException, msg="sigil.exceptions.AccessError"), mute_logger("sigil.http"):
            self._mock_translation_request({"message_id": self.message.id})
