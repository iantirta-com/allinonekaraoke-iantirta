...
from sigil import tools, fields
from sigil.addons.base.models.ir_mail_server import IrMail_Server
from sigil.addons.base.models.ir_mail_server import MailDeliveryException
from sigil.addons.base.tests.common import MockSmtplibCase
from sigil.addons.bus.models.bus import BusBus, json_dump
from sigil.addons.bus.tests.common import BusCase
from sigil.addons.mail.models import mail_thread
from sigil.addons.mail.models.mail_mail import MailMail
from sigil.addons.mail.models.mail_message import MailMessage
from sigil.addons.mail.models.mail_notification import MailNotification
from sigil.addons.mail.models.res_users import ResUsers
from sigil.addons.mail.tools.discuss import Store
from sigil.tests import common, RecordCapturer, new_test_user
from sigil.tools import mute_logger
from sigil.tools.mail import (
    email_normalize, email_normalize_all, email_split, email_split_and_format_normalize, formataddr
)
from sigil.tools.translate import code_translations
...
class MockEmail(common.BaseCase, MockSmtplibCase):
    ...
    def mock_push_to_end_point(self, max_direct_push=5):
        with patch.object(mail_thread, 'push_to_end_point') as patched_push, \
             patch('sigil.addons.mail.models.mail_thread.MAX_DIRECT_PUSH', max_direct_push):
    ...
    def assertNotSentEmail(self, recipients=None, message_id=None):
        ...
        :param message_id:
            ...
            from the a specific message in sigil.
        """
        mails = self._mails
        if message_id:
...
class MailCommon(MailCase):
    ...
    def _activate_multi_lang(cls, lang_code='es_ES', layout_arch_db=None, test_record=False, test_template=False):
        ...
        with mute_logger("sigil.addons.base.models.ir_module", "sigil.tools.translate"):
...
<body>
    ...
    def _setup_push_devices_for_partners(cls, partners, endpoint=None):
        ...
        endpoint = endpoint or "https://test.iantirta.com/webpush/user"
...
def freeze_all_time(dt=None):
    ...
    with patch('sigil.sql_db.BaseCursor.now', return_value=dt), freeze_time(dt):
        yield
