...
from sigil.http import STATIC_CACHE_LONG
from sigil import Command, fields


@sigil.tests.tagged("-at_install", "post_install", "mail_controller")
class TestMessageController(HttpCaseWithUserDemo):
    ...
    @mute_logger("sigil.addons.http_routing.models.ir_http", "sigil.http")
...
class TestMessageLinks(MailCommon, HttpCase):
    ...
    def test_message_link_by_employee(self):
        ...
        with self.subTest(channel_message=channel_message):
            expected_url = self.base_url() + f'/sigil/action-mail.action_discuss?active_id={channel_message.res_id}&highlight_message_id={channel_message.id}'
