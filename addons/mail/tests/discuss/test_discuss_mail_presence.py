...
from sigil.tests import tagged, new_test_user
from sigil.addons.bus.tests.common import WebsocketCase
from sigil.addons.mail.tests.common import MailCommon, freeze_all_time
...
class TestMailPresence(WebsocketCase, MailCommon):
    def _receive_presence(self, requested_by, target, has_token=False):
        ...
        channel_parts = ["sigil-presence", f"{target_channel._name}_{target_channel.id}"]
        if has_token:
