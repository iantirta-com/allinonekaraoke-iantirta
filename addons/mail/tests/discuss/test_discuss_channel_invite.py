...
from sigil.tests import HttpCase, new_test_user, tagged, users
from sigil.tools.misc import hash_sign
...
class TestDiscussChannelInvite(HttpCase, MailCommon):
    def test_01_invite_by_email_flow(self):
        ...
        with self.mock_mail_gateway():
            self.start_tour(
                f"/sigil/discuss?active_id={group_chat.id}", "discuss.invite_by_email", login="bob"
            )
