...
class TestMailComposerForm(TestMailComposer):
    ...
    @mute_logger('sigil.addons.base.models.ir_rule', 'sigil.addons.mail.models.mail_mail')
    @users('employee')
...
class TestMailComposerUI(MailCommon, HttpCase):
    ...
    def test_mail_composer_test_tour(self):
        ...
        with self.mock_mail_app():
            self.start_tour(
                f"/sigil/res.partner/{partner.id}",
    ...
    def test_send_attachment_without_body(self):
        self.start_tour("/sigil/discuss", "create_thread_for_attachment_without_body",login="admin")
