...
import { queryFirst } from "@sigil/hoot-dom";
import { disableAnimations } from "@sigil/hoot-mock";
...
test("reply to logged note in chatter keeps prefilled mention in html composer", async () => {
    ...
    await click(".o-mail-Message:contains('Test message from B') [title='Reply']");
    await contains("button.active:text('Log note')");
    await contains(".o-mail-Composer.o-focused .o-mail-Composer-html.sigil-editor-editable");
    await contains(
        ".o-mail-Composer-html.sigil-editor-editable a.o_mail_redirect:text('@Partner B')"
    );
    const editor = {
        document,
        editable: queryFirst(".o-mail-Composer-html.sigil-editor-editable"),
    };
...
test("Click reply to note again preserves composer content", async () => {
    ...
    await contains(
        ".o-mail-Composer-html.sigil-editor-editable:text('@Batman Strong Text'):has(a.o_mail_redirect:text('@Batman')):has(strong:text('Strong Text'))"
    );
