...
import { press, queryFirst, queryOne, queryValue } from "@sigil/hoot-dom";
import { Deferred, mockDate, tick } from "@sigil/hoot-mock";
import {
    asyncStep,
    Command,
    getService,
...
test("Post a message containing an email address followed by a mention on another line", async () => {
    ...
    await start();
    await openDiscuss(channelId);
    await insertText(".o-mail-Composer-input", "email@iantirta.com\n@Te");
    await click(".o-mail-Composer-suggestion");
    await contains(".o-mail-Composer-input", { value: "email@iantirta.com\n@TestPartner " });
    await press("Enter");
    await contains(
        `.o-mail-Message-body .o_mail_redirect[data-oe-id="${partnerId}"][data-oe-model="res.partner"]`,
        { text: "@TestPartner" }
...
test("Opening thread with needaction messages should mark all messages of thread as read", async () => {
    ...
    await triggerEvents(".o-mail-Composer-html.sigil-editor-editable", ["blur", "focusout"]);
    await click("button", { text: "Inbox" });
    await contains("h4", { text: "Congratulations, your inbox is empty" });
    const messageId = pyEnv["mail.message"].create({
        author_id: partnerId,
...
test("Transient messages are added at the end of the thread", async () => {
    ...
    await contains(":nth-child(2 of .o-mail-Message)", { text: "SigilBot" });
});
