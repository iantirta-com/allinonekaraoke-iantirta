...
import { advanceTime, mockDate, Deferred } from "@sigil/hoot-mock";
import { manuallyDispatchProgrammaticEvent, queryAll } from "@sigil/hoot-dom";
...
test("Open avatar card when clicking on partner mention", async () => {
    ...
    const partnerId = pyEnv.user.partner_id;
    pyEnv["mail.scheduled.message"].create({
        body: `<a href="${getOrigin()}/sigil#model=res.partner&id=${partnerId}" class="o_mail_redirect" data-oe-model="res.partner" data-oe-id="${partnerId}">@Mitchell Admin</a>`,
        model: "res.partner",
        res_id: partnerId,
        scheduled_date: "2024-10-20 11:00:00",
    });
...
test("Open chat when clicking on channel mention", async () => {
    ...
    pyEnv["mail.scheduled.message"].create({
        body: `<a href="${getOrigin()}/sigil#model=discuss?channel&id=${channelId}" class="o_channel_redirect" data-oe-model="discuss.channel" data-oe-id="${channelId}">#my-channel</a>`,
        model: "res.partner",
        res_id: partnerId,
        scheduled_date: "2024-10-20 11:00:00",
    });
