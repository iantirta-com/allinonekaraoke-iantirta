...
test("media dialog: upload", async function () {
    ...
    const anchorNode = queryOne(".sigil-editor-editable div.o-paragraph");
...
test("mention a partner", async () => {
    ...
    const editable = queryOne(`.sigil-editor-editable`);
    ...
    expect(getContent(editable)).toBe(
        `<p>\uFEFF<a href="/sigil/res.partner/${serverState.partnerId}" class="o_mail_redirect" data-oe-id="${serverState.partnerId}" data-oe-model="res.partner" target="_blank" contenteditable="false">@Mitchell Admin</a>\uFEFF[]</p>`
    );
});
...
test("mention a channel", async () => {
    ...
    const anchorNode = queryOne(`[name='body'] .sigil-editor-editable div.o-paragraph`);
    setSelection({ anchorNode, anchorOffset: 0 });
    await insertText(htmlEditor, "#");
    await animationFrame();
...
describe("Remove attachments", () => {
    ...
    test("should remove file from html editor if removed from attachment list", async () => {
        ...
        await waitFor(".sigil-editor-editable");
        const anchorNode = queryOne(".sigil-editor-editable div.o-paragraph");
        setSelection({ anchorNode, anchorOffset: 0 });
        ...
        await waitFor(".sigil-editor-editable .o_file_box:has(a:contains('file.txt'))");
        await click("[name='attachment_ids'] button:has(i.fa-times)");
        await waitForNone("[name='attachment_ids'] a:contains('file.txt')");
        await waitForNone(".sigil-editor-editable .o_file_box:has(a:contains('file.txt'))");
    });
    ...
    test("should remove image from html editor if removed from attachment list", async () => {
        ...
        await waitFor(".sigil-editor-editable img[data-attachment-id='1']");
        await click("[name='attachment_ids'] button:has(i.fa-times)");
        await waitForNone("[name='attachment_ids'] a:contains('test.jpg')");
        await waitForNone(".sigil-editor-editable img[data-attachment-id='1']");
    });
});
