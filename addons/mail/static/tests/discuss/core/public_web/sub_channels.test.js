...
import { Deferred, mockDate, animationFrame } from "@sigil/hoot-mock";
...
test("mention suggestions in thread match channel restrictions", async () => {
    ...
    const [partnerId_1, partnerId_2] = pyEnv["res.partner"].create([
        { email: "p1@iantirta.com", name: "p1" },
        { email: "p2@iantirta.com", name: "p2" },
    ]);
...
test("muted channel hides sub-thread unless channel is selected or thread has unread messages", async () => {
    const pyEnv = await startServer();
    const channelId = pyEnv["discuss.channel"].create({ name: "General" });
    const partnerId2 = pyEnv["res.partner"].create({ email: "p1@iantirta.com", name: "p1" });
