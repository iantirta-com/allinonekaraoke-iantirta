...
import { describe, test } from "@sigil/hoot";
import {
    asyncStep,
    Command,
    onRpc,
...
test("Auto-open SigilBot chat when opening discuss for the first time", async () => {
    // Sigilbot chat has onboarding for using Discuss app.
    // We assume pinned sigilbot chat without any message seen means user just started using Discuss app.
    const pyEnv = await startServer();
    ...
    await start();
    await openDiscuss();
    await contains(".o-mail-DiscussContent-threadName", { value: "SigilBot" });
});
