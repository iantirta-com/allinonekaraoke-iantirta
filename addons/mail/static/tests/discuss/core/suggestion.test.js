...
import { beforeEach, describe, test } from "@sigil/hoot";
...
test("use a command for a specific channel type", async () => {
    ...
    await htmlInsertText(editor, "/");
    await click(".o-mail-Composer-suggestion strong", { text: "who" });
    await contains(".o-mail-Composer-html.sigil-editor-editable", { text: "/who" });
});
...
test("mention suggestion displays SigilBot before archived partners", async () => {
    const pyEnv = await startServer();
