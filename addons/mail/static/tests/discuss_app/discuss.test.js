...
import { animationFrame, Deferred, press, runAllTimers, tick, waitFor } from "@sigil/hoot-dom";
import { mockDate } from "@sigil/hoot-mock";
import {
    asyncStep,
    Command,
    getService,
...
test("Posting message should transform links.", async () => {
    ...
    await start();
    await openDiscuss(channelId);
    await insertText(".o-mail-Composer-input", "test https://www.iantirta.com/");
    await press("Enter");
    await contains("a[href='https://www.iantirta.com/']");
});
...
test("no out-of-focus notification on receiving self messages in chat", async () => {
    ...
    // simulate receiving a new message of self with sigil out-of-focused
...
test("out-of-focus notif on needaction message in channel", async () => {
    ...
    await contains(".o_menu_systray i[aria-label='Messages']");
    await contains(".o-mail-ChatWindow", { count: 0 });
    // simulate receiving a new needaction message with sigil out-of-focused
...
test("receive new chat message: out of sigil focus (notification, chat)", async () => {
    const pyEnv = await startServer();
    const partnerId = pyEnv["res.partner"].create({ name: "Dumbledore" });
    const userId = pyEnv["res.users"].create({ partner_id: partnerId });
    const channelId = pyEnv["discuss.channel"].create({
    ...
    await contains(".o_menu_systray i[aria-label='Messages']");
    await contains(".o-mail-ChatWindow", { count: 0 });
    // simulate receiving a new message with sigil out-of-focused
    withUser(userId, () =>
        rpc("/mail/message/post", {
            post_data: {
                body: "New message",
...
test("receive new chat messages: out of sigil focus (tab title)", async () => {
    let stepCount = 0;
    const pyEnv = await startServer();
    ...
    // simulate receiving a new message in chat 1 with sigil out-of-focused
    ...
    // simulate receiving a new message in chat 2 with sigil out-of-focused
    await withUser(bobUserId, () =>
        rpc("/mail/message/post", {
            post_data: { body: "Hello world!", message_type: "comment" },
    ...
    await waitForSteps(["set_counters:discuss"]);
    // simulate receiving another new message in chat 2 with sigil focused
    await withUser(bobUserId, () =>
        rpc("/mail/message/post", {
            post_data: { body: "Hello world!", message_type: "comment" },
            thread_id: channelId_2,
...
test("Partner IM status is displayed as thread icon in top bar of channels of type 'chat'", async () => {
    ...
    pyEnv["discuss.channel"].create([
        ...
        },
        {
            channel_member_ids: [
                Command.create({ partner_id: serverState.partnerId }),
                Command.create({ partner_id: serverState.sigilbotId }),
            ],
            channel_type: "chat",
        },
    ]);
    ...
    await click(".o-mail-DiscussSidebarChannel", { text: "SigilBot" });
...
test("Chatter notification in messaging menu should open the form view even when discuss app is open", async () => {
    ...
    const messageId = pyEnv["mail.message"].create({
        ...
        author_id: serverState.sigilbotId,
        needaction: true,
        res_id: partnerId,
    });
    pyEnv["mail.notification"].create({
