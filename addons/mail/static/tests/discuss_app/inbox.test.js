...
test("reply: discard on pressing escape", async () => {
    const pyEnv = await startServer();
    const partnerId = pyEnv["res.partner"].create({
        email: "testpartnert@iantirta.com",
        name: "TestPartner",
    });
    const messageId = pyEnv["mail.message"].create({
        body: "not empty",
...
test("error notifications should not be shown in Inbox", async () => {
    ...
    await contains(`.o-mail-Message-header a[href*='/sigil/res.partner/${partnerId}']`, {
        text: "Demo User",
    });
...
test("Counter should be incremented by 1 when receiving a message with a mention in a channel", async () => {
    ...
    withUser(userId, () =>
        rpc("/mail/message/post", {
            post_data: {
                body: `<a href="https://www.hoot.test/sigil/res.partner/17" class="o_mail_redirect" data-oe-id="${mention[0]}" data-oe-model="res.partner" target="_blank" contenteditable="false">@${mentionName}</a> mention`,
                message_type: "comment",
                partner_ids: mention,
                subtype_xmlid: "mail.mt_comment",
            },
