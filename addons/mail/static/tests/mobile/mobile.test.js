...
import { advanceTime, pointerDown, press } from "@sigil/hoot-dom";
import { Deferred, mockTouch, mockUserAgent } from "@sigil/hoot-mock";
...
test("click on an sigil link should fold the chat window (mobile)", async () => {
    const pyEnv = await startServer();
    const channelId = pyEnv["discuss.channel"].create({});
    patchUiSize({ size: SIZES.SM });
    await start();
    await openDiscuss(channelId);
    await insertText(".o-mail-Composer-input", `http://${browser.location.host}/iantirta.com`);
    await click(".o-mail-Composer button[title='Send']");
    await contains(".o-mail-ChatWindow");
    await click(`a[href="http://${browser.location.host}/iantirta.com"]`);
