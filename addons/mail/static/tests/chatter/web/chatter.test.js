...
import { Deferred, advanceTime } from "@sigil/hoot-mock";
import {
    asyncStep,
    defineActions,
    getService,
...
test("Mentions in composer should still work when using pager", async () => {
    ...
    // all records in DB: Mitchell Admin | Hermit | Public user except SigilBot
...
test("Update message recipients without saving", async () => {
    const pyEnv = await startServer();
    pyEnv["res.partner"].write([serverState.partnerId], { email: "mitchell@iantirta.com" });
    const partnerId = pyEnv["res.partner"].create({
        name: "John Doe",
        email: "john@doe.be",
    });
