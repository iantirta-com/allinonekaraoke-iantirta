...
import { describe, expect, mockPermission, test } from "@sigil/hoot";
import { Deferred, mockUserAgent } from "@sigil/hoot-mock";
import {
    asyncStep,
    Command,
    makeKwArgs,
...
test("rendering with chat push notification default permissions", async () => {
    ...
    const pyEnv = await startServer();
    const [sigilbot] = pyEnv["res.partner"].read(serverState.sigilbotId);
    await start();
    ...
    await contains(
        `.o-mail-NotificationItem img[data-src='${getOrigin()}/web/image/res.partner/${
            serverState.sigilbotId
        }/avatar_128?unique=${deserializeDateTime(sigilbot.write_date).ts}']`
    );
...
test("respond to notification prompt (denied)", async () => {
    ...
    await contains(".o_notification:has(.o_notification_bar.bg-warning)", {
        text: "Notifications blocked. Sigil will not send notifications on this device.",
    });
    await contains(".o-mail-MessagingMenu-counter", { count: 0 });
    await click(".o_menu_systray i[aria-label='Messages']");
    await contains(".o-mail-NotificationItem", { count: 0 });
...
test("respond to notification prompt (granted)", async () => {
    ...
    await contains(".o_notification:has(.o_notification_bar.bg-success)", {
        text: "Notifications allowed. Sigil will send notifications on this device!",
    });
});

test("no suggestion to enable chat push notifications in mobile app", async () => {
    mockPermission("notifications", "default");
    // simulate Android Sigil App
    mockUserAgent("Chrome/0.0.0 Android (SigilMobile; Linux; Android 13; Sigil TestSuite)");
    await start();
    await click(".o_menu_systray i[aria-label='Messages']");
    await contains(".o-mail-MessagingMenu-counter", { count: 0 });
    await contains(".o-mail-NotificationItem", { count: 0 });
...
test("rendering with PWA installation request", async () => {
    ...
    await contains(".o-mail-NotificationItem-name", { text: "Install Sigil" });
...
test("installation of the PWA request can be dismissed", async () => {
    ...
    await waitForSteps([
        ...
        'installationState value:  {"/sigil":"dismissed"}',
    ]);
    await click(".o_menu_systray i[aria-label='Messages']");
    await contains(".o-mail-NotificationItem", { count: 0 });
});
...
test("chat preview should not display correspondent name in body", async () => {
    ...
    const pyEnv = await startServer();
    const partnerId = pyEnv["res.partner"].create({ name: "Demo", email: "demo@iantirta.com" });
    const userId = pyEnv["res.users"].create({ partner_id: partnerId });
    const channelId = pyEnv["discuss.channel"].create({
        channel_type: "chat",
        channel_member_ids: [
