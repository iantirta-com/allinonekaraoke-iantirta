...
# sigil-mailgate
#
# This program will read an email from stdin and forward it to sigil. Configure
# a pipe alias in your mail server to use it, postfix uses a syntax that looks
# like:
#
# email@address: "|/home/sigil/src/sigil-mail.py"
#
# while exim uses a syntax that looks like:
#
# *: |/home/sigil/src/sigil-mail.py
...
def main():
    ...
    op.add_option("-d", "--database", dest="database", help="Sigil database name (default: %default)", default='sigil')
    op.add_option("-u", "--userid", dest="userid", help="Sigil user id to connect with (default: %default)", default=1, type=int)
    op.add_option("-p", "--password", dest="password", help="Sigil user password (default: %default)", default='admin')
    op.add_option("--host", dest="host", help="Sigil host (default: %default)", default='localhost')
    op.add_option("--port", dest="port", help="Sigil port (default: %default)", default=8069, type=int)
    ...
    try:
        ...
        if e.faultString == 'Access denied' and e.faultCode == 3:
            ...
            postfix_exit(EX_NOUSER, "alias does not exist in sigil\n", o.debug)
        else:
...
try:
    ...
    # may include the password for the sigil user.
