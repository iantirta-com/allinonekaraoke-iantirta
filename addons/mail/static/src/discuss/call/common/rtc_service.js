...
import { reactive, toRaw } from "@sigil/owl";
...
/**
 * @return {Promise<{ SfuClient: import("@mail/../lib/sigil_sfu/sigil_sfu").SfuClient, SFU_CLIENT_STATE: import("@mail/../lib/sigil_sfu/sigil_sfu").SFU_CLIENT_STATE }>}
 */
const loadSfuAssets = memoize(async () => await loadBundle("mail.assets_sigil_sfu"));
...
export class Network {
    ...
    p2p;
    /** @type {import("@mail/../lib/sigil_sfu/sigil_sfu").SfuClient} */
    sfu;
    /** @type {[{ name: string, f: EventListener }]} */
    _listeners = [];
    /**
     * @param {import("@mail/discuss/call/common/peer_to_peer").PeerToPeer} p2p
     * @param {import("@mail/../lib/sigil_sfu/sigil_sfu").SfuClient} [sfu]
     */
    constructor(p2p, sfu) {
        this.p2p = p2p;
        this.sfu = sfu;
...
export class Rtc extends Record {
    ...
    async _loadSfu() {
        const load = async () => {
            await loadSfuAssets();
            const sfuModule = sigil.loader.modules.get("@mail/../lib/sigil_sfu/sigil_sfu");
...
export const rtcService = {
    ...
    /**
     * @param {import("@web/env").SigilEnv} env
