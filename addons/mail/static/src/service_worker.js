...
let db;
const unread_store = new Store("sigil-mail-unread-db", "sigil-mail-unread-store");
 ...
 * @param {Object} param1
 * @param {string} [param1.action] sigil client action
...
async function openDiscussChannel(channelId, { action, joinCall = false, source } = {}) {
    const discussURLRegexes = [new RegExp("/sigil/discuss")];
    if (action) {
        discussURLRegexes.push(
            new RegExp(`/sigil/\\d+/action-${action}`),
            new RegExp(`/sigil/action-${action}`)
        );
    }
    let targetClient;
    ...
    if (action) {
        const url = new URL(`/sigil/action-${action}`, location.origin);
...
self.addEventListener("notificationclick", (event) => {
    ...
    if (event.notification.data) {
        ...
        if (model === "discuss.channel") {
            ...
            event.waitUntil(clients.openWindow(`/sigil/${modelPath}/${res_id}`));
        }
    }
});
