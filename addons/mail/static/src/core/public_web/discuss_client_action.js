...
import { Component, onMounted, onWillStart, onWillUnmount, onWillUpdateProps } from "@sigil/owl";
...
export class DiscussClientAction extends Component {
    ...
    async restoreDiscussThread(props) {
        ...
        if (!parsedActiveId) {
            ...
            const sigilbotChat = this.store.sigilbot?.searchChat();
            const selfMember = sigilbotChat?.self_member_id;
            if (sigilbotChat && selfMember?.is_pinned && !selfMember.seen_message_id) {
                sigilbotChat.setAsDiscussThread(false);
            }
            return;
        }
