...
from sigil.exceptions import MissingError
from sigil.http import request
from sigil.tools import groupby
from sigil.addons.bus.websocket import wsrequest
...
def get_sfu_url(env) -> str | None:
    ...
    if not sfu_url:
        sfu_url = os.getenv("SIGIL_SFU_URL")
    if sfu_url:
...
def get_sfu_key(env) -> str | None:
    ...
    if not sfu_key:
        return os.getenv("SIGIL_SFU_KEY")
    return sfu_key
    ...
    def _add_values(self, values, model_name, index=None):
        ...
        for key, val in values.items():
            ...
            if isinstance(val, Store.Relation):
                ...
                target[key] = sigil.fields.Datetime.to_string(val)
            elif isinstance(val, date):
                target[key] = sigil.fields.Date.to_string(val)
