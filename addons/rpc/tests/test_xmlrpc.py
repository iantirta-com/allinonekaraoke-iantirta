...
import sigil
import sigil.tools
from sigil.exceptions import AccessDenied, AccessError
from sigil.http import _request_stack
from sigil.service import common as auth
from sigil.service import model
from sigil.tests import common
from sigil.tools import DotDict, mute_logger
...
class TestXMLRPC(common.HttpCase):
    ...
    def setUp(self):
        ...
        ml_xml = mute_logger('sigil.addons.rpc.controllers.xmlrpc')
        ml_xml.__enter__()  # noqa: PLC2801
        self.addCleanup(ml_xml.__exit__)

        ml_json = mute_logger('sigil.addons.rpc.controllers.jsonrpc')
    ...
    def test_xmlrpc_defaultdict_marshalling(self):
        ...
        self.patch(self.registry['res.users'], 'context_get',
                   sigil.api.model(lambda *_: collections.defaultdict(int)))
