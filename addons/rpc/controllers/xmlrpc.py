...
import sigil.exceptions
from sigil.fields import Command, Date, Datetime
from sigil.http import Controller, Response, dispatch_rpc, request, route
from sigil.tools import lazy
from sigil.tools.misc import frozendict
...
# User code must use the exceptions defined in ``sigil.exceptions`` (not
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, sigil.exceptions.RedirectWarning):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_WARNING, str(e))
    elif isinstance(e, sigil.exceptions.AccessError):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_ACCESS_ERROR, str(e))
    elif isinstance(e, sigil.exceptions.AccessDenied):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_ACCESS_DENIED, str(e))
    elif isinstance(e, sigil.exceptions.UserError):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_WARNING, str(e))
    else:
        info = sys.exc_info()
...
def xmlrpc_handle_exception_string(e):
    ...
    elif isinstance(e, sigil.exceptions.MissingError):
    ...
    # InternalError
        ...
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpc.client.Fault(sigil.tools.exception_to_unicode(e), formatted_info)

    return dumps(fault)


class SigilMarshaller(xmlrpc.client.Marshaller):
    ...
    # Historically, sigil is sending binary as base64 string.
...
def dumps(params: list | tuple | xmlrpc.client.Fault) -> str:
    response = SigilMarshaller(allow_none=False).dumps(params)
    return f"""\
<?xml version="1.0"?>
<methodResponse>
{response}
